...
export default function AIAssistantHealthPage() {
  ...
  const [deterministicCount, setDeterministicCount] = useState(1000);
  ...
  useEffect(() => {
    ...
    validateMarketingAccessKey(stored).then(async () => {
      ...
      const payload = await loadAssistantHealthConfiguration();
  ...
  async function unlock(event) {
    ...
    try {
      ...
      const payload = await loadAssistantHealthConfiguration(); setConfiguration(payload.configuration);
  ...
  async function runValidation(live) {
    const total = live ? Math.min(100, Math.max(50, Number(liveCount) || 50)) : Math.min(10000, Math.max(1, Number(deterministicCount) || 1000));
    const batchSize = live ? configuration.live_batch_limit : configuration.deterministic_batch_limit;
    cancelRef.current = false; setError(""); setReport(null); setProgress({ running: true, completed: 0, total });
  ...
  function saveBaseline() { if (!report) return; localStorage.setItem(BASELINE_KEY, JSON.stringify(report)); setBaseline(report); }
  ...
  return <div className="page-stack competence-page">
    <section className="operations-summary competence-hero"><div><div className="eyebrow">Internal regression suite</div><h2>AI Assistant Health</h2><p>Validate orchestration, approved retrieval, product boundaries, conversation memory and application progression after every PR. Deterministic runs never call OpenAI or write test/customer records.</p></div><div className="competence-target"><strong>{configuration?.scenario_library_size || "—"}</strong><span>source scenarios</span></div></section>
    ...
    <div className="card-actions"><button className="button button--ghost" disabled={!report} onClick={saveBaseline}>Save as PR baseline</button><button className="button button--ghost" disabled={!report} onClick={exportReport}>Export JSON report</button>{baseline ? <span className="badge">Baseline: {baseline.commit?.slice(0, 7) || "local run"}</span> : null}</div>
    <HealthReport report={report} baseline={baseline?.mode === mode ? baseline : null} live={mode === "live"} />
  </div>;
}
