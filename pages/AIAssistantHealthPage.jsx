...
export default function AIAssistantHealthPage() {
  ...
  return <div className="page-stack competence-page">
    ...
    {mode === "deterministic" ? <section className="panel"><div className="panel__header"><div><h3>Run up to 10,000 synthetic conversations</h3><p>Baseline One defaults to the full 10,000. It runs the existing server classifiers, memory, journey, orchestrator, lexical retrieval and product filters. Approved evidence is selected deterministically; OpenAI, geocoding and database writes are disabled.</p></div></div><div className="field-grid"><label className="field"><span className="field__label">Conversation count</span><input className="field__input" type="number" min="1" max="10000" value={deterministicCount} onChange={(event) => setDeterministicCount(event.target.value)} /></label></div><div className="card-actions"><button className="button button--primary" disabled={progress.running} onClick={() => runValidation(false)}>Run Deterministic Validation</button>{progress.running ? <button className="button button--ghost" onClick={() => { cancelRef.current = true; }}>Stop after current batch</button> : null}</div></section> : <section className="panel"><div className="panel__header"><div><h3>Sample 50–100 conversations with OpenAI</h3><p>Baseline One defaults to 100. This paid mode is manually started from the protected Marketing CRM session and uses the configured server-side model. It never creates customer records or competence results.</p></div></div>{configuration?.live_validation_available ? <><div className="field-grid"><label className="field"><span className="field__label">Representative sample size</span><input className="field__input" type="number" min="50" max="100" value={liveCount} onChange={(event) => setLiveCount(event.target.value)} /></label></div><div className="notice"><strong>Model:</strong> {configuration.model}<br /><strong>Cost estimates:</strong> {configuration.pricing_configured ? "Configured" : "Unavailable until both pricing environment variables are set"}<br /><strong>Execution:</strong> protected production · 2 conversations per batch · write-free until you save a baseline</div><button className="button button--primary" disabled={progress.running} onClick={() => runValidation(true)}>Run Live AI Validation</button></> : <div className="notice notice--warning">Paid Live AI Validation is disabled by default. The customer assistant is unaffected. Enable AI_HEALTH_ENABLE_LIVE_VALIDATION=true only for a deliberate paid regression run, then disable it again afterwards.</div>}</section>}
