...
import {
  ...
  validateMarketingCampaignAccess,
...
import {
  createCampaignDraftFromArticle,
...
function ArticleCampaignBuilder({ onCreated }) {
  ...
  useEffect(() => {
    ...
    loadAiMarketingPlatform()
      .then((result) => {
        if (!active) return;
        setArticles((result.articles || []).filter((article) => article.status === "approved"));
        setAssets((result.assets || []).filter((asset) => asset.channel === "email" && asset.status !== "archived"));
      })
      .catch((loadError) => {
        if (active && !isMarketingAccessDenied(loadError)) setError(loadError.message || "Approved articles could not be loaded.");
      });
    return () => { active = false; };
  }, []);
  ...
  function chooseArticle(value) {
    ...
    const emailAsset = assets.find((asset) => asset.source_article_id === value);
    setArticleId(value);
    setAssetId(emailAsset?.id || "");
    setName(article ? `Article campaign · ${article.title}` : "");
    setSubjectLine(emailAsset?.title || article?.seo_title || article?.title || "");
    setPreviewText(emailAsset?.preview_text || article?.excerpt || "");
    setEmailDraft(emailAsset?.body || "");
    setMessage("");
  }
  ...
  async function createDraft() {
    ...
    setError("");
    try {
      const result = await createCampaignDraftFromArticle({
        ...
        audience: { pipeline, source, required_tags: [], exclude_tags: [], last_seen_period: "all", created_period: "all", exclude_unknown_pipeline: false },
      });
      setMessage(`Campaign draft "${result.campaign.name}" is ready for review. Nothing was sent.`);
      setArticleId("");
      setAssetId("");
      setName("");
      setSubjectLine("");
      ...
      setError(createError.message || "Campaign draft could not be created.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <section className="panel">
      <div className="panel__header">
        <div>
          ...
          <h3>Create Campaign From Article</h3>
          <p>Approved article → audience selection → email draft → campaign draft → ready for review. No sending.</p>
        </div>
        <span className="status-pill stat-card--amber">Draft only</span>
      </div>
      <div className="field-grid">
        <label className="field" style={{ gridColumn: "1 / -1" }}><span className="field__label">Approved article</span><select className="field__input" value={articleId} onChange={(event) => chooseArticle(event.target.value)}><option value="">Select approved article</option>{articles.map((article) => <option key={article.id} value={article.id}>{article.title}</option>)}</select></label>
        <label className="field"><span className="field__label">Audience pipeline</span><select className="field__input" value={pipeline} onChange={(event) => setPipeline(event.target.value)}>{Object.entries(pipelineLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        <label className="field"><span className="field__label">Audience source</span><select className="field__input" value={source} onChange={(event) => setSource(event.target.value)}><option value="all">All Sources</option></select></label>
        <label className="field" style={{ gridColumn: "1 / -1" }}><span className="field__label">Campaign name</span><input className="field__input" value={name} onChange={(event) => setName(event.target.value)} /></label>
        <label className="field"><span className="field__label">Subject line</span><input className="field__input" value={subjectLine} onChange={(event) => setSubjectLine(event.target.value)} /></label>
        <label className="field"><span className="field__label">Preview text</span><input className="field__input" value={previewText} onChange={(event) => setPreviewText(event.target.value)} /></label>
        <label className="field" style={{ gridColumn: "1 / -1" }}><span className="field__label">Email draft for review</span><textarea className="field__input" rows="10" value={emailDraft} onChange={(event) => setEmailDraft(event.target.value)} placeholder="Select an article with an Email Content Factory asset, or prepare the draft manually." /></label>
      </div>
      <div className="notice">Creating this record does not calculate an audience, generate a batch, send email or change the source article.</div>
      {error ? <div className="notice notice--error">{error}</div> : null}
      {message ? <div className="notice knowledge-notice-success">{message}</div> : null}
      <div className="card-actions"><button className="button button--primary" type="button" disabled={busy || !articleId || !name.trim() || !emailDraft.trim()} onClick={createDraft}>{busy ? "Creating draft..." : "Create Campaign Draft"}</button></div>
    </section>
  );
}
...
export default function MarketingCentrePage() {
  ...
  function clearProtectedMarketingCentreState() {
    setCampaigns([]);
    setCampaignStats(EMPTY_CAMPAIGN_STATS);
    setAudienceOptions({ sources: [], tags: [] });
    setOpportunities([]);
  ...
  function lockMarketingCentre(message = "") {
    clearMarketingAccessKey();
    clearProtectedMarketingCentreState();
    setAccessStatus("locked");
    setAccessMessage(typeof message === "string" ? message : "");
    setError("");
  }
  ...
  async function loadCampaigns() {
    ...
    try {
      ...
      if (isMarketingAccessDenied(loadError)) return;
  ...
  async function loadAudienceOptions() {
    try {
      ...
      if (isMarketingAccessDenied(optionsError)) return;
    ...
    async function checkStoredAccess() {
      ...
      try {
        ...
        if (!cancelled) {
          clearMarketingAccessKey();
          setAccessStatus("locked");
          setAccessMessage(isMarketingAccessDenied(accessError) ? "Your saved access has expired or is no longer valid. Please unlock again." : accessError.message || "Could not validate saved Marketing Centre access.");
        }
      }
    }
  ...
  useEffect(() => {
    function handleAccessDenied(event) {
      lockMarketingCentre(event?.detail?.message || "Your saved access has expired or is no longer valid. Please unlock again.");
    }
  ...
  useEffect(() => { if (accessStatus === "unlocked") loadCampaigns(); }, [accessStatus, includeArchived]);
  useEffect(() => { if (accessStatus === "unlocked") { loadAudienceOptions(); loadOpportunities(); } }, [accessStatus]);
  ...
  function openOpportunityCampaign(opportunity) {
    if (!opportunity?.campaign_creation_supported) return;    resetWizard(opportunity?.recommended_channel || "email", opportunity);
    setSelectedCampaign(null);
    setModalMode("wizard");
  }
  ...
  async function handleUnlockMarketingCentre(event) {
    event.preventDefault();
    const apiKey = accessKeyInput.trim();
    if (!apiKey) {
      setAccessMessage("Enter the Marketing access key.");
      return;
    }
    ...
    try {
      ...
      if (!saveMarketingAccessKey(apiKey)) {
        setAccessMessage("Could not save the API key in this browser.");
        return;
      }
      setAccessKeyInput("");
      setAccessStatus("unlocked");
    } catch (unlockError) {
      setAccessMessage(isMarketingAccessDenied(unlockError) ? "Access key not recognised." : unlockError.message || "Could not validate Marketing Centre access.");
    } finally {
      setUnlocking(false);
    }
  }
