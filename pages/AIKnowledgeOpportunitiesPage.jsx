...
import { analyseExistingCompetenceResults, createOpportunityArticleDraft, createOpportunityFaqDraft, linkOpportunityArticle, loadKnowledgeOpportunities, updateKnowledgeOpportunity } from "../services/aiKnowledgeOpportunities.js";
...
function OpportunityCard({ item, selected, onSelect }) {
  return <button type="button" className={`panel competence-library-card${selected ? " is-selected" : ""}`} onClick={onSelect} style={{ textAlign: "left", width: "100%" }}>
    <div className="panel__header"><div><span className="badge">{item.product}</span><h3>{item.title}</h3><p>{item.diagnosis}</p></div><strong>{item.priority_score} · {item.priority_level}</strong></div>
    ...
    <small>{OPPORTUNITY_STATUS_LABELS[item.status]} · Last seen {fmt(item.last_seen_at)}</small>
  </button>;
}
...
function OpportunityDetail({ item, busy, onRefresh, setMessage, setError }) {
  ...
  return <div className="page-stack">
    <section className="panel"><div className="panel__header"><div><div className="eyebrow">Opportunity detail</div><h2>{item.title}</h2><p>{item.summary}</p></div><span className="badge">{item.product} · {item.priority_level}</span></div>
      ...
      <p><strong>Diagnosis:</strong> {item.diagnosis}<br /><strong>Suggested action:</strong> {String(item.recommended_action || "").replaceAll("_", " ")}<br /><strong>Intent/category:</strong> {item.normalised_intent} / {item.category}<br /><strong>First/last seen:</strong> {fmt(item.first_seen_at)} / {fmt(item.last_seen_at)}</p>
...
export default function AIKnowledgeOpportunitiesPage() {
  ...
  const filtered = useMemo(() => (data?.opportunities || []).filter((item) => product === "all" || item.product === product).filter((item) => status === "all" || (status === "active" ? !["dismissed", "completed"].includes(item.status) : item.status === status)), [data, product, status]);
  ...
  return <div className="page-stack"><section className="operations-summary competence-hero"><div><div className="eyebrow">Knowledge Learning Engine V2</div><h2>AI Knowledge Opportunities</h2><p>Turn weak, unanswered and repeated customer questions into grouped, prioritised improvements. Nothing is approved or published automatically.</p></div><button className="button button--primary" disabled={busy} onClick={analyse}>{busy ? "Working…" : "Analyse Existing Competence Results"}</button></section>
    ...
    {data ? <><section className="panel"><div className="competence-metrics">{Object.entries(SUMMARY_LABELS).map(([key, label]) => <Metric label={label} value={data.summary[key] || 0} key={key} />)}</div></section>
      ...
      {selected ? <><button className="button button--ghost" onClick={() => setSelectedId(null)}>← Back to grouped opportunities</button><OpportunityDetail item={selected} busy={busy} onRefresh={refresh} setMessage={setMessage} setError={setError} /></> : <div className="page-stack">{filtered.length ? filtered.map((item) => <OpportunityCard item={item} selected={false} onSelect={() => setSelectedId(item.id)} key={item.id} />) : <div className="competence-empty">No opportunities match these filters. Run the controlled backfill when ready.</div>}</div>}
