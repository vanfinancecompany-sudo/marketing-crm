...
function displayItem(item) {
  ...
  const field = item?.section || "Article";
  const phrase = item?.phrase || item?.type || "Issue";
  const excerpt = item?.excerpt ? ` — ${item.excerpt}` : "";
  return `${field}: ${phrase}${excerpt}`;
}

function ListNotice({ title, items, error = false }) {
  if (!items?.length) return null;
  return (
    <div className={`notice ${error ? "notice--error" : ""}`} style={{ marginTop: 12 }}>
      <strong>{title}</strong>
      <ul>
...
function Field({ label, value }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <strong>{label}</strong>
      <pre style={{ whiteSpace: "pre-wrap", margin: 0 }}>{value || "—"}</pre>
    </div>
  );
}

function DiffColumn({ title, article }) {
  const faqText = (article?.faq_json || [])
    .map((item, index) => `${index + 1}. ${item?.question || ""}\n${item?.answer || ""}`)
    .join("\n\n");
  const linkText = (article?.internal_link_suggestions || [])
    .map(
      (item) =>
        `${item?.anchor_text || item?.label || ""} → ${
          item?.destination_url || item?.url || ""
        }`
    )
    .join("\n");
...
function CorrectionPanel({ articleTitle }) {
  ...
  const needsCorrection =
    safety.hard_blocked ||
    Object.values(safety.checks || {}).some((value) => value !== "passed");

  async function propose(reasons = []) {
    setStatus("working");
    setMessage("Correction in progress");
    setLargeReductionConfirmed(false);
    try {
      const result = await proposePublishingCorrection(article.id, reasons);
      setProposal(result.proposal);
      setStatus("ready");
      setMessage(result.proposal.correction_complete ? "Proposed correction ready" : "Correction incomplete");
    } catch (error) {
      setStatus("idle");
  ...
  async function accept() {
    ...
    try {
      ...
      await acceptPublishingCorrection(proposal, largeReductionConfirmed);
  ...
  const needsLossReview =
    proposal &&
    (proposal.excessive_content_loss || proposal.unexplained_content_loss_percent > 20);
  const acceptDisabled =
    status === "working" ||
    !proposal?.correction_complete ||
    !proposal?.markdown_structure_valid ||
    proposal?.rent2buy_semantic_valid === false ||
    (needsLossReview && !largeReductionConfirmed);
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      {message ? (
        <div
          className={`notice ${proposal && !proposal.correction_complete ? "notice--error" : ""}`}
          style={{ marginTop: 12 }}
        >
      ...
      {proposal ? (
        <>
          <div className="notice" style={{ marginTop: 12 }}>
            ...
            <p>
              Content retained: {proposal.content_retained_percent}% · Valid removals:{" "}
              {proposal.valid_removed_word_count} words · Unexplained removals:{" "}
              {proposal.unexplained_removed_word_count} words
            </p>
          </div>
          ...
          {proposal.rent2buy_semantic_valid === false ? (
            <>
              <div className="notice notice--error" style={{ marginTop: 12 }}>
                <strong>Rent2Buy correction still contains prohibited wording.</strong>
              </div>
              <ListNotice
                title="Field-specific unresolved issues"
                items={proposal.rent2buy_semantic_errors}
                error
              />
            </>
          ) : null}
          ...
          {!proposal.markdown_structure_valid ? (
            <ListNotice
              title="Correction damaged article formatting."
              items={proposal.markdown_structure_errors}
              error
            />
          ) : null}
          ...
          <div className="card-actions" style={{ marginTop: 12 }}>
            ...
            {!proposal.correction_complete ? (
              <button
                ...
                onClick={() =>
                  propose(
                    proposal.rent2buy_semantic_errors?.length
                      ? proposal.rent2buy_semantic_errors
                      : proposal.unresolved_reasons || proposal.remaining_hard_blocks || []
                  )
                }
              >
            ...
            <button
              ...
              onClick={() => {
                setProposal(null);
                setStatus("idle");
                setMessage("Corrections discarded. The article was not changed.");
              }}
            >
              Discard Corrections
            </button>
  ...
  return (
    <div style={{ marginTop: 12 }}>
      <div className="card-actions">
        <button type="button" className="button button--ghost" disabled={busy} onClick={audit}>
          Select Published Articles for AI Fix
        </button>
        {flagged.length ? (
          <button
            type="button"
            ...
            onChange={(event) =>
              setSelected((current) =>
                event.target.checked
                  ? [...current, item.article.id].slice(0, 5)
      ...
      {results.map((item) => (
        <div
          className={`notice ${
            item.status === "failed" || !item.proposal?.correction_complete ? "notice--error" : ""
          }`}
          key={item.article_id}
        >
          <strong>
            {item.status === "ready"
              ? item.proposal.correction_complete
