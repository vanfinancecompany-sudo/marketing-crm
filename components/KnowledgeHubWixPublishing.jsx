...
import { createOrUpdateWixDraft, resetAcceptedInternalLinks } from "../services/wixPublishing.js";
...
export default function KnowledgeHubWixPublishing({ article, linkSuggestions = [], hasUnsavedChanges = false, assessment = null, businessKnowledge = [], onSynced }) {
  ...
  async function resetLinks(){if(running.current||busy||!acceptedCount)return;if(!window.confirm(`Reset ${acceptedCount} accepted internal link${acceptedCount===1?"":"s"} to Pending? You can then accept or reject them again.`))return;running.current=true;setBusy(true);setStage("Resetting accepted links…");setError(null);try{const response=await resetAcceptedInternalLinks(currentArticle.id);setPartial(null);setResult(null);setStage(`${response.reset_count||0} accepted link${response.reset_count===1?"":"s"} reset to Pending.`);window.location.reload();}catch(caught){setError(caught.message||"Accepted internal links could not be reset.");}finally{setBusy(false);running.current=false;}}
  ...
  return <>
    ...
    <section className="panel knowledge-wix-publishing">
      ...
      {error?<div className="notice notice--error"><strong>{partial?"Article approved, but Wix draft creation failed.":"Action could not be completed."}</strong><div>{error}</div><div className="card-actions" style={{marginTop:10}}>{acceptedCount>0?<button className="button button--danger" type="button" disabled={busy} onClick={resetLinks}>Reset accepted links</button>:null}{partial?.retry_wix?<button className="button button--ghost" type="button" disabled={busy||technicalBlocked} onClick={retryWix}>Retry Wix Draft</button>:null}</div></div>:null}
