...
function Modal({ title, children, confirmLabel, busy, onConfirm, onClose }) {
  return (
...
function FinderWorkspace() {
  ...
  return (
    <section className="page-stack" data-topic-workspace="finder">
      <div className="panel knowledge-form-panel">
        ...
        <div className="knowledge-category-picker">
          {KNOWLEDGE_CATEGORIES.map((category) => <label key={category}><input type="checkbox" checked={categories.includes(category)} onChange={() => setCategories((current) => current.includes(category) ? current.filter((item) => item !== category) : [...current, category])} />{category}</label>)}
        </div>
      ...
      {ideas.length ? <div className="panel">
        ...
        <div className="knowledge-finder-grid">
          {ideas.map((idea, index) => <article className="knowledge-finder-card" key={`${idea.title}-${index}`}>
            ...
            <h4>{idea.title}</h4>
            <p><strong>{idea.category}</strong></p>
            ...
            <div className="card-actions">
              <button className="button button--primary" disabled={busy} onClick={() => saveIndexes([index])}>Save to Topic Planner</button>
              <button className="button button--danger" disabled={busy} onClick={() => reject([index])}>Reject</button>
            </div>
          </article>)}
        </div>
      </div> : null}
...
function PlannerWorkspace({ legacyPanel }) {
  ...
  async function load() {
    try {
      ...
      setDuplicates(result.duplicate_groups || []);
  ...
  function proxyLegacy(topic, label) {
    ...
    const button = [...(row?.querySelectorAll("button") || [])].find((candidate) => candidate.textContent?.trim() === label);
    button?.click();
  }
...
function mount() {
  ...
  const heading = headings.find((item) => ["Topic Planner", "AI Topic Finder"].includes(item.textContent?.trim()));
  if (!heading) {
  ...
  if (panel?.dataset.topicWorkspace) return;
  if (hiddenPanel && hiddenPanel !== panel) hiddenPanel.style.display = "";
  hiddenPanel = panel;
