...
import { enforceRent2BuyReplyBoundary } from "./assistantProductBrains.js";
...
const UNSUPPORTED_OFFER_QUESTION = /^(?:would you like me to|shall i|do you want me to|want me to|would you like me to tell you more|would you like to know more)\b/i;
...
function removeUnsupportedActionOffers(value) {
  const parts = sentences(value);
  const unsupported = (sentence) => UNSUPPORTED_ACTION_OFFER.test(sentence) || UNSUPPORTED_OFFER_QUESTION.test(sentence.trim());
  if (parts.length <= 1) return unsupported(value) ? "" : value;
  return parts.filter((sentence) => !unsupported(sentence)).join(" ").trim();
}
...
export function enforceAnswerableFollowUp(value, { intent = {}, journey = {}, orchestration = {}, insufficientKnowledge = false } = {}) {
  ...
  if (!unsupportedOfferRemoved) {
    return { reply: value, supported_question: supportedQuestion, guard_applied: false, unsupported_offer_removed: false };
  }

  const fallback = supportedQuestion || "I can answer questions using the information available here, but I won’t pretend to perform an action I can’t complete.";
  return {
    reply: withoutUnsupportedOffer || fallback,
    supported_question: supportedQuestion,
    guard_applied: true,
    unsupported_offer_removed: true,
  };
}
...
export function polishConversationPresentation({
  ...
  if (productContext === "rent2buy") polished = enforceRent2BuyReplyBoundary(polished);
