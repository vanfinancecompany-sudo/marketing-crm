...
export function polishConversationPresentation({
  ...
  if (shouldResume || shouldOfferEarly) {
  ...
  } else if (intent.retrieval_required && ["Interested", "High Intent"].includes(journey.buying_intent_level) && journey.next_best_question && !polished.includes("?")) {
    polished = `${polished}\n\n${journey.next_best_question}`.trim();
    transitionApplied = true;
  }
  ...
  return {
    ...
    transition_type: shouldResume ? "resume_application" : shouldOfferEarly ? "offer_application" : transitionApplied ? "resume_sales_conversation" : "none",
    ...diagnostics,
  };
}
