...
export function buildBusinessIntelligencePrompt({
  sections = [],
  settings = {},
  specialist = {},
  topic = {},
  ...
  const prompt = `# Task
${task === "article_review" ? "Review the supplied article without rewriting it." : "Create a new knowledge article draft."}
...
export function parseKnowledgeArticleReviewResponse(value) {
  ...
  if (!categoryKeys.every((key) => parsed.categories?.[key])) {
