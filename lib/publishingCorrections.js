...
function preserveLinkDestinations(original = [], proposed = []) {
  return (Array.isArray(original) ? original : []).map((item, index) => {
    ...
    return { ...item, anchor_text: clean(candidate.anchor_text || candidate.label || item.anchor_text || item.label), label: clean(candidate.label || candidate.anchor_text || item.label || item.anchor_text), destination_url: clean(item.destination_url || item.url || item.destination), url: clean(item.url || item.destination_url || item.destination), status: item.status };
  });
}
...
function articleFieldValues(article = {}) {
  const values = [
    ["Article title", article.title], ["SEO title", article.seo_title], ["Meta description", article.meta_description], ["Excerpt", article.excerpt], ["Article body", article.content_markdown], ["CTA", article.cta],
  ];
  (article.faq_json || []).forEach((faq, index) => { values.push([`FAQ ${index + 1} question`, faq?.question], [`FAQ ${index + 1} answer`, faq?.answer]); });
  (article.internal_link_suggestions || []).forEach((link, index) => { values.push([`Internal-link anchor ${index + 1}`, link?.anchor_text || link?.label], [`Internal-link destination ${index + 1}`, link?.destination_url || link?.url]); });
  return values.filter(([, value]) => value !== undefined && value !== null).map(([field, value]) => ({ field, value: String(value) }));
}
...
function markdownDestinations(markdown = "") { return [...String(markdown || "").matchAll(/\[[^\]]*\]\(([^)]+)\)/g)].map((match) => clean(match[1])); }
function excerptFor(value, token) { const index = String(value || "").indexOf(token); if (index < 0) return clean(value).slice(0, 140); return clean(String(value).slice(Math.max(0, index - 45), Math.min(String(value).length, index + token.length + 65))); }
function tokenRecords(article, kind) {
  const pattern = kind === "url" ? URL_TOKEN_PATTERN : EMAIL_TOKEN_PATTERN;
  return articleFieldValues(article).flatMap(({ field, value }) => extractTokens(value, pattern).map((token) => ({ field, value: token, excerpt: excerptFor(value, token) })));
}
...
export function validateProtectedValues(before = {}, after = {}, options = {}) {
  ...
  for (const kind of ["url", "email"]) {
    ...
    fields.forEach((field) => {
      ...
      const max = Math.max(originals.length, proposals.length);
      for (let index = 0; index < max; index += 1) if (originals[index] !== proposals[index]) errors.push(protectedError(field, originals[index], proposals[index], `${kind}_changed`, (afterMap.get(field) || [])[index]?.excerpt || (beforeMap.get(field) || [])[index]?.excerpt || ""));
    });
  }
  ...
  for (let index = 0; index < Math.max(beforeLinks.length, afterLinks.length); index += 1) {
    const original = clean(beforeLinks[index]?.destination_url || beforeLinks[index]?.url); const proposed = clean(afterLinks[index]?.destination_url || afterLinks[index]?.url);
    if (original !== proposed) errors.push(protectedError(`Internal-link destination ${index + 1}`, original, proposed, "internal_link_destination_changed", `${afterLinks[index]?.anchor_text || afterLinks[index]?.label || ""} → ${proposed}`));
    if (proposed && (/^\s|\s$/.test(afterLinks[index]?.destination_url || afterLinks[index]?.url || "") || /\s/.test(proposed))) errors.push(protectedError(`Internal-link destination ${index + 1}`, original, proposed, "internal_link_url_whitespace", proposed));
  }
  const beforeCompany = articleFieldValues(before).flatMap(({ field, value }) => [...value.matchAll(new RegExp(COMPANY_NAME, "g"))].map(() => field));
  const afterCompany = articleFieldValues(after).flatMap(({ field, value }) => [...value.matchAll(new RegExp(COMPANY_NAME, "g"))].map(() => field));
  if (beforeCompany.length !== afterCompany.length) errors.push(protectedError("Company name", COMPANY_NAME, afterCompany.length ? COMPANY_NAME : "", "company_name_changed", articleFieldValues(after).map((item) => item.value).join(" ")));
  if (before.content_markdown?.includes(RENT2BUY_SEPARATION_SENTENCE) && !after.content_markdown?.includes(RENT2BUY_SEPARATION_SENTENCE)) errors.push(protectedError("Article body", RENT2BUY_SEPARATION_SENTENCE, "", "approved_sentence_changed", after.content_markdown));
  ...
  return { protected_values_valid: errors.length === 0, protected_value_errors: errors };
}
...
export function validateTargetedRepairText(article = {}) {
  ...
  articleFieldValues(article).forEach(({ field, value }) => patterns.forEach(([pattern, message]) => { if (pattern.test(value)) errors.push({ field, error_type: "replacement_corruption", excerpt: excerptFor(value, value.match(pattern)?.[0] || ""), message }); }));
  tokenRecords(article, "url").forEach((record) => { try { const parsed = new URL(record.value); if (!parsed.hostname || /\s/.test(record.value)) errors.push({ field: record.field, error_type: "invalid_url", excerpt: record.excerpt, message: `Invalid URL: ${record.value}` }); } catch { errors.push({ field: record.field, error_type: "invalid_url", excerpt: record.excerpt, message: `Invalid URL: ${record.value}` }); } });
...
const SENTENCE_REWRITES = [
  ...
  { pattern: /\bnew to finance\b/i, replacement: "Applicants with a range of credit histories may apply, subject to the Rent2Buy eligibility process." },
...
function sentenceSpan(text, phrase) {
  const lower = text.toLowerCase(); const index = lower.indexOf(clean(phrase).toLowerCase()); if (index < 0) return null;
  let start = index; while (start > 0 && !/[.!?\n]/.test(text[start - 1])) start -= 1; while (start < index && /\s/.test(text[start])) start += 1;
  let end = index + phrase.length; while (end < text.length && !/[.!?\n]/.test(text[end])) end += 1; if (end < text.length && /[.!?]/.test(text[end])) end += 1;
...
function rewriteSentenceAtError(text, error) {
  ...
  if (extractTokens(span.sentence, URL_TOKEN_PATTERN).length || extractTokens(span.sentence, EMAIL_TOKEN_PATTERN).length) return { text, repaired: false };
  const rule = SENTENCE_REWRITES.find((item) => item.pattern.test(span.sentence) || item.pattern.test(phrase)); if (!rule) return { text, repaired: false };
  const prefix = span.sentence.match(/^\s*(?:[-*+]\s+|\d+\.\s+|>\s*)/)?.[0] || "";
  let replacement = rule.replacement;
  if (/traditional credit checks/i.test(span.sentence)) replacement = span.sentence.replace(/traditional credit checks/gi, "the usual credit checks").trim();
  return { text: `${text.slice(0, span.start)}${prefix}${replacement}${text.slice(span.end)}`, repaired: true };
}
...
function repairBodyBySection(markdown, errors, report) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const matching = errors.filter((error) => clean(error.section) === heading && lines[index].toLowerCase().includes(clean(error.phrase).toLowerCase())); if (!matching.length) continue;
...
function removeRequiredStatementDuplicates(markdown = "") {
  let text = String(markdown || "");
  text = text.split(RENT2BUY_SEPARATION_SENTENCE).join("");
  text = text.split(RENT2BUY_COLLECTION_SENTENCE).join("");
  text = text.replace(/^\s*\*\*\s*(?:[.!?\s]|&nbsp;)*\*\*\s*$/gm, "").replace(/^\s*\*\*\s*\*\*\s*$/gm, "");
  const lines = text.split(/\r?\n/);
  const introIndex = lines.findIndex((line) => /^#{1,6}\s+Introduction\s*$/i.test(line.trim()));
  const featuresIndex = lines.findIndex((line) => /^#{1,6}\s+Key Features(?: and Benefits)?\s*$/i.test(line.trim()));
  if (introIndex >= 0) { let at = introIndex + 1; while (at < lines.length && !lines[at].trim()) at += 1; lines.splice(at, 0, RENT2BUY_SEPARATION_SENTENCE, ""); }
  if (featuresIndex >= 0) { const adjusted = lines.findIndex((line) => /^#{1,6}\s+Key Features(?: and Benefits)?\s*$/i.test(line.trim())); let at = adjusted + 1; while (at < lines.length && !lines[at].trim()) at += 1; lines.splice(at, 0, RENT2BUY_COLLECTION_SENTENCE, ""); }
...
function repairFaqs(faqs = [], errors = [], report) {
  ...
  (Array.isArray(faqs) ? faqs : []).forEach((faq, index) => {
    ...
    const question = repairTextLocation(faq?.question, flagged); const answer = repairTextLocation(faq?.answer, flagged); if (!question.repairedCount && !answer.repairedCount) { output.push(structuredClone(faq)); return; }
...
export function applyTargetedRent2BuyRepairs(article = {}, semanticErrors = [], options = {}) {
  ...
  repaired.content_markdown = removeRequiredStatementDuplicates(repaired.content_markdown);
