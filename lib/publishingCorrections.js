...
import { RENT2BUY_COLLECTION_SENTENCE, RENT2BUY_SEPARATION_SENTENCE, classifyArticleProduct, rent2BuyPromptRule, validateComparisonStructure, validateMarkdownStructure, validateRent2BuySemantics } from "./rent2BuyRules.js";
...
function safeRent2BuyTextRepair(value = "", errors = []) {
  ...
  const replacements = [
    ...
    [/finance approval/gi, "eligibility outcome"], [/specific rates/gi, "specific agreement terms"], [/finance applications?/gi, "Rent2Buy applications"], [/finance agreements?/gi, "Rent2Buy agreements"], [/traditional finance/gi, "other routes to obtaining a van"], [/van finance/gi, "van ownership"], [/lender panels?/gi, "eligibility process"], [/lenders?/gi, "the Rent2Buy team"], [/representative apr|\bapr\b|interest rates?|finance rates?/gi, "agreement terms"], [/hire purchase|lease purchase|\bpcp\b/gi, "another arrangement"], [/finance deposits?|\bdeposits?\b/gi, "initial payment requirements"], [/finance products?/gi, "arrangements"], [/\bfinance\b/gi, "Rent2Buy"],
    [/try(?:ing)? (?:the )?van(?: before (?:buying|committing))?/gi, "review the vehicle details and agreement terms carefully before proceeding"], [/test(?:ing)? (?:the )?van(?: before buying)?/gi, "review the vehicle details and agreement terms carefully before proceeding"], [/trial(?:ling)? (?:the )?van/gi, "review the vehicle details and agreement terms carefully before proceeding"], [/try before committing|try before buying|test before buying/gi, "review the vehicle details and agreement terms carefully before proceeding"], [/decide later after using it|decide after trying it/gi, "review the vehicle details and agreement terms carefully before proceeding"], [/rent before committing/gi, "make rental payments under the agreement"],
  ];
  replacements.forEach(([pattern, replacement]) => { text = text.replace(pattern, replacement); });
  for (const error of errors) {
    const phrase = clean(error?.phrase); if (!phrase || !text.toLowerCase().includes(phrase.toLowerCase())) continue;
  ...
  return text.replaceAll(SEPARATION_TOKEN, RENT2BUY_SEPARATION_SENTENCE).replace(/[ \t]{2,}/g, " ").trim();
}
...
function ensureCollectionPlacement(markdown = "") {
  ...
  let insertAt = headingIndex + 1; while (insertAt < lines.length && !lines[insertAt].trim()) insertAt += 1; lines.splice(insertAt, 0, RENT2BUY_COLLECTION_SENTENCE, ""); return normalizeMarkdown(lines.join("\n"));
}
...
function repairFaqs(faqs = [], errors = [], report) {
  ...
  (Array.isArray(faqs) ? faqs : []).forEach((faq, index) => {
    const flagged = errorsByFaq.get(index) || []; if (!flagged.length) { output.push({ question: clean(faq?.question), answer: clean(faq?.answer) }); return; }
    const combined = `${faq?.question || ""} ${faq?.answer || ""}`;
    if (flagged.some((item) => item.category === "delivery") || /delivery/i.test(combined)) { report.removed_faqs.push(clean(faq?.question) || `FAQ ${index + 1}`); if (!collectionFaqAdded && !output.some((item) => /where are rent2buy vans collected/i.test(item.question))) { output.push({ question: "Where are Rent2Buy vans collected?", answer: RENT2BUY_COLLECTION_SENTENCE }); report.rewritten_faqs.push("Where are Rent2Buy vans collected?"); collectionFaqAdded = true; } return; }
    ...
    output.push({ question: safeRent2BuyTextRepair(faq?.question, flagged), answer: safeRent2BuyTextRepair(faq?.answer, flagged) }); report.rewritten_faqs.push(clean(faq?.question) || `FAQ ${index + 1}`);
  });
...
export function applyTargetedRent2BuyRepairs(article = {}, semanticErrors = [], options = {}) {
  ...
  [["Article title", "title"], ["SEO title", "seo_title"], ["Meta description", "meta_description"], ["Excerpt", "excerpt"], ["CTA", "cta"]].forEach(([label, key]) => { const errors = byField.get(label) || []; if (errors.length) { repaired[key] = safeRent2BuyTextRepair(repaired[key], errors); report.repaired_fields.push(label); } });
  const bodyErrors = semanticErrors.filter((error) => /^Article\b/i.test(clean(error?.field || error?.section))); if (bodyErrors.length) { repaired.content_markdown = safeRent2BuyTextRepair(repaired.content_markdown, bodyErrors); report.repaired_fields.push("Article body"); }
  ...
  repaired.internal_link_suggestions = (repaired.internal_link_suggestions || []).map((link, index) => { const label = `Internal-link anchor ${index + 1}`; const errors = byField.get(label) || []; if (!errors.length) return link; report.repaired_fields.push(label); const anchor = safeRent2BuyTextRepair(link.anchor_text || link.label, errors); return { ...link, anchor_text: anchor, label: anchor }; });
  report.targeted_repairs_applied = true; report.repaired_fields = [...new Set(report.repaired_fields)]; report.repaired_phrases = [...new Set(report.repaired_phrases.filter(Boolean))]; return { article: repaired, ...report };
}
...
export function buildCorrectionPreview({ originalArticle, proposed, safetyOptions = {}, scopeOverride = "" }) {
  ...
  const firstSemanticValidation = validateRent2BuySemantics(normalized.corrected_article, { intent: safetyOptions.intent, scopeOverride: scope }); const repair = applyTargetedRent2BuyRepairs(normalized.corrected_article, firstSemanticValidation.rent2buy_semantic_errors, { intent: safetyOptions.intent, scopeOverride: scope }); normalized.corrected_article = repair.article;
