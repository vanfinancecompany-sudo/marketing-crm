...
export async function refreshArticleInternalLinks(
  ...
  const [articleResult, pagesResult, articlesResult, intentResult, settingsResult] =
    await Promise.all([
      ...
      supabase
        .from("knowledge_articles")
        .select("id,title,slug,seo_title,meta_description,excerpt,category,article_type,status")
        .eq("status", "approved"),
      supabase
  ...
  const knowledgeArticles = articlesResult.data || [];
  const websitePages = pagesResult.data || [];
