...
import { suggestInternalLinks } from "./internalLinking.js";
...
export async function refreshArticleInternalLinks(
  ...
  const [articleResult, pagesResult, articlesResult, intentResult, settingsResult] =
    await Promise.all([
      supabase
        ...
        .eq("id", articleId)
        .single(),
      supabase.from("knowledge_business_pages").select("*").eq("active", true),
      supabase
        .from("knowledge_articles")
  ...
  const currentByPage = new Map(
    existing
      .filter((item) => item.source_content_hash === sourceHash)
  ...
  const rows = suggestions
    .filter((item) => !acceptedPageIds.has(item.website_page_id))
  ...
  return [...created, ...retained].sort(
    (first, second) => second.confidence_score - first.confidence_score
  );
}
