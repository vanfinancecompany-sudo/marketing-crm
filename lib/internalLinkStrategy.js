...
const TOPIC_GENERIC_WORDS = new Set([
  "about", "after", "another", "article", "before", "business", "businesses", "buy", "buyer", "buyers", "buying",
  "can", "company", "complete", "consider", "considering", "first", "finance", "financial", "for",
  "from", "guide", "guides", "how", "information", "into", "it", "more", "most", "should", "time", "tips", "top",
  "the", "their", "this", "used", "uk", "van", "vans", "vehicle", "vehicles", "what",
  "when", "which", "with", "work", "you", "your",
]);
...
function articleHighSignalText(article = {}, topic = {}) {
  return [article.title, article.seo_title, clean(article.slug).replaceAll("-", " "), article.category, article.article_type, topic.title, topic.primary_keyword, ...(Array.isArray(topic.secondary_keywords) ? topic.secondary_keywords : []), topic.intent, markdownHeadings(article.content_markdown)].join(" ");
}

function linkedArticleTopicText(article = {}) {
  return [article.title, article.seo_title, clean(article.slug).replaceAll("-", " "), article.category, article.article_type, article.meta_description, article.excerpt].join(" ");
}

export function hasStrongKnowledgeTopicOverlap({ sourceArticle = {}, sourceTopic = {}, linkedArticle = {} } = {}) {
  const sourceTitle = topicTokens([sourceArticle.title, sourceArticle.seo_title, clean(sourceArticle.slug).replaceAll("-", " "), sourceTopic.title, sourceTopic.primary_keyword].join(" "));
  const destinationTitle = topicTokens([linkedArticle.title, linkedArticle.seo_title, clean(linkedArticle.slug).replaceAll("-", " ")].join(" "));
...
export function classifyKnowledgeLinkProduct({ article = {}, topic = {}, intent = {} } = {}) {
  const highSignal = normalized([article.title, article.seo_title, article.slug, article.category, article.article_type, topic.title, topic.category, topic.intent, intent.primary_product].join(" "));
...
export function isApplicationDestinationRelevant({ article = {}, topic = {}, intent = {}, product = "finance" } = {}) {
  const highSignal = normalized([article.title, article.seo_title, article.slug, article.category, article.article_type, topic.title, topic.primary_keyword, topic.intent, intent.customer_journey, intent.search_intent, intent.conversion_goal].join(" "));
  if (product === "rent2buy") return /\b(?:apply|application|eligib|proof|afford|rent2buy|rent 2 buy|rent to buy)\w*\b/.test(highSignal);
  return /\b(?:apply|application|approval|eligib|finance|apr|interest|lender)\w*\b/.test(highSignal);
}

export function filterInternalLinkCandidates({ article = {}, topic = {}, intent = {}, websitePages = [], knowledgeArticles = [] } = {}) {
  const sourceProduct = classifyKnowledgeLinkProduct({ article, topic, intent });
  const articleById = new Map((Array.isArray(knowledgeArticles) ? knowledgeArticles : []).map((item) => [item.id, item]));
  return (Array.isArray(websitePages) ? websitePages : []).filter((page) => {
    ...
    if (role === "application") return isApplicationDestinationRelevant({ article, topic, intent, product: sourceProduct });
    return true;
  });
}

export function selectFocusedInternalLinkSuggestions(suggestions = [], { maximumKnowledgeLinks = 2, maximumCommercialLinks = 2, minimumKnowledgeConfidence = 45 } = {}) {
  const ordered = Array.isArray(suggestions) ? suggestions : [];
  const knowledge = ordered.filter((item) => item.target_type === "knowledge_article").filter((item) => Number(item.confidence_score || 0) >= minimumKnowledgeConfidence).slice(0, Math.max(0, Number(maximumKnowledgeLinks) || 0));
  const commercial = [];
  ...
  return [...knowledge, ...commercial].sort((first, second) => Number(second.confidence_score || 0) - Number(first.confidence_score || 0) || clean(first.destination_title).localeCompare(clean(second.destination_title)));
}
