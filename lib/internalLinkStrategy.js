...
export function commercialDestinationRole(page = {}, product = "finance") {
  ...
  if (product === "rent2buy") {
    if (
      path === "/guaranteed-van-lease" ||
      /^what is rent2buy(?: van finance)?$/.test(title)
    ) return "home";

    if (
      path === "/rent2buy-application" ||
      /\brent2buy application\b/.test(title)
    ) return "application";

    if (
      path === "/rent2buy-all-vans" ||
      path === "/rent2buyvans" ||
      /\brent2buy vans no credit check\b/.test(title)
    ) return "stock";
  ...
  if (
    path === "/apply-by-reg-finance/application-form" ||
    /\bvan finance application form\b/.test(title)
  ) return "application";

  if (
    path === "/vans-on-finance" ||
    /\bview vans van finance\b/.test(title)
  ) return "stock";
...
export function filterInternalLinkCandidates({
  ...
  return (Array.isArray(websitePages) ? websitePages : []).filter((page) => {
    if (page.knowledge_article_id) {
      ...
      return classifyKnowledgeLinkProduct({ article: linkedArticle }) === sourceProduct;
    }

    return Boolean(commercialDestinationRole(page, sourceProduct));
  });
}
...
export function selectFocusedInternalLinkSuggestions(
  ...
  for (const item of ordered) {
    ...
    const product = /rent2buy|rent 2 buy|rent-to-buy/i.test(`${item.destination_title || ""} ${item.destination_url || ""}`)
      ? "rent2buy"
      : "finance";
    const role = commercialDestinationRole(
      { title: item.destination_title, url: item.destination_url },
      product
    );
