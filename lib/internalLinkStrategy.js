...
export function hasStrongKnowledgeTopicOverlap({ sourceArticle = {}, sourceTopic = {}, linkedArticle = {} } = {}) {
  ...
  const sourceFull = topicTokens([
    articleHighSignalText(sourceArticle, sourceTopic),
    sourceArticle.meta_description,
    sourceArticle.excerpt,
    sourceArticle.content_markdown,
  ].join(" "));
  const destination = topicTokens(linkedArticleTopicText(linkedArticle));
  if ([...destination].some((word) => sourceHigh.has(word))) return true;
  return [...destination].filter((word) => sourceFull.has(word)).length >= 2;
}
