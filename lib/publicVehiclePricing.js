import { classifyMonthlyAmountIntent } from "./conversationIntelligence.js";
...
const VEHICLE_SPEC_QUERY_OPEN = /^\s*(?:and\s+)?(?:is|are|does|do|has|have|what|what's|whats|which|how many|tell me|can you tell me)\b/i;
...
export function isSpecificVehiclePricingQuestion({ message = "", pageType = "", rememberedFacts = {} } = {}) {
  ...
  if (classifyMonthlyAmountIntent(text).kind === "budget_statement") return false;
...
function vehicleText(vehicleContext = {}) {
  const features = Array.isArray(vehicleContext.important_features) ? vehicleContext.important_features : [];
  return [vehicleContext.description, vehicleContext.highlights, vehicleContext.specification, ...features, JSON.stringify(vehicleContext.public_specifications || {})].map((value) => clean(value, 7000)).filter(Boolean).join("\n");
}
...
function vehicleOverviewReply(productLock, vehicleContext = {}) {
  ...
  const rawMileage = clean(vehicleContext.mileage || fields.MILEAGE, 80);
  const mileage = rawMileage ? `${rawMileage}${/miles?/i.test(rawMileage) ? "" : " miles"}` : "";
  const fuel = clean(vehicleContext.fuel || fields["FUEL TYPE"], 80).toLowerCase();
  const transmission = clean(vehicleContext.transmission || fields.TRANSMISSION, 80).toLowerCase();
  const engine = clean(vehicleContext.public_specifications?.engine_size || fields["ENGINE SIZE"], 80);
  const bhp = clean(vehicleContext.public_specifications?.power || fields.BHP, 80);
  const euro = clean(vehicleContext.public_specifications?.euro_standard || fields.EURO, 80).replace(/^euro\s*/i, "");
  const colour = clean(vehicleContext.colour || fields.COLOUR || fields.COLOR, 80).toLowerCase();
...
function attributeReply(message, vehicleContext = {}) {
  ...
  const rawMileage = clean(vehicleContext.mileage || fields.MILEAGE, 80);
  const mileage = rawMileage ? `${rawMileage}${/miles?/i.test(rawMileage) ? "" : " miles"}` : "";
  const transmission = clean(vehicleContext.transmission || fields.TRANSMISSION, 80);
  const fuel = clean(vehicleContext.fuel || fields["FUEL TYPE"], 80);
  const facts = [];
  const confirmedFeatures = [];
  const missing = [];
  const addRequestedFact = (pattern, value, missingLabel) => {
    if (!pattern.test(text)) return;
    if (value) facts.push(value);
    else missing.push(missingLabel);
  };
  addRequestedFact(/\bmileage|miles\b/, mileage, "the mileage");
  addRequestedFact(/\bautomatic|auto|manual|gearbox|transmission\b/, transmission ? `a ${transmission.toLowerCase()} gearbox` : "", "the gearbox");
  addRequestedFact(/\bfuel|diesel|petrol|electric|hybrid\b/, fuel ? fuel.toLowerCase() : "", "the fuel type");
  addRequestedFact(/\bengine size|engine capacity\b/, fields["ENGINE SIZE"] ? `a ${fields["ENGINE SIZE"]} engine` : "", "the engine size");
  addRequestedFact(/\bbhp|horsepower\b/, fields.BHP ? `${fields.BHP} BHP` : "", "the power output");
  addRequestedFact(/\bcolour|color\b/, fields.COLOUR || fields.COLOR ? `${(fields.COLOUR || fields.COLOR).toLowerCase()} paintwork` : "", "the colour");
  addRequestedFact(/\beuro ?6|\beuro\b/, fields.EURO ? `Euro ${String(fields.EURO).replace(/^euro\s*/i, "")} compliance` : "", "the Euro emissions standard");
  ...
  if (facts.length || missing.length) {
    ...
    const knownReply = uniqueFacts.length
      ? confirmedFeatures.length === uniqueFacts.length && !missing.length
        ? `Yes, this van lists ${naturalList(uniqueFacts)}.`
        : `This van has ${naturalList(uniqueFacts)}.`
      : "";
    const missingReply = missing.length ? `I can’t confirm ${naturalList([...new Set(missing)])} from the vehicle information for this van, so I won’t guess.` : "";
    return [knownReply, missingReply].filter(Boolean).join(" ");
  }
...
export function publicVehiclePricingReply({ message = "", pageType = "", productLock = "", vehicleContext = {}, rememberedFacts = {} } = {}) {
  ...
  if (termQuestion) {
    const term = normaliseVehicleTermMonths(vehicleContext?.term_months);
    return term
      ? `The current Rent2Buy vehicle page shows a ${term}-month term${subject}.`
      : "I can’t confirm the term for that specific Rent2Buy van from the current vehicle information, so I won’t guess. Please check the term shown on its listing or ask the team to confirm it.";
  }
