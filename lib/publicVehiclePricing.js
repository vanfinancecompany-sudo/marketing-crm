...
const VEHICLE_SPEC_ATTRIBUTE = /\b(?:automatic|auto|manual|gearbox|transmission|fuel(?: type)?|diesel|petrol|electric|hybrid|mileage|engine size|engine capacity|bhp|horsepower|colour|color|wheelbase|lwb|mwb|swb|payload|towing|air ?con|air conditioning|sat ?nav|navigation|parking sensors?|reverse camera|camera|cruise control|doors?|seats?|spec(?:ification)?|features?|equipment)\b/i;
...
function vehicleSpecificationBoundaryReply(productLock) {
  if (productLock === "rent2buy") {
    return "I can help with the Rent2Buy costs, agreement terms and application for this van, but I don’t reliably have the full vehicle specification. Please check the Vehicle Information on this page for details such as transmission, mileage and equipment.";
  }
  return "I can help with the finance, pricing and application for this van, but I don’t reliably have the full vehicle specification. Please check the Vehicle Information on this page for details such as transmission, mileage and equipment.";
}
...
export function publicVehiclePricingReply({
  ...
  if (!pricingQuestion && !termQuestion) {
    if (hasCurrentVehicleContext(pageType, vehicleContext) && isVehicleSpecificationQuestion(text)) {
      return vehicleSpecificationBoundaryReply(productLock);
    }
    return null;
  }
