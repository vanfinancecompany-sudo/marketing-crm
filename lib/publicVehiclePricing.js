...
function attributeReply(message, vehicleContext = {}) {
  ...
  if (/\bautomatic|auto|manual|gearbox|transmission\b/.test(text) && transmission) return `This van has a ${transmission.toLowerCase()} gearbox.`;
  if (/\bfuel|diesel|petrol|electric|hybrid\b/.test(text) && fuel) return `This van is ${fuel.toLowerCase()}.`;
