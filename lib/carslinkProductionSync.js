...
const VFC_PUBLIC_ORIGIN = "https://vanfinance.co";
const DEFAULT_DEALER_PHONE = "03301336376";
...
function normalizeListingUrl(value, registration) {
  ...
  if (reg) return `${VFC_PUBLIC_ORIGIN}/vans/${encodeURIComponent(reg)}`;

  const raw = String(value || "").trim();
  if (!raw) return `${VFC_PUBLIC_ORIGIN}/vans`;
  try {
    const parsed = new URL(raw, VFC_PUBLIC_ORIGIN);
    return `${VFC_PUBLIC_ORIGIN}${parsed.pathname}${parsed.search}`;
  } catch {
    return raw.startsWith("/") ? `${VFC_PUBLIC_ORIGIN}${raw}` : `${VFC_PUBLIC_ORIGIN}/vans`;
  }
}

function normalizeDealerPhone(value) {
  const raw = String(value || DEFAULT_DEALER_PHONE).trim();
  if (!raw) return DEFAULT_DEALER_PHONE;
  const hasLeadingPlus = raw.startsWith("+");
  const digits = raw.replace(/\D/g, "");
  return `${hasLeadingPlus ? "+" : ""}${digits}` || DEFAULT_DEALER_PHONE;
}
...
function descriptionWithPriceAndVat(listing) {
  const description = String(listing?.description || "").trim();
  if (/^£\s*[0-9]/.test(description)) return description;
  const price = formatListingPrice(listing?.price);
  if (!price) return description;
  const noVat = /\bno\s+vat\b/i.test(description);
  const vatText = noVat ? "NO VAT" : "+ VAT";
  return `${price} ${vatText}. ${description}`.trim();
}

function colourFromListing(listing) {
  const explicit = String(listing?.colour || listing?.color || "").trim();
  if (!explicit) return "";
  for (const [colour, pattern] of COLOUR_RULES) {
    if (pattern.test(explicit)) return colour;
  }
  return "";
}
...
function enrichCarslinkPayload(payload) {
  ...
  const dealer = {
    ...(payload.dealer || {}),
    phone: normalizeDealerPhone(payload?.dealer?.phone),
  ...
  const listings = Array.isArray(payload.listings)
    ? payload.listings.map((listing) => {
        ...
        return {
          ...listing,
          description: descriptionWithPriceAndVat(listing),
