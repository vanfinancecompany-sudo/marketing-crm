...
function isVfcArticleUrl(value) {
  try {
    const url = new URL(clean(value, 1000));
    return ["vanfinancecompany.co.uk", "www.vanfinancecompany.co.uk"].includes(url.hostname.toLowerCase())
...
export function isPublicKnowledgeHubArticle(article = {}) {
  return isConfirmedPublishedArticle(article) && isVfcArticleUrl(article.live_wix_url);
}
...
export function searchPublicKnowledgeHubArticles(articles = [], { query = "", category = "all", limit = 8 } = {}) {
  ...
  return (Array.isArray(articles) ? articles : [])
    .filter(isPublicKnowledgeHubArticle)
