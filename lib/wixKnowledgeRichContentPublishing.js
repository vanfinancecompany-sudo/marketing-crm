...
export function buildKnowledgeWixRichContent(markdown, suggestions = [], cta = "") {
  const safeMarkdown = sanitizeKnowledgeMarkdownLinks(markdown);
  return enrichLinkedFormatting(buildWixRichContent(safeMarkdown, suggestions, cta));
}
...
function buildRichArticleData(article, suggestions, richContentFieldId) {
  ...
  return { ...metadata, [richContentFieldId]: buildKnowledgeWixRichContent(article.content_markdown, suggestions, article.cta) };
}
...
export async function createOrUpdateKnowledgeRichContentDraft({ article, suggestions = [], configuration, environment = process.env, fetchImpl = fetch }) {
  ...
  const linkedMarkdown = applyAcceptedInternalLinks(sanitizeKnowledgeMarkdownLinks(article.content_markdown), suggestions);
  ...
  return { operation, itemId: savedItemId, collectionId: configuration.collectionId, dashboardUrl: configuration.dashboardUrl, syncStatus: "synced", payloadVersion: `${WIX_KNOWLEDGE_PAYLOAD_VERSION}-rich-links`, contentFieldType: "RICH_CONTENT", contentFieldId: richContentFieldId, recoveredMissingItem: Boolean(missingStoredItemId), replacedItemId: missingStoredItemId || null, tableConversionWarnings, data };
}
