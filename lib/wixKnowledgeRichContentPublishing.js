...
async function seedPublishedItemIntoDrafts(publishedClient, draftClient, article, publishedItemId) {
  try {
    await createWixDraftFromPublished(publishedClient, publishedItemId);
    return publishedItemId;
  } catch (error) {
    const recoveredDraftId = await resolveExistingWixItem(draftClient, article).catch(() => "");
    if (recoveredDraftId) return recoveredDraftId;
    throw error;
  }
}
...
export async function createOrUpdateKnowledgeRichContentDraft({ article, suggestions = [], configuration, environment = process.env, fetchImpl = fetch }) {
  ...
  let itemId = await resolveExistingWixItem(draftClient, article);
  let operation = itemId ? "updated" : "created";
  let serverState = "DRAFT";
  let item;

  if (!itemId) {
    let publishedItemId = await resolveExistingWixItem(publishedClient, article);
    if (!publishedItemId && clean(article.wix_item_id, 500)) {
      try {
        publishedItemId = await seedPublishedItemIntoDrafts(publishedClient, draftClient, article, clean(article.wix_item_id, 500));
    ...
    if (!itemId && publishedItemId) {
      itemId = await seedPublishedItemIntoDrafts(publishedClient, draftClient, article, publishedItemId);
      operation = "updated";
      serverState = "CHANGED";
    }
  }
  ...
  if (itemId) {
    item = await draftClient.update(itemId, data);
  } else {
    try {
      item = await draftClient.create(data, article.id);
    } catch (error) {
      if (error.details?.wix_status !== 409) throw error;
      const recoveredDraftId = await resolveExistingWixItem(draftClient, article);
      if (!recoveredDraftId) throw error;
      operation = "updated";
      itemId = recoveredDraftId;
      item = await draftClient.update(recoveredDraftId, data);
    }
  }
