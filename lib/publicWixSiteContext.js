...
async function queryPublicVehiclePage(route, fetchImpl) {
  const response = await fetchImpl(route.page_url, {
    ...
    headers: {
      ...
      "User-Agent": "VanFinanceCompany-AI-Assistant/1.0",
    },
  });
...
export async function resolvePublicWixPageContext(pageUrl, { environment = process.env, fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    if (pageContext && hasResolvedVehicleData(pageContext)) {
      vehicleCache.set(key, { context: pageContext, expiresAt: Date.now() + VEHICLE_CACHE_TTL_MS });
      return pageContext;
    }
  } catch (error) {
  ...
  // Do not cache the identity-only fallback. A later request should be able to recover from a transient Wix/API failure.
