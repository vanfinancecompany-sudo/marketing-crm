...
export const DEALERKIT_WIX_MEDIA_PLAN_VERSION = 1;
...
function orderedSourceImages(vehicle = {}, decision = {}) {
  ...
  const excluded = new Set(
    (Array.isArray(decision.excludedImageIds) ? decision.excludedImageIds : [])
      .map((id) => clean(id, 300))
      .filter(Boolean),
  );
  ...
  const orderedIds = [];
  for (const rawId of [
    ...(Array.isArray(decision.imageOrderIds) ? decision.imageOrderIds : []),
    ...imageById.keys(),
  ]) {
...
export function buildDealerKitWixMediaPlan({ vehicle = {}, decision = {} } = {}) {
  ...
  const primaryImageId = clean(decision.primaryImageId, 300) || null;
  ...
  const items = ordered.orderedIds.map((id, index) => {
    ...
    return {
      id,
      stableKey: supplierStockId ? `${supplierStockId}:${id}` : id,
      sourceUrl,
      position: index + 1,
      ...
      wixImportRequestPreview: sourceUrl ? {
        url: sourceUrl,
        mediaType: "IMAGE",
  ...
  return {
    ...
    sourceRequirements: {
      ...
      note: "Before live import is unlocked, verify the DealerKit API image URL lifetime/authentication contract and either MIME metadata, filename extension or HEAD support for every source image.",
    },
    storagePolicy: {
