...
const KNOWLEDGE_CATEGORY_ALIASES = new Map([
  ["finance", "Van Finance"],
  ["van finance", "Van Finance"],
  ["rent 2 buy", "Rent2Buy"],
  ["rent to buy", "Rent2Buy"],
  ["rent2buy", "Rent2Buy"],
  ["credit", "Credit"],
  ["self employed", "Self Employed"],
  ["limited companies", "Limited Company"],
  ["limited company", "Limited Company"],
  ["buying guide", "Vehicle Guides"],
  ["buying guides", "Vehicle Guides"],
  ["vehicle guide", "Vehicle Guides"],
  ["vehicle guides", "Vehicle Guides"],
  ["vehicle review", "Vehicle Guides"],
  ["vehicle reviews", "Vehicle Guides"],
...
export const KNOWLEDGE_CARD_TITLE_MAX = 60;
export const KNOWLEDGE_CARD_EXCERPT_MAX = 149;
...
export function normalizeKnowledgeCategory(value) {
  ...
  return KNOWLEDGE_CATEGORY_ALIASES.get(normalized) || "";
}
...
export function parseKnowledgeTopicIdeasResponse(value) {
  ...
  return parsed.ideas.map((idea) => {
    ...
    return {
      ...
      estimated_value: Math.min(5, Math.max(1, Number(idea.estimated_value) || 3)),
      difficulty: Math.min(5, Math.max(1, Number(idea.difficulty) || 3)),
      target_persona: String(idea.target_persona || "").trim(),
      seasonal: Boolean(idea.seasonal),
      opportunity_reason: String(idea.opportunity_reason || idea.rationale || "").trim(),
      status: "idea",
...
function hasEmptyMarkdownSection(content) {
  const lines = String(content || "").split(/\r?\n/);

  for (let index = 0; index < lines.length; index += 1) {
    const currentLevel = markdownHeadingLevel(lines[index]);
    ...
    const nextLevel = markdownHeadingLevel(lines[next]);
    if (nextLevel && nextLevel <= currentLevel) return true;
  }
...
export function calculateKnowledgeQualityChecks(article, targetLength = 1000) {
  ...
  const claimText = `${content} ${article?.cta || ""}`
    .replace(/^#{1,3}\s+.*\?\s*$/gm, "")
    .replace(/\b(?:not|never)\s+guaranteed\b/gi, "")
    .replace(/\bno\s+guarantee\b/gi, "");

  return [
    ["clear_intent", "Answers a clear question or intent", Boolean(article?.title && article?.excerpt)],
    [
      "card_title_length",
      `Article title is ${KNOWLEDGE_CARD_TITLE_MAX} characters or fewer`,
      String(article?.title || "").length <= KNOWLEDGE_CARD_TITLE_MAX,
    ],
    [
      "card_excerpt_length",
      `Excerpt is ${KNOWLEDGE_CARD_EXCERPT_MAX} characters or fewer`,
      String(article?.excerpt || "").length <= KNOWLEDGE_CARD_EXCERPT_MAX,
    ],
    [
      "adequate_length",
    ...
    ["empty_sections", "No empty sections", !hasEmptyMarkdownSection(content)],
    ...
    [
      ...
      !unsupportedClaim.test(claimText),
    ],
...
export function validateKnowledgeArticle(article) {
  ...
  if (title.length > KNOWLEDGE_CARD_TITLE_MAX) {
    errors.title = `Article title must be ${KNOWLEDGE_CARD_TITLE_MAX} characters or fewer so the Knowledge Hub card layout is not stretched.`;
  }
  if (excerpt.length > KNOWLEDGE_CARD_EXCERPT_MAX) {
    errors.excerpt = `Excerpt must be ${KNOWLEDGE_CARD_EXCERPT_MAX} characters or fewer so the Knowledge Hub card layout is not stretched.`;
  }
