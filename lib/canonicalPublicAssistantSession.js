import { mergeRememberedFacts } from "./conversationIntelligence.js";
...
export function londonRuntimeContext(now = new Date()) {
  ...
  return {
    current_date: `${value("year")}-${value("month")}-${value("day")}`,
    current_day: value("weekday"),
    timezone: "Europe/London",
  };
}

export function buildCanonicalConversationInput({ session, message, requestId, history, pageContext = {}, controlledFallback = null, runtimeNow = new Date() }) {
  ...
  const rememberedFactMetadata = { ...object(session?.journey_state?.remembered_fact_metadata) };
  for (const field of Object.keys(rememberedFacts)) {
    if (rememberedFactMetadata[field]) continue;
    rememberedFactMetadata[field] = {
      provenance: field === "product_context" || (field === "vehicle_interest" && Object.keys(object(session?.vehicle_context)).length) ? "system_context" : "legacy_customer_explicit",
      confidence: 0.9,
  ...
  return {
    ...
    runtime_context: londonRuntimeContext(runtimeNow),
    messages: Array.isArray(history) ? history : [],
    remembered_facts: rememberedFacts,
    remembered_fact_metadata: rememberedFactMetadata,
...
export function canonicalSessionState({ session, result, productLock }) {
  ...
  const previousMetadata = object(session?.journey_state?.remembered_fact_metadata);
  const resultMetadata = object(result?.remembered_fact_metadata);
  const correctionFields = Array.isArray(result?.corrections) ? result.corrections.map((item) => clean(item?.field, 100)).filter(Boolean) : [];
  const hasResultMetadata = Object.keys(resultMetadata).length > 0;
  const trustedResultFacts = Object.fromEntries(Object.entries(resultFacts).filter(([field]) => !hasResultMetadata || field === "product_context" || Object.hasOwn(previousFacts, field) || Object.hasOwn(resultMetadata, field)));
  const merged = mergeRememberedFacts({
    previousFacts,
    previousMetadata,
    updates: trustedResultFacts,
    updateMetadata: resultMetadata,
    correctionFields,
  });
  const rememberedFacts = { ...merged.remembered_facts, product_context: productContext };
