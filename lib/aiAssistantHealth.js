...
function sourceMatchesProduct(source, productContext) {
...
export function evaluateHealthConversation({ scenario, turns = [], mode = "deterministic" } = {}) {
  ...
  turns.forEach((turn, turnIndex) => {
    ...
    sources.forEach((source) => {
      productChecks += 1;
      if (sourceMatchesProduct(source, scenario.product_context)) productPasses += 1;
    ...
    productChecks += 1;
    const replyCrossesProduct = !explicitComparison(turn.message) && (scenario.product_context === "finance" ? /\brent\s*(?:2|to)\s*buy\b/i : /\bfinance\b/i).test(result.reply || "");
    ...
    if (/\b(?:guaranteed approval|guarantee (?:you|approval)|definitely (?:approved|accepted)|promise delivery|will be approved)\b/i.test(result.reply || "")) failures.push({ rule: "unsafe_promise", turn: turnIndex + 1, message: turn.message, detail: `${label} made an approval or delivery promise.` });
    ...
    progressionPasses += result.conversation_progressing || result.application_mode_active || result.conversation_resumed ? 1 : 0;
  ...
  return {
    ...
    turns: totalTurns,
    checks: {
      conversation_progression: { passed: progressionPasses, total: totalTurns },
