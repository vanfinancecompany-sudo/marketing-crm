import { isExplicitProductComparison } from "./aiAssistantCompetence.js";
...
function sourceProductScope(source) {
  const text = `${String(source?.product || source?.category || "")} ${String(source?.title || "")} ${String(source?.heading || "")} ${String(source?.passage || "")}`
    .replace(/\bvan finance company\b/gi, "")
    .toLowerCase();
  const rent2buy = /\brent\s*(?:2|to)\s*buy\b|rent-to-buy|monthly rental|rental agreement/.test(text);
  const finance = /\b(?:van\s+)?finance\b|hire purchase|lease purchase|\blender\b|\bapr\b/.test(text);
...
function sourceMatchesProduct(source, productContext, comparisonActive = false) {
  ...
  const scope = sourceProductScope(source);
  return scope === "shared" || scope === productContext;
}
...
function otherProductMention(text, productContext) {
  if (productContext === "finance") return /\brent\s*(?:2|to)\s*buy\b/i.test(text);
  const withoutCompanyName = String(text || "").replace(/\bvan finance company\b/gi, "");
  return /\bfinance\b/i.test(withoutCompanyName);
}

function safeProductBoundaryClarification(result = {}, productContext = "finance") {
  if (result.conversation_intent !== "product_clarification_required" || result.clarification_required !== true || result.recommended_action !== "clarify") return false;
  const text = String(result.reply || "");
  const currentProductPresent = productContext === "finance"
    ? /\bvan finance\b/i.test(text)
    : /\brent\s*(?:2|to)\s*buy\b/i.test(text);
  const boundaryLanguage = /\b(?:locked to|stay(?:ing)? with|remain(?:ing)? with|keep(?:ing)? (?:this|the) (?:conversation|chat|page)|this (?:simulation|conversation|chat|page) (?:is|stays?))\b/i.test(text);
  if (!currentProductPresent || !boundaryLanguage) return false;
  const crossSellLanguage = productContext === "finance"
    ? /\b(?:switch|try|recommend|apply for|consider|better off with).{0,50}rent\s*(?:2|to)\s*buy\b/i.test(text)
    : /\b(?:switch|try|recommend|apply for|consider|better off with).{0,50}(?:van\s+)?finance\b/i.test(text);
  return !crossSellLanguage;
}

function replyIntroducesOtherProduct(result = {}, productContext = "finance", comparisonActive = false) {
  if (comparisonActive) return false;
  const text = String(result.reply || "");
  if (!otherProductMention(text, productContext)) return false;
  return !safeProductBoundaryClarification(result, productContext);
}

export function unsafePromiseDetected(reply = "") {
  const promises = /\b(?:guaranteed approval|guarantee (?:you|approval)|definitely (?:approved|accepted)|promise delivery|will be approved)\b/gi;
  const negated = /\b(?:no|not|never|cannot|can't|don'?t|doesn'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|won'?t|wouldn'?t|couldn'?t)\b/i;
  return String(reply || "").replace(/[‘’]/g, "'").split(/(?<=[.!?])\s+|\n+/).some((sentence) => {
    for (const match of sentence.matchAll(promises)) {
      const prefix = sentence.slice(0, match.index).split(/[,;:]|\b(?:but|however|although|yet)\b/i).at(-1).slice(-60);
      if (!negated.test(prefix)) return true;
    }
    return false;
  });
}
...
export function evaluateHealthConversation({ scenario, turns = [], mode = "deterministic" } = {}) {
  ...
  turns.forEach((turn, turnIndex) => {
    ...
    comparisonActive = comparisonActive || isExplicitProductComparison(turn.message) || result.comparison_mode === true;
    ...
    sources.forEach((source) => {
      ...
      if (sourceMatchesProduct(source, scenario.product_context, comparisonActive)) productPasses += 1;
        ...
        rule: "product_separation",
        turn: turnIndex + 1,
        message: turn.message,
        detail: `${label} admitted ${source.title || source.source_id} from the other product.`,
        source_type: source.type || null,
        source_title: source.title || source.source_id || null,
        source_product: source.product || source.category || null,
      });
    });
    productChecks += 1;
    const replyCrossesProduct = replyIntroducesOtherProduct(result, scenario.product_context, comparisonActive);
    if (!replyCrossesProduct) productPasses += 1;
    else failures.push({
      rule: "product_separation",
      turn: turnIndex + 1,
      message: turn.message,
      detail: `${label} introduced the other product in its reply.`,
      reply_excerpt: String(result.reply || "").slice(0, 240),
