...
function replyIntroducesOtherProduct(reply = "", productContext = "finance", comparisonActive = false) {
  if (comparisonActive) return false;
  const text = String(reply || "");
  if (productContext === "finance") return /\brent\s*(?:2|to)\s*buy\b/i.test(text);
  // The registered business name is not a reference to the Finance product.
  const withoutCompanyName = text.replace(/\bvan finance company\b/gi, "");
...
export function evaluateHealthConversation({ scenario, turns = [], mode = "deterministic" } = {}) {
  ...
  turns.forEach((turn, turnIndex) => {
    ...
    productChecks += 1;
    const replyCrossesProduct = replyIntroducesOtherProduct(result.reply, scenario.product_context, comparisonActive);
