...
export function summarizeDailyActivity({ targets, events = [], emailRecipients = [] }) {
  const normalizedTargets = normalizeTargets(targets);
  const actual = Object.fromEntries(DAILY_ACTIVITY_TYPES.map((type) => [type, 0]));
  events.forEach((event) => {
    if (SOCIAL_ACTIVITY_TYPES.includes(event.activity_type)) actual[event.activity_type] += Math.max(0, Number(event.quantity || 0));
  });
  ...
  const targetTotal = DAILY_ACTIVITY_TYPES.reduce((sum, type) => sum + metrics[type].target, 0);
  const creditedTotal = DAILY_ACTIVITY_TYPES.reduce((sum, type) => sum + Math.min(metrics[type].completed, metrics[type].target), 0);
  const remainingTotal = DAILY_ACTIVITY_TYPES.reduce((sum, type) => sum + metrics[type].remaining, 0);
  return {
    targets: normalizedTargets,
    metrics,
    completion_percentage: targetTotal === 0 ? 100 : Math.round((creditedTotal / targetTotal) * 100),
