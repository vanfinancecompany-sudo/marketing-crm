...
export const EVENT_ACTIVITY_TYPES = Object.freeze(DAILY_ACTIVITY_TYPES.filter((type) => type !== "emails_sent"));
export const SOCIAL_ACTIVITY_TYPES = Object.freeze(EVENT_ACTIVITY_TYPES.filter((type) => type !== "knowledge_hub_article"));

export const DEFAULT_DAILY_TARGETS = Object.freeze({
  van_finance_facebook_post: 10,
  rent2buy_facebook_post: 10,
  van_finance_google_business_post: 6,
  rent2buy_google_business_post: 4,
  van_finance_groups_post: 5,
  rent2buy_groups_post: 5,
  van_finance_marketplace_post: 5,
  rent2buy_marketplace_post: 5,
  van_finance_reel: 10,
  rent2buy_reel: 10,
  emails_sent: 200,
  knowledge_hub_article: 0,
  off_day: false,
});
...
export const ACTIVITY_LABELS = Object.freeze({
  ...
  van_finance_google_business_post: "Van Finance Google Business posts",
  rent2buy_google_business_post: "Rent2Buy Google Business posts",
  ...
  knowledge_hub_article: "Knowledge Hub articles",
});
...
function generatedReelIdentity(reel = {}) {
  return String(reel.id || `${reel.created_at || ""}:${reel.registration || ""}:${reel.pipeline || ""}`);
}

function eventCreativeIdentity(event = {}) {
  return String(event.metadata?.marketing_creative_id || event.metadata?.creative_id || "");
}
...
export function summarizeDailyActivity({ targets, events = [], emailRecipients = [], emailSends = [], generatedReels = [] }) {
  ...
  const generatedReelIds = new Set();

  generatedReels.forEach((reel) => {
    const identity = generatedReelIdentity(reel);
    if (generatedReelIds.has(identity)) return;
    generatedReelIds.add(identity);
  ...
  events.forEach((event) => {
    ...
    if (!EVENT_ACTIVITY_TYPES.includes(type)) return;
    if (["van_finance_reel", "rent2buy_reel"].includes(type)) {
      if (event.metadata?.status_event === "facebook_published") return;
      const creativeIdentity = eventCreativeIdentity(event);
      if (creativeIdentity && generatedReelIds.has(creativeIdentity)) return;
    }
    actual[type] += Math.max(0, Number(event.quantity || 0));
  });
