...
export function reviewableHistoricSuggestions(suggestions = []) {
  return suggestions.filter((item) => ["pending", "accepted"].includes(item?.status));
}

export function sourceSnippetsForSuggestion(articleMarkdown, suggestion = {}, maximum = 1) {
  ...
  return sentences
    ...
    .slice(0, Math.max(0, Math.min(2, Number(maximum) || 1)))
    .map((item) => clean(item.sentence, 220));
}
...
export function compactHistoricSuggestion(articleMarkdown, suggestion = {}) {
  ...
  return {
    id: clean(suggestion.id, 100),
    status: clean(suggestion.status, 20),
    destination_title: clean(suggestion.destination_title, 120),
    destination_url: clean(suggestion.destination_url, 220),
    anchor_text: clean(suggestion.anchor_text, 160),
    anchor_found: validation.found,
    anchor_match_count: validation.match_count || 0,
    source_snippets: sourceSnippetsForSuggestion(articleMarkdown, suggestion, 1),
  };
}
