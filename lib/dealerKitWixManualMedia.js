...
export const VERIFIED_RENT2BUY_WIX_SITE_ID = "548f025b-673c-47f7-9bb6-383ab5d946e4";
...
export function manualMediaSiteConfiguration(purposeValue, environment = process.env) {
  ...
  const configuredRent2BuySiteId = clean(environment.WIX_RENT2BUY_SITE_ID, 500);
  const siteId = purpose.siteScope === "rent2buy"
    ? configuredRent2BuySiteId || VERIFIED_RENT2BUY_WIX_SITE_ID
  ...
  return {
    ...
    siteIdSource: purpose.siteScope === "rent2buy"
      ? (configuredRent2BuySiteId ? "environment" : "verified_site_binding")
      : "environment",
...
export function manualMediaRowToClient(row = {}) {
  ...
  return {
    ...
    operationStatus: clean(row.operation_status, 80).toUpperCase() || "PENDING",
    ready: clean(row.operation_status, 80).toUpperCase() === "READY",
