...
export function googleCustomerStatus(result = {}) {
  if (result.result_status === "inconclusive") return "pending";
  return result.result_status || "not_checked";
}
...
export function buildArticleVisibility({
  ...
  const googleStatus = latestGoogleAttempt?.result_status === "error"
    ? "error"
    : googleCustomerStatus(google || {});
  ...
  else if (googleStatus === "not_indexed")
    recommendedAction =
      "Review Google indexing evidence and the published page.";
  else if (googleStatus === "pending")
    recommendedAction =
      "Google completed the check but has not returned an indexing verdict yet. Check again later.";
  else if (visible)
  ...
  return {
    ...
    google_indexing_status: googleStatus,
    platforms_checked: [
      ...new Set(
        latest
...
export function buildVisibilitySummary({
  ...
  return {
    ...
    google_pending: articleResults.filter(
      (item) => item.google_indexing_status === "pending",
    ).length,
...
export function filterVisibilityArticles(
  ...
  const filtered = items.filter((item) => {
    ...
    if (status === "indexed" && item.google_indexing_status !== "indexed")
      return false;
    if (status === "pending" && item.google_indexing_status !== "pending")
      return false;
