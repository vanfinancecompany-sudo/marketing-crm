...
export function buildArticleVisibility({
  ...
  const latestCompleted = latestVisibilityResults(
  ...
  const aiCompleted = latestCompleted.filter((result) => AI_PROVIDER_KEYS.has(result.provider));
  const detections = effectiveResults.filter((result) => DETECTION_STATUSES.has(result.result_status));
  const currentDetections = latestCompleted.filter((result) => DETECTION_STATUSES.has(result.result_status));
  const google = latestCompleted
  ...
  const aiEligible = aiCompleted.length > 0;
  ...
  const needsAttention = Boolean(
    oldEnough &&
    checkedSuccessfully &&
    (google?.result_status === "not_indexed" || (aiEligible && !visible))
  );
  let recommendedAction = "Run the first verified visibility check.";
  if (needsAttention && google?.result_status === "not_indexed") {
    recommendedAction = "Review Google indexing evidence and the published page.";
  } else if (needsAttention) {
    recommendedAction = "Review monitored prompts and strengthen unsupported content gaps.";
  } else if (visible) {
    recommendedAction = "Continue monitoring; retain the evidence history.";
  } else if (checkedSuccessfully) {
    recommendedAction = "No detection is verified yet; review again after the monitoring period.";
  }
  return {
    article,
    ...
    awaiting_first_check: completed.length === 0,
...
export function buildVisibilitySummary({
  ...
  const detectionTotal = Object.values(providerDetections).reduce((total, count) => total + count, 0);
  ...
  return {
    ...
    total_verified_detections: detectionTotal,
...
export function deriveVisibilityPrompts({
  ...
  const vocabulary = businessSections
    ...
    .filter(Boolean)
    .slice(0, 2);
  vocabulary.forEach((term) => candidates.push({
...
export function filterVisibilityArticles(items = [], {
  ...
  const filtered = items.filter((item) => {
    ...
    if (
      ...
      !(provider === "all"
        ? item.visible
        : providerResults.some((result) => DETECTION_STATUSES.has(result.result_status)))
    ) return false;
    if (status === "indexed" && item.google_indexing_status !== "indexed") return false;
    if (status === "checked" && !item.ai_eligible) return false;
