...
const LOCATION_REFERENCE_ONLY = /^(?:me|here|there|home|my (?:area|home|location|place|town|city)|our (?:area|location|place)|where i am)$/i;
...
export function extractUkLocation(question) {
  ...
  const patterns = [
    ...
    /\b(?:available|cover(?:ed)?)\s+(?:in|near|to)\s+([^?.!,;]+)/i,
    /\bdeliver(?:\s+(?:it|that|this(?:\s+(?:van|vehicle))?|the\s+(?:van|vehicle)))?\s+to\s+([^?.!,;]+)/i,
    /\b(?:from|near|in|about)\s+([^?.!,;]+)/i,
  ];
  for (const pattern of patterns) {
    const match = text.match(pattern);
    const candidate = clean(match?.[1], 120).replace(/\s+(?:and|but|because|with|for|please|before|by|on)\b.*$/i, "").trim();
    if (candidate && !/^(the|your|our|this|that)\b/i.test(candidate) && !LOCATION_REFERENCE_ONLY.test(candidate)) {
      return { query: candidate, type: "town_or_city" };
    }
  }
  return null;
}
