...
async function loadPagedRecipients(makeQuery, assertResult, target) {
  let from = 0;
  while (true) {
    // Offset pagination must use an immutable order or changing recipient rows can be skipped or repeated.
    const result = assertResult(
      await makeQuery()
        .order("id", { ascending: true })
        .range(from, from + 999),
      "Could not inspect previous campaign recipients."
    );
  ...
  while (true) {
    const result = assertResult(
      await supabase.from("marketing_email_send_recipients")
        ...
        // Keep pages stable while delivery/webhook updates mutate other recipient columns.
        .order("id", { ascending: true })
        .range(from, from + 999),
