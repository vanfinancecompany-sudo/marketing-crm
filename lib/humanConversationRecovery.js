...
function contextualTurnAnchor(priorAssistant, currentText) {
  ...
  const shortAnswerToQuestion = raw.includes("?") && wordCount(currentText) <= 5 && hasLettersOrNumbers(currentText);
  return {
    offeredExplanation,
    shortAnswerToQuestion,
    anchor: offeredExplanation || shortAnswerToQuestion ? raw : "",
    requiresKnowledge: (offeredExplanation || shortAnswerToQuestion) && FACTUAL_CONTEXT.test(prior),
  };
}
...
export function classifyUniversalMessage({ message, messages = [], journey = {} } = {}) {
  ...
  else if (context.shortAnswerToQuestion && !BUSINESS.test(text) && !QUESTION_WORD.test(text)) {
    messageType = "follow_up_question";
    confidence = 92;
    reason = "The short response directly follows an assistant question, so the previous assistant turn supplies the missing context.";
    recoveryRequired = false;
    contextualAnchor = context.anchor;
    contextualRequiresKnowledge = context.requiresKnowledge;
  }
