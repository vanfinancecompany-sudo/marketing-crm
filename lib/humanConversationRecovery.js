...
export const UNIVERSAL_MESSAGE_TYPES = Object.freeze([
  "greeting", "goodbye", "question", "follow_up_question", "clarification", "agreement", "disagreement",
...
const CONFUSION = /^(?:what|eh|sorry|explain|huh|pardon|what do you mean|what does (?:this|that) (?:question|field) mean|what is (?:this|that) (?:question|field) asking|i don(?:\s*)?t get it|dont get it|i do not understand|confused|not following|\?+)$/i;
const AGREEMENT = /^(?:yes(?: please)?|yep(?: please)?|yeah(?: please)?|sure(?: please)?|of course|correct|exactly|right|ok(?:ay)?(?: please)?|okay then|alright|all right|fine|sounds good|that\s*s right|thats right|that\s*s fine|thats fine|please do)$/i;
...
const THANKS = /^(?:thanks|thank you|cheers|nice one|brilliant|great|perfect|helpful|that helps|that s useful|that is useful|useful|got it|understood|makes sense|that makes sense|that s clear|that is clear|all good)[!. ]*$/i;
const FAREWELL = /^(?:bye|goodbye|good bye|see you|see you soon|speak soon|talk soon|catch you later|later)[!. ]*$/i;
...
const RETURNING = /^(?:back now|i\s*m back|im back|carry on|carry on please|yes carry on|yeah carry on|continue|continue please|can i continue|ok continue|okay continue|resume|back to it|go on|go on then|ok go on|okay go on|keep going)$/i;
...
const CLARIFICATION_CHOICE = /\b(?:or|do you mean|are you asking|is it asking|which (?:one|option|part)|whether)\b/i;
const EXPLAIN_PREVIOUS = /^(?:(?:can|could|would) you )?(?:please )?(?:explain(?: that| it)?|tell me more|explain more|go into more detail|what do you mean|why is that|how so)(?: please)?[?.! ]*$/i;
...
function contextualTurnAnchor(priorAssistant, currentText) {
  ...
  const clarificationChoice = raw.includes("?") && wordCount(prior) >= 12 && CLARIFICATION_CHOICE.test(prior);
  const shortClarificationAnswer = clarificationChoice && wordCount(currentText) <= 5 && hasLettersOrNumbers(currentText);
  const directQuestion = raw.includes("?") && wordCount(prior) >= 3;
  const shortAnswerToQuestion = directQuestion
    && wordCount(currentText) > 0
    && wordCount(currentText) <= 8
    && hasLettersOrNumbers(currentText)
    && !currentText.includes("?");
  const explainPrevious = Boolean(raw) && EXPLAIN_PREVIOUS.test(currentText);
  const anchored = offeredExplanation || shortClarificationAnswer || explainPrevious;
  return {
    offeredExplanation,
    shortClarificationAnswer,
    shortAnswerToQuestion,
    explainPrevious,
    anchor: anchored ? raw : "",
    requiresKnowledge: explainPrevious || ((offeredExplanation || shortClarificationAnswer) && FACTUAL_CONTEXT.test(prior)),
  };
}
...
export function classifyUniversalMessage({ message, messages = [], journey = {} } = {}) {
  ...
  const context = contextualTurnAnchor(priorAssistant, text);
  ...
  else if (FAREWELL.test(text)) { messageType = "goodbye"; confidence = 99; reason = "Natural conversation closing."; recoveryRequired = false; }
  else if (context.explainPrevious) {
    messageType = "follow_up_question";
    confidence = 98;
    reason = "The customer is asking for the assistant's immediately preceding answer to be explained, so that answer supplies the subject.";
    recoveryRequired = false;
    contextualAnchor = context.anchor;
    contextualRequiresKnowledge = true;
  }
  ...
  else if (RETURNING.test(text)) { messageType = "agreement"; confidence = 96; reason = "The customer wants to continue the existing conversation or application."; recoveryRequired = false; }
  ...
  else if (context.shortClarificationAnswer && !BUSINESS.test(text) && !QUESTION_WORD.test(text)) {
    messageType = "follow_up_question";
    confidence = 92;
    reason = "The short response answers the assistant's immediately preceding clarification choice, so that question supplies the missing context.";
    recoveryRequired = false;
    contextualAnchor = context.anchor;
    contextualRequiresKnowledge = context.requiresKnowledge;
  }
  else if (context.shortAnswerToQuestion && !QUESTION_WORD.test(text)) {
    messageType = "clarification";
    confidence = 96;
    reason = "The customer is directly answering the assistant's immediately preceding question, so the answer should be consumed before asking anything else.";
    recoveryRequired = false;
  }
  ...
  return {
    ...
    short_answer_to_question: context.shortAnswerToQuestion,
    emotion,
    objection,
  };
}
...
export function contextualRecoveryQuestion(message, messages = [], facts = {}, productContext = "finance") {
  ...
  if (/what does (?:this|that) (?:question|field) mean|what is (?:this|that) (?:question|field) asking/.test(text)) return "Tell me the exact wording of the question or field you’re looking at, and I’ll explain what it means without guessing.";
...
export function humanRecoveryReply(classification, { messages = [], facts = {}, productContext = "finance", journey = {} } = {}) {
  ...
  const directAnswerReply = classification.short_answer_to_question
    ? journey.next_best_question
      ? `Thanks. ${journey.next_best_question}`
  ...
  const replies = {
    ...
    clarification: directAnswerReply || journey.next_best_question || `That’s okay. What are you unsure about with ${subject}?`,
  ...
  return {
    ...
    recommended_action: type === "frustration" ? "human_handoff" : classification.short_answer_to_question ? "continue" : "clarify",
    confidence: 100,
