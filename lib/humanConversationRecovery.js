...
export const UNIVERSAL_MESSAGE_TYPES = Object.freeze([
  ...
  "contextual_acceptance", "contextual_answer",
...
const AGREEMENT = /^(?:yes(?: please)?|yep(?: please)?|yeah(?: please)?|sure(?: please)?|of course|correct|exactly|right|ok(?:ay)?(?: please)?|fine|sounds good|that\s*s right|thats right|please do|go on|go ahead)$/i;
...
const READY = /ready to apply|let\s*s apply|apply now|can i apply|start (?:the |an )?application|what\s*s next|let\s*s do it|send me the application|i want this van|proceed|begin/i;
...
const DIRECT_APPLICATION_ACTION = /\b(?:start|submit|begin|apply now|application below|continue with your .* application)\b/i;
...
function continuationOfferContext(priorAssistant) {
  const text = normaliseCustomerMessage(priorAssistant);
  const offered = CONTINUATION_OFFER.test(text) && EXPLANATION_OFFER.test(text);
  return {
    offered,
    requiresKnowledge: offered && FACTUAL_CONTEXT.test(text) && !DIRECT_APPLICATION_ACTION.test(text),
  };
}

function contextualAnswerContext(priorAssistant, currentText) {
  const rawPrior = clean(priorAssistant, 1500);
  const prior = normaliseCustomerMessage(rawPrior);
  const shortAnswer = Boolean(rawPrior.includes("?") && wordCount(currentText) <= 5 && hasLettersOrNumbers(currentText));
  const explanationBeforeQuestion = rawPrior.split(/[.!?]\s+/).filter(Boolean).length >= 2;
  return {
    shortAnswer,
    requiresKnowledge: shortAnswer && (explanationBeforeQuestion || FACTUAL_CONTEXT.test(prior)),
  };
}
...
export function classifyUniversalMessage({ message, messages = [], journey = {} } = {}) {
  ...
  const priorOffer = continuationOfferContext(priorAssistant);
  const priorQuestion = contextualAnswerContext(priorAssistant, text);
  ...
  else if (AGREEMENT.test(text) && priorOffer.offered) {
    messageType = "contextual_acceptance";
    confidence = 98;
    reason = "The customer is accepting the assistant’s immediately preceding offer, so that offer supplies the meaning of the turn.";
    recoveryRequired = false;
    contextualAnchor = clean(priorAssistant, 1000);
    contextualRequiresKnowledge = priorOffer.requiresKnowledge;
  }
  else if (AGREEMENT.test(text)) { messageType = "agreement"; confidence = 95; reason = "Short agreement whose meaning depends on recent context."; recoveryRequired = false; }
  ...
  else if (priorQuestion.shortAnswer && !BUSINESS.test(text) && !QUESTION_WORD.test(text)) {
    messageType = "contextual_answer";
    confidence = 92;
    reason = "The short answer directly follows an assistant question, so the previous assistant turn supplies the missing context.";
    recoveryRequired = false;
    contextualAnchor = clean(priorAssistant, 1000);
    contextualRequiresKnowledge = priorQuestion.requiresKnowledge;
  }
