...
const AGREEMENT = /^(?:yes|yep|yeah|correct|exactly|right|ok|okay|fine|sounds good|that\s*s right|thats right)$/i;
...
export function classifyUniversalMessage({ message, messages = [], journey = {} } = {}) {
  ...
  else if (original.includes("?") || QUESTION_WORD.test(text)) { messageType = messages.length ? "follow_up_question" : "question"; confidence = 72; reason = "A question is present, but it is not clearly within approved business knowledge."; }
