...
const DURATION_ANSWER = /^(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+(?:days?|weeks?|months?|years?)$/i;
...
function contextualTurnAnchor(priorAssistant, currentText) {
  ...
  const preserveDurationRecovery = /how long.{0,30}(?:trad|business)/i.test(prior) && DURATION_ANSWER.test(currentText);
  const directAnswerToQuestion = Boolean(raw)
    && raw.includes("?")
    && !preserveDurationRecovery
    && wordCount(currentText) <= 8
    && hasLettersOrNumbers(currentText)
    && !QUESTION_WORD.test(currentText)
    && !currentText.includes("?");
  const anchored = offeredExplanation || shortClarificationAnswer || explainPrevious || directAnswerToQuestion;
  return {
    offeredExplanation,
    shortClarificationAnswer,
    explainPrevious,
    directAnswerToQuestion,
    anchor: anchored ? raw : "",
...
export function classifyUniversalMessage({ message, messages = [], journey = {} } = {}) {
  ...
  else if (context.directAnswerToQuestion) {
    messageType = "clarification";
    confidence = 94;
    reason = "The customer directly answered the assistant's immediately preceding question, so that question supplies the meaning of the response.";
    recoveryRequired = false;
    contextualAnchor = context.anchor;
    contextualRequiresKnowledge = false;
  }
