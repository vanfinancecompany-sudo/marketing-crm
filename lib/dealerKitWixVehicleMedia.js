...
export function buildProductImageSets({
  ...
  // Keep the remaining reviewed DealerKit gallery in order, but remove the
  // DealerKit primary that the manual image has explicitly replaced.
  ...
  const vfcGalleryUrls = uniqueUrls([vfcMainUrl, ...financeGalleryUrls]);
  const rent2buyGalleryUrls = uniqueUrls([rent2buyMainUrl, ...rent2buyGalleryDealerKitUrls]);
