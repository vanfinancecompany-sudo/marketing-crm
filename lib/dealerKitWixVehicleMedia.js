...
export function buildProductImageSets({
  ...
  return {
    ...
    vanFinance: {
      ...
      // The actual Van Finance primary is the only hard image gate. READY
      // secondary images are used now; processing/unprepared secondaries can
      // be added by the later media refresh workflow.
      ready: Boolean(vfcMainUrl && vfcGalleryUrls.length),
    },
    rent2buy: {
      ...
      // Rent2Buy still requires its selected template/primary to be READY.
      // DealerKit secondary photos are opportunistic and never block publish.
      ready: !decision.rent2buyEnabled || Boolean(rent2buyMainUrl && rent2buyGalleryUrls.length),
    },
  };
}
