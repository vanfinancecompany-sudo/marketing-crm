...
export function buildProductImageSets({
  ...
  const vfcMainUrl = vfcManual?.selectedAndReady ? clean(vfcManual.url, 3000) : financePrimaryDealerKitUrl;
  const rent2buyMainUrl = rent2buyTemplate?.selectedAndReady ? clean(rent2buyTemplate.url, 3000) : null;

  const vfcGalleryUrls = uniqueUrls([vfcMainUrl, ...financeUrls]);
  const rent2buyGalleryUrls = uniqueUrls([rent2buyMainUrl, ...rent2buyUrls]);
  ...
  return {
    ...
    vanFinance: {
      mainUrl: vfcMainUrl,
      mainSource: vfcManual?.selectedAndReady ? "manual_replacement" : financePrimaryDealerKitUrl ? "dealerkit_primary" : null,
      ...
      ready: Boolean(vfcMainUrl && financeUrls.length && !financeMissingIds.length),
    },
    rent2buy: {
      ...
      ready: !decision.rent2buyEnabled || Boolean(rent2buyMainUrl && rent2buyUrls.length && !rent2buyMissingIds.length),
    },
  };
}
