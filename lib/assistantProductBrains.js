...
export function enforceRent2BuyReplyBoundary(value) {
  const text = clean(value);
  if (!text || !RENT2BUY_STATUS_CONTAMINATION.test(text)) return text;
  const sentences = text.match(/[^.!?]+[.!?]+|[^.!?]+$/g) || [text];
  const kept = sentences.filter((sentence) => !RENT2BUY_STATUS_CONTAMINATION.test(sentence)).map((sentence) => sentence.trim()).filter(Boolean);
