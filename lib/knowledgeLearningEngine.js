...
export function assessKnowledgeGapCandidate(result = {}, review = {}, settings = {}) {
  ...
  const humanWeak = ["needs_adjustment", "incorrect", "too_vague", "robotic", "lost_context", "asked_unnecessary_clarification", "failed_to_clarify", "wrong_product", "hallucinated_fact", "too_long", "too_formal", "too_salesy", "missed_buying_signal", "weak_next_question", "repeated_information", "failed_to_use_known_fact", "missed_application_opportunity", "should_have_shown_application", "asked_unnecessary_question", "failed_to_recognise_buying_intent", "repeated_itself", "weak_sales_progression"].includes(review.outcome) || Number(review.accuracy || 5) <= 3 || Number(review.helpfulness || 5) <= 3;
  const reasons = [];
  ...
  if (["too_long", "too_formal", "too_salesy", "repeated_information"].includes(review.outcome)) reasons.push("sales_conversation_quality_issue");
  if (["missed_buying_signal", "weak_next_question"].includes(review.outcome)) reasons.push("sales_next_step_issue");
  if (review.outcome === "failed_to_use_known_fact") reasons.push("context_memory_failure");
  if (["missed_application_opportunity", "should_have_shown_application", "failed_to_recognise_buying_intent", "weak_sales_progression"].includes(review.outcome)) reasons.push("application_journey_issue");
  if (review.outcome === "asked_unnecessary_question") reasons.push("clarification_behavior_issue");
  if (review.outcome === "repeated_itself") reasons.push("sales_conversation_quality_issue");
