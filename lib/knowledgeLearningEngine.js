...
export function assessKnowledgeGapCandidate(result = {}, review = {}, settings = {}) {
  ...
  const humanWeak = ["needs_adjustment", "incorrect", "too_vague"].includes(review.outcome) || Number(review.accuracy || 5) <= 3 || Number(review.helpfulness || 5) <= 3;
  const reasons = [];
...
export function diagnoseExistingKnowledge(opportunity, knowledge = {}) {
  ...
  if (opportunity.conflict_count > 0) { diagnosis = "Sources conflict"; recommendedAction = "improve_business_brain"; }
