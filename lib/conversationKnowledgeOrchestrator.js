...
import { detectRent2BuyLocationTurn } from "./productCoverageRules.js";
...
function applyRent2BuyLocationRule(intent = {}, human = {}, message = "") {
  if (intent.product_context !== "rent2buy") return null;
  const location = detectRent2BuyLocationTurn(message, human.previous_assistant_message || "");
...
export function orchestrateConversationTurn({
  ...
  const rent2BuyLocation = applyRent2BuyLocationRule(intent, human, message);
  ...
  const locationKnowledge = Boolean(rent2BuyLocation);
  ...
  const detectedIntents = unique([
    ...
    locationKnowledge ? "rent2buy_location" : "",
  ...
  return {
    ...
    rent2buy_location_turn: rent2BuyLocation,
