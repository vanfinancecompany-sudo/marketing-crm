...
export function orchestrateConversationTurn({
  ...
  const answeredAssistantQuestion = Boolean(human.short_answer_to_question && !contextualTurn);
  ...
  const detectedIntents = unique([
    ...
    recoveryCandidate && !businessKnowledgeIntent && !contextualTurn && !conversationControl ? "conversation_recovery" : "",
  ]);
  const retrievalRequired = businessKnowledgeIntent;
  const recoveryRequired = recoveryCandidate && !retrievalRequired && !productBoundaryBlocked && !applicationContinuation && !contextualTurn && !conversationControl;
