...
const SALES_QUALIFICATION_QUESTION = /\b(?:what (?:size|type) of van|what (?:type|size).{0,20}van|van (?:are you|do you).{0,20}(?:looking for|need)|monthly budget|employed or self-employed|how long have you been trading|town or postcode)\b/i;
...
function applyContextualAnchor(intent = {}, human = {}, message = "") {
  ...
  const normalisedAnchor = normaliseCustomerMessage(anchor);
  const salesQualification = SALES_QUALIFICATION_QUESTION.test(normalisedAnchor);
  const requiresKnowledge = Boolean(
    !salesQualification && (
      human.contextual_requires_knowledge
        || (inferredAnchor && CONTEXTUAL_KNOWLEDGE.test(normalisedAnchor))
    ),
  );
...
export function orchestrateConversationTurn({
  ...
  if (answeredAssistantQuestion && human.message_type === "follow_up_question") human.message_type = "clarification";
