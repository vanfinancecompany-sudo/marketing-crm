...
export function orchestrateConversationTurn({
  ...
  const knowledgeMessage = KNOWLEDGE_MESSAGE_TYPES.has(human.message_type) && !(human.message_type === "objection" && human.objection?.objection === "uncertainty");
  const businessKnowledgeIntent = !productBoundaryBlocked && (
    explicitKnowledgeTopic || (Boolean(intent.retrieval_required) && knowledgeMessage && !recoveryCandidate)
  );
