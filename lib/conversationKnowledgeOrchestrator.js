...
function inferredShortChoiceAnchor(human = {}, message = "") {
  ...
  if (!previous || !current || wordCount(current) > 5 || SHORT_REPLY_QUESTION.test(current) || !/[a-z0-9]/i.test(current)) return "";
...
function applyContextualAnchor(intent = {}, human = {}, message = "") {
  ...
  const anchor = human.contextual_anchor || inferredAnchor;
...
function applyRent2BuyLocationRule(intent = {}, human = {}, message = "") {
  ...
  intent.clarification_required = false;
  intent.suggested_clarification_question = "";
  // A current postcode/town is already an authoritative coverage turn. Do not leave
  // `coverage` in secondary_intents here because the competence layer uses that flag
  // to substitute remembered location context. The current customer input must win.
