...
export const INTERNAL_ANALYTICS_VISITOR_PREFIX = "internal:";
export const CUSTOMER_ANALYTICS_TRUSTED_SINCE = "2026-08-15T06:00:00.000Z";
...
const ASSISTANT_ACCESS_EVENT_TYPES = new Set(["launcher_open", "conversation_start", "customer_message"]);
const TRUSTED_SINCE_MS = new Date(CUSTOMER_ANALYTICS_TRUSTED_SINCE).getTime();
...
function normaliseIntentLabels(values = []) {
  if (!Array.isArray(values)) return [];
  return [...new Set(values
    .map((value) => clean(value, 80).toLowerCase().replace(/[^a-z0-9_]+/g, "_"))
    .map((value) => value.replace(/^_+|_+$/g, ""))
    .filter(Boolean))].slice(0, 16);
}

export function isInternalAnalyticsVisitorHash(value) {
  return clean(value, 200).startsWith(INTERNAL_ANALYTICS_VISITOR_PREFIX);
}

export function isTrustedCustomerAnalyticsEvent(event = {}) {
  if (isInternalAnalyticsVisitorHash(event?.visitor_hash)) return false;
  const rawCreatedAt = clean(event?.created_at, 100);
  if (!rawCreatedAt) return true;
  const createdAt = new Date(rawCreatedAt).getTime();
  return Number.isFinite(createdAt) && createdAt >= TRUSTED_SINCE_MS;
}

export function filterTrustedCustomerAnalyticsEvents(events = []) {
  const source = Array.isArray(events) ? events : [];
  const internalHashes = new Set(source
    .map((event) => clean(event?.visitor_hash, 200))
    .filter((hash) => hash.startsWith(INTERNAL_ANALYTICS_VISITOR_PREFIX))
    .map((hash) => hash.slice(INTERNAL_ANALYTICS_VISITOR_PREFIX.length))
    .filter(Boolean));

  return source.filter((event) => {
    const hash = clean(event?.visitor_hash, 200);
    if (hash.startsWith(INTERNAL_ANALYTICS_VISITOR_PREFIX)) return false;
    if (hash && internalHashes.has(hash)) return false;
    return isTrustedCustomerAnalyticsEvent(event);
  });
}

export function assistantTelemetryVisitorHash(visitorId, environment = process.env) {
  const value = clean(visitorId, 200);
  const secret = clean(environment?.AI_ASSISTANT_SESSION_SECRET, 1000);
  if (!value || !secret) return null;
  const internal = value.startsWith(INTERNAL_ANALYTICS_VISITOR_PREFIX);
  const anonymousValue = internal ? clean(value.slice(INTERNAL_ANALYTICS_VISITOR_PREFIX.length), 160) : clean(value, 160);
  if (!anonymousValue) return null;
  const hash = secureHash(`analytics:${anonymousValue}`, secret);
  return internal ? `${INTERNAL_ANALYTICS_VISITOR_PREFIX}${hash}` : hash;
}
...
export function telemetryFromAssistantResult(result = {}) {
  return {
    ...
    secondary_intents: normaliseIntentLabels(result?.secondary_intents),
...
export function normaliseAssistantTelemetryEvent(input = {}) {
  ...
  return {
    ...
    visitor_hash: clean(input.visitor_hash, 200) || null,
    ...
    secondary_intents: normaliseIntentLabels(input.secondary_intents),
...
function assistantActorKey(event) {
  const visitor = clean(event?.visitor_hash, 200);
  if (visitor) return `visitor:${visitor}`;
  const session = clean(event?.customer_session_id, 80);
  return session ? `session:${session}` : null;
}

function activeUserWindow(events, days, nowMs) {
  const since = nowMs - days * 86_400_000;
  const windowEvents = (Array.isArray(events) ? events : []).filter((event) => eventTime(event?.created_at) >= since);
  const accessEvents = windowEvents.filter((event) => ASSISTANT_ACCESS_EVENT_TYPES.has(event?.event_type));
  const activeUsers = uniqueCount(accessEvents.map(assistantActorKey));
  const uniqueOpenUsers = uniqueCount(windowEvents.filter((event) => event?.event_type === "launcher_open").map(assistantActorKey));
  const conversations = uniqueCount(windowEvents.filter((event) => event?.event_type === "conversation_start").map((event) => event?.customer_session_id));
  return {
    period_days: days,
    active_users: activeUsers,
    unique_open_users: uniqueOpenUsers,
    conversations_started: conversations,
    customer_messages: windowEvents.filter((event) => event?.event_type === "customer_message").length,
  };
}

export function buildAssistantActiveUserWindows(events = [], now = new Date()) {
  const trustedEvents = filterTrustedCustomerAnalyticsEvents(events);
  const nowMs = now instanceof Date ? now.getTime() : new Date(now).getTime();
  const resolvedNow = Number.isFinite(nowMs) ? nowMs : Date.now();
  return {
    last_24_hours: activeUserWindow(trustedEvents, 1, resolvedNow),
    last_7_days: activeUserWindow(trustedEvents, 7, resolvedNow),
    last_30_days: activeUserWindow(trustedEvents, 30, resolvedNow),
  };
}

export function buildAssistantMeasurementSummary(events = [], searchEvents = []) {
  const assistantEvents = filterTrustedCustomerAnalyticsEvents(events);
  const searches = filterTrustedCustomerAnalyticsEvents(searchEvents);
  ...
  return {
    ...
    measurement: {
      customer_only: true,
      trusted_since: CUSTOMER_ANALYTICS_TRUSTED_SINCE,
      internal_test_events_excluded: (Array.isArray(events) ? events : []).filter((event) => isInternalAnalyticsVisitorHash(event?.visitor_hash)).length,
      internal_test_searches_excluded: (Array.isArray(searchEvents) ? searchEvents : []).filter((event) => isInternalAnalyticsVisitorHash(event?.visitor_hash)).length,
    },
  };
}
