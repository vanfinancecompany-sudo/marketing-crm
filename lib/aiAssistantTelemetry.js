...
export function telemetryFromAssistantResult(result = {}) {
  return {
    ...
    knowledge_sources: normaliseAssistantKnowledgeSources(result?.knowledge_sources_used),
  };
}
...
export async function recordAssistantTelemetryEvent(supabase, input = {}) {
  ...
  const payload = normaliseAssistantTelemetryEvent(input);
  const result = await supabase.from("ai_assistant_events").insert(payload).select("id").single();
  if (result?.error) throw result.error;
