...
function assistantActorKey(event) {
  const visitor = clean(event?.visitor_hash, 160);
...
export function buildAssistantActiveUserWindows(events = [], now = new Date()) {
  ...
  return {
    last_24_hours: activeUserWindow(events, 1, resolvedNow),
    last_7_days: activeUserWindow(events, 7, resolvedNow),
    last_30_days: activeUserWindow(events, 30, resolvedNow),
  };
}
