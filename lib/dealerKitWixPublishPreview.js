...
import { buildDealerKitWixMediaPlan } from "./dealerKitWixMediaPlan.js";
...
export function buildDealerKitWixPublishPreview({ vehicle = {}, decision = {}, wixResults = [] } = {}) {
  ...
  for (const mediaBlocker of mediaPlan.blockers || []) {
    if (["no_images", "missing_primary", "primary_not_included"].includes(mediaBlocker.code)) continue;
    blockers.push(blocker(`media_${mediaBlocker.code}`, mediaBlocker.message));
  }
