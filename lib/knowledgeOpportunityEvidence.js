...
const ASSISTANT_INTENT_PHRASES = Object.freeze({
  ...
  poor_credit: "am I applying with bad credit",
  ...
  vehicle: "which vehicle can I choose",
  application: "how do I apply and what happens next",
  ownership: "how does this product work and who owns the van at the end",
  multiple_vehicles: "which vehicles can I choose for my business",
});
...
export function aggregateKnowledgeOpportunityEvidence({ assistantEvents = [], searchEvents = [], visibilityResults = [], articles = [] } = {}) {
  ...
  (Array.isArray(assistantEvents) ? assistantEvents : [])
    ...
    .forEach((event) => {
      ...
      const question = safeQuery(event.customer_question);
      const intents = [...new Set(Array.isArray(event.secondary_intents) ? event.secondary_intents : [])];
      const classifications = new Map();

      const questionClassification = question ? classifyTextEvidenceIntent(question, product) : null;
      if (questionClassification) {
        classifications.set(questionClassification.key, { classified: questionClassification, intents: [] });
      }

      intents.forEach((intent) => {
        const classified = classifyAssistantEvidenceIntent(intent, product);
        if (!classified) return;
        const current = classifications.get(classified.key) || { classified, intents: [] };
        current.intents.push(intent);
        classifications.set(classified.key, current);
      });

      classifications.forEach(({ classified, intents: matchedIntents }) => {
        ...
        if (question) incrementQuery(group.assistant_questions, question, { count: 1 });
        matchedIntents.forEach((intent) => incrementQuery(group.assistant_intents, intent, { count: 1 }));
        touch(group, event.created_at);
      });

      if ((question || intents.length) && classifications.size === 0) unclassifiedAssistantEvents += 1;
    });
  ...
  return {
    groups: [...groups.values()].map((group) => ({
      ...group,
      assistant_questions: topQueries(group.assistant_questions),
...
export function evidenceChannelPayload(evidence = {}, windowDays = 90) {
  return {
    ...
    live_assistant: {
      ...
      question_variations: evidence.assistant_questions || [],
