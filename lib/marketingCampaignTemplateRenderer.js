...
function sampleData(values = {}) {
  return {
    ...
    first_name: cleanText(values.first_name || "Alex", 200),
...
function renderEscapedTextBlock(value, colour = "#1f2937", align = "left", typography = {}) {
  const normalized = String(value || "").replace(/\r\n/g, "\n");
  if (!normalized.trim()) return "";
  return normalized
    .split(/\n{2,}/)
    .map((part, index, parts) => `<p class="email-body-copy" style="margin:0 0 ${index < parts.length - 1 ? 16 : 0}px;font-family:Arial,sans-serif;font-size:${typography.size || 15}px;line-height:${typography.leading || 23}px;color:${colour};text-align:${align === "centre" ? "center" : "left"};">${part.replace(/\n/g, "<br>")}</p>`)
    .join("");
}
...
function formatRent2BuyMoney(value) {
  return String(value || "")
    .replace(/£\s*([\d,]+)(\.\d{1,2})?/g, (match, whole, decimals = "") => {
      const digits = whole.replace(/,/g, "");
      return /^\d+$/.test(digits) ? `£${Number(digits).toLocaleString("en-GB")}${decimals}` : match;
    })
    .replace(/\+\s*VAT\b/gi, "+ VAT")
    .trim();
}
function rent2BuyInitialRental(value) {
  return formatRent2BuyMoney(String(value || "").replace(/^(?:initial\s+rental\s*:?\s*)+/i, ""));
}
function rent2BuyTerm(value) {
  const text = String(value || "").replace(/^(?:term\s*:?\s*)+/i, "").trim();
  const abbreviated = text.match(/^x\s*(\d+)\s*(?:months?)?$/i);
  if (abbreviated) return `${abbreviated[1]} months`;
  return /^\d+$/.test(text) ? `${text} months` : text;
}
function rent2BuySupportingLines(profile = {}) {
  const initialRental = rent2BuyInitialRental(profile.initialRental);
  const term = rent2BuyTerm(profile.term);
  return [
    initialRental ? `Initial rental: ${initialRental}` : "",
    term ? `Term: ${term}` : "",
  ].filter(Boolean);
}
...
function renderVehicleImageCell(vehicle, href, imageWidth = 280) {
  ...
  const image = `<img class="email-vehicle-image" src="${escapeHtml(imageUrl)}" alt="${escapeHtml(alt)}" width="${imageWidth}" style="display:block;width:100%;max-width:${imageWidth}px;height:auto;border:0;outline:none;text-decoration:none;">`;
...
function renderSelectedVehicleCard(vehicle = {}, productMode = "finance", primaryColour, context = {}) {
  ...
  const supportingLines = isRent2Buy ? rent2BuySupportingLines(profile) : [];
  const ctaText = isRent2Buy ? "View Rent2Buy van" : "View van";
  // Presentation only: use the same resolved snapshot, copy and URL as the classic card.
  if (context.newStock) {
    const cashLine = compactLine([profile.price, profile.vat], " ");
    const monthlyLine = financeMonthlyLine(profile.monthly);
    const priceHtml = isRent2Buy ? escapeHtml(headline) : [
      cashLine ? `<span style="display:block;font-size:26px;line-height:32px;">${escapeHtml(cashLine)}</span>` : "",
      monthlyLine ? `<span style="display:block;padding-top:${cashLine ? 4 : 0}px;font-size:20px;line-height:27px;color:${primaryColour};">${escapeHtml(monthlyLine)}</span>` : "",
    ].join("");
    return `<table class="email-vehicle-card new-stock-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:0;border:1px solid #dce3ea;border-radius:10px;background:#ffffff;">
      ${renderVehicleImageCell(vehicle, href, context.twoColumn ? 318 : 654)}
      <tr><td class="email-vehicle-title" style="padding:20px 20px 10px;font-family:Arial,sans-serif;font-size:21px;line-height:27px;color:#142536;font-weight:bold;">${escapeHtml(title)}</td></tr>
      ${description ? `<tr><td class="email-vehicle-detail" style="padding:0 20px 6px;font-family:Arial,sans-serif;font-size:14px;line-height:21px;color:#526171;">${escapeHtml(description)}</td></tr>` : ""}
      ${spec ? `<tr><td class="email-vehicle-detail" style="padding:0 20px 12px;font-family:Arial,sans-serif;font-size:14px;line-height:21px;color:#526171;">${escapeHtml(spec)}</td></tr>` : ""}
      <tr><td style="padding:6px 20px 0;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:1px solid #e6ebf0;">
        ${headline ? `<tr><td class="email-vehicle-payment" style="padding:16px 0 8px;font-family:Arial,sans-serif;font-size:26px;line-height:32px;color:#142536;font-weight:bold;">${priceHtml}</td></tr>` : ""}
        <tr><td class="email-vehicle-detail" style="padding:${headline ? 0 : 16}px 0 12px;font-family:Arial,sans-serif;font-size:12px;line-height:18px;color:#526171;font-weight:bold;letter-spacing:0.04em;text-transform:uppercase;">${escapeHtml(fixedLine)}</td></tr>
        ${supportingLines.map((line) => `<tr><td class="email-vehicle-detail email-vehicle-terms" style="padding:0 0 6px;font-family:Arial,sans-serif;font-size:14px;line-height:21px;color:#526171;">${escapeHtml(line)}</td></tr>`).join("")}
        ${fallbackRegistration ? `<tr><td class="email-vehicle-detail" style="padding:0 0 8px;font-family:Arial,sans-serif;font-size:12px;line-height:18px;color:#64748b;">Reg: ${escapeHtml(fallbackRegistration)}</td></tr>` : ""}
      </table></td></tr>
      ${href ? `<tr><td style="padding:6px 20px 22px;"><table class="email-vehicle-button-table" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center" bgcolor="${primaryColour}" style="border:1px solid ${primaryColour};border-radius:6px;mso-padding-alt:15px 20px;"><a class="email-button-text email-vehicle-button" href="${escapeHtml(href)}" style="display:block;padding:15px 20px;font-family:Arial,sans-serif;font-size:16px;line-height:22px;color:#ffffff;text-decoration:none;font-weight:bold;text-align:center;mso-padding-alt:0;">${escapeHtml(ctaText)}</a></td></tr></table></td></tr>` : ""}
    </table>`;
  }
  return `<table class="email-vehicle-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;border:1px solid #dbe2ea;background:#ffffff;">
    ${renderVehicleImageCell(vehicle, href)}
    ${headline ? `<tr><td class="email-vehicle-payment" style="padding:14px 14px 2px;font-family:Arial,sans-serif;font-size:22px;line-height:27px;color:#0f172a;font-weight:bold;">${escapeHtml(headline)}</td></tr>` : ""}
    ...
    <tr><td class="email-vehicle-title" style="padding:0 14px 4px;font-family:Arial,sans-serif;font-size:16px;line-height:21px;color:#0f172a;font-weight:bold;">${escapeHtml(title)}</td></tr>
    ...
    ${supportingLines.map((line) => `<tr><td class="email-vehicle-detail email-vehicle-terms" style="padding:0 14px 5px;font-family:Arial,sans-serif;font-size:13px;line-height:19px;color:#334155;">${escapeHtml(line)}</td></tr>`).join("")}
    ...
    ${href ? `<tr><td style="padding:7px 14px 16px;"><table class="email-vehicle-button-table" role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${primaryColour}" style="border-radius:7px;"><a class="email-button-text email-vehicle-button" href="${escapeHtml(href)}" style="display:inline-block;padding:10px 13px;font-family:Arial,sans-serif;font-size:13px;line-height:18px;color:#ffffff;text-decoration:none;font-weight:bold;text-align:center;">${escapeHtml(ctaText)}</a></td></tr></table></td></tr>` : ""}
  </table>`;
}
...
function renderVehicleRows(cards, twoColumn) {
  ...
  for (let index = 0; index < cards.length; index += twoColumn ? 2 : 1) {
    rows.push(`<tr class="email-vehicle-row">${cards.slice(index, index + (twoColumn ? 2 : 1)).join("")}${twoColumn && !cards[index + 1] ? '<td class="email-vehicle-card-cell" width="50%" valign="top" style="padding:8px;"></td>' : ""}</tr>`);
  }
  return rows.join("");
}
...
function renderPlaceholderVehicleGrid(settings = {}, primaryColour) {
  ...
  const cards = vehicles.map((vehicle) => `
    <td class="email-vehicle-card-cell" width="${twoColumn ? "50%" : "100%"}" valign="top" style="padding:8px;">
      <table class="email-vehicle-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;border:1px solid #dbe2ea;background:#f8fafc;">
        ...
        <tr><td class="email-vehicle-title" style="padding:13px 12px 4px;font-family:Arial,sans-serif;font-size:15px;line-height:20px;color:#0f172a;font-weight:bold;">${escapeHtml(vehicle.name)}</td></tr>
        ...
        <tr><td class="email-vehicle-payment" style="padding:3px 12px 10px;font-family:Arial,sans-serif;font-size:16px;line-height:22px;color:#0f172a;font-weight:bold;">&#163;${escapeHtml(vehicle.price)}</td></tr>
        <tr><td style="padding:0 12px 14px;"><table class="email-vehicle-button-table" role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${primaryColour}" style="border-radius:7px;"><a class="email-button-text email-vehicle-button" href="https://www.vanfinancecompany.co.uk" style="display:inline-block;padding:10px 12px;font-family:Arial,sans-serif;font-size:13px;line-height:18px;color:#ffffff;text-decoration:none;font-weight:bold;text-align:center;">View Van</a></td></tr></table></td></tr>
      </table>
    </td>
  `);
  return `<table class="email-vehicle-grid" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin:10px 0;">${renderVehicleRows(cards, twoColumn)}</table>`;
}

function renderSelectedVehicleGrid(settings = {}, primaryColour, context = {}) {
  ...
  const twoColumn = settings.layout === "two_column";
  const cards = selected.map((vehicle) => `<td class="email-vehicle-card-cell" width="${twoColumn ? "50%" : "100%"}" valign="top" style="padding:8px;">${renderSelectedVehicleCard(vehicle, productMode, primaryColour, { ...context, twoColumn })}</td>`);
  return `<table class="email-vehicle-grid" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin:10px 0;">${renderVehicleRows(cards, twoColumn)}</table>`;
}

function renderVehicleGrid(settings = {}, primaryColour, context = {}) {
  const selected = Array.isArray(settings.selected_vehicles) ? settings.selected_vehicles : [];
  if (settings.source_mode === "selected" && selected.length) return renderSelectedVehicleGrid(settings, primaryColour, context);
  return renderPlaceholderVehicleGrid(settings, primaryColour);
}

function textToHtml(value, values = {}, typography = {}) {
  ...
  return escapeHtml(withTextPlaceholders)
    ...
    .map((part, index, parts) => `${renderEscapedTextBlock(part, typography.colour, "left", typography)}${index < parts.length - 1 ? renderVehicleGrid({ number_of_vehicles: 3, layout: "one_column" }, cleanColour(values.brand_colour, "#2563eb")) : ""}`)
    .join("");
}
...
function renderTextBlock(block, values, context = {}) {
  ...
  const padding = paddingPixels(s.padding_size) + (context.newStock ? 6 : 0);
  ...
  return `<tr><td class="email-text-panel" bgcolor="${s.background_colour}" style="padding:${topPadding}px ${context.newStock ? 32 : 30}px ${bottomPadding}px;background:${s.background_colour};">
    ${heading ? `<h2 class="email-section-heading" style="margin:0 0 12px;font-family:Arial,sans-serif;font-size:${context.newStock ? 25 : 23}px;line-height:${context.newStock ? 32 : 29}px;color:${s.text_colour};text-align:${s.alignment === "centre" ? "center" : "left"};">${escapeHtml(heading)}</h2>` : ""}
    ${renderEscapedTextBlock(escapeHtml(body), s.text_colour, s.alignment, context.newStock ? { size: 16, leading: 26 } : {})}
  </td></tr>`;
}
...
function renderManualImageBlock(block, values) {
  ...
  return `<tr><td class="email-text-panel" bgcolor="${s.background_colour}" align="${s.alignment === "right" ? "right" : s.alignment === "left" ? "left" : "center"}" style="padding:${padding}px 30px;background:${s.background_colour};">
    ${heading ? `<h2 class="email-section-heading" style="margin:0 0 12px;font-family:Arial,sans-serif;font-size:22px;line-height:28px;color:#0f172a;">${escapeHtml(heading)}</h2>` : ""}
    ${linkedImage}
...
function renderButtonBlock(block, values, context = {}) {
  ...
  if (context.newStock) return `<tr><td class="new-stock-cta-panel" align="${s.alignment === "right" ? "right" : s.alignment === "centre" ? "center" : "left"}" style="padding:16px 32px ${bottomPadding}px;background:#ffffff;">
    <table class="new-stock-main-button" role="presentation" ${full ? 'width="100%"' : ""} cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${s.primary_colour}" align="center" style="border:1px solid ${s.primary_colour};border-radius:6px;mso-padding-alt:18px 30px;"><a class="email-button-text" href="${escapeHtml(s.url)}" style="display:block;padding:18px 30px;font-family:Arial,sans-serif;font-size:18px;line-height:24px;color:${s.text_colour};text-decoration:none;font-weight:bold;text-align:center;mso-padding-alt:0;">${escapeHtml(text)}</a></td></tr></table>
  </td></tr>`;
...
function renderVehicleGridBlock(block, values, context = {}) {
  ...
  return `<tr><td class="email-vehicle-panel" style="padding:${topPadding}px ${context.newStock ? 24 : 22}px ${bottomPadding}px;background:#ffffff;">
    ${heading ? `<h2 class="email-section-heading" style="margin:0 8px 8px;font-family:Arial,sans-serif;font-size:23px;line-height:29px;color:#0f172a;">${escapeHtml(heading)}</h2>` : ""}
    ...
    ${renderVehicleGrid(s, cleanColour(values.brand_colour, "#2563eb"), context)}
...
function renderContentBlocks(values = {}) {
  ...
  return blocks
    .map((block, index) => {
      const context = { newStock: values.master_layout === "new_stock", previousType: blocks[index - 1]?.type || "", nextType: blocks[index + 1]?.type || "" };
...
export function renderEmailHtml(values = {}) {
  const snapshot = normalizeTemplateSnapshot(values);
  ...
  const newStock = snapshot.master_layout === "new_stock";
  const canvas = newStock ? "#e9eef2" : "#eef3f8";
  const emailWidth = newStock ? 720 : 660;
  const heroColour = newStock ? "#142536" : primary;
  const logoHtml = snapshot.header_logo ? `<tr><td align="${newStock ? "left" : "center"}" style="padding:${newStock ? "24px 32px" : "22px 24px 14px"};background:#ffffff;"><img src="${escapeHtml(snapshot.header_logo)}" alt="${escapeHtml(companyName)}" width="180" style="display:block;max-width:180px;width:100%;height:auto;border:0;outline:none;text-decoration:none;"></td></tr>` : `<tr><td align="${newStock ? "left" : "center"}" style="padding:${newStock ? "24px 32px" : "24px 24px 14px"};background:#ffffff;font-family:Arial,sans-serif;font-size:20px;line-height:26px;color:#0f172a;font-weight:bold;">${escapeHtml(companyName)}</td></tr>`;
  return `<!doctype html>
...
@media only screen and (max-width:600px) {
  ...
  .email-outer-padding { padding:12px 6px !important; }
  .email-hero-heading { font-size:32px !important; line-height:38px !important; }
  .email-section-heading { font-size:24px !important; line-height:31px !important; }
  .email-text-panel { padding-left:18px !important; padding-right:18px !important; }
  .email-vehicle-panel { padding-left:8px !important; padding-right:8px !important; }
  .email-vehicle-grid, .email-vehicle-card { width:100% !important; }
  .email-vehicle-button-table { width:100% !important; }
  .email-vehicle-row, .email-vehicle-card-cell { display:block !important; width:100% !important; max-width:100% !important; box-sizing:border-box !important; }
  .email-vehicle-card-cell { padding:8px 0 !important; }
  .email-vehicle-image { width:100% !important; max-width:none !important; height:auto !important; }
  .email-vehicle-payment { font-size:21px !important; line-height:29px !important; }
  .email-vehicle-title { font-size:16px !important; line-height:23px !important; }
  .email-vehicle-detail, .email-vehicle-terms { font-size:15px !important; line-height:22px !important; }
  .email-button-text { font-size:16px !important; line-height:22px !important; }
  .email-vehicle-button { display:block !important; width:auto !important; }
}${newStock ? `
@media only screen and (max-width:600px) {
  .new-stock-email .email-outer-padding { padding:0 !important; }
  .new-stock-email .email-hero { padding:30px 24px 26px !important; }
  .new-stock-email .email-hero-heading { font-size:34px !important; line-height:39px !important; }
  .new-stock-email .email-text-panel, .new-stock-cta-panel { padding-left:24px !important; padding-right:24px !important; }
  .new-stock-email .email-vehicle-panel { padding-left:16px !important; padding-right:16px !important; }
  .new-stock-email .email-vehicle-card-cell { padding:8px 0 16px !important; }
  .new-stock-email .email-vehicle-card-cell:empty { display:none !important; }
  .new-stock-email .email-vehicle-title { font-size:22px !important; line-height:28px !important; }
  .new-stock-email .email-body-copy { line-height:26px !important; }
  .new-stock-main-button { width:100% !important; }
  .new-stock-footer { padding:24px !important; }
  .new-stock-footer .email-body-copy { font-size:12px !important; line-height:19px !important; }
}` : ""}
</style></head>
<body${newStock ? ' class="new-stock-email"' : ""} style="margin:0;padding:0;background:${canvas};"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${canvas}" style="width:100%;background:${canvas};border-collapse:collapse;"><tr><td class="email-outer-padding" align="center" style="padding:${newStock ? "32px 12px" : "24px 12px"};">${newStock ? '<!--[if mso]><table role="presentation" width="720" cellpadding="0" cellspacing="0" border="0"><tr><td><![endif]-->' : ""}<table role="presentation" width="${emailWidth}" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:${emailWidth}px;background:#ffffff;border-collapse:collapse;${newStock ? "" : "border-radius:14px;overflow:hidden;"}">
${logoHtml}<tr><td class="email-hero" bgcolor="${heroColour}" style="padding:${newStock ? "36px 32px 30px" : "34px 30px"};background:${heroColour};${newStock ? `border-top:4px solid ${primary};` : ""}"><p style="margin:0 0 ${newStock ? 14 : 8}px;font-family:Arial,sans-serif;font-size:${newStock ? 11 : 13}px;line-height:18px;color:${newStock ? "#c2d2e0" : "#ffffff"};font-weight:bold;letter-spacing:0.06em;text-transform:uppercase;">${escapeHtml(companyName)}</p><h1 class="email-hero-heading" style="margin:0;font-family:Arial,sans-serif;font-size:${newStock ? 42 : 34}px;line-height:${newStock ? 46 : 40}px;color:#ffffff;font-weight:bold;">${escapeHtml(heroHeading)}</h1></td></tr>
<tr><td class="email-support-copy" bgcolor="${newStock ? heroColour : secondary}" style="padding:${newStock ? "0 32px 32px" : "14px 30px"};background:${newStock ? heroColour : secondary};font-family:Arial,sans-serif;font-size:${newStock ? 16 : 14}px;line-height:${newStock ? 25 : 20}px;color:${newStock ? "#d5dfe8" : "#334155"};">${escapeHtml(previewText)}</td></tr>
${hasBlocks ? renderContentBlocks(snapshot) : renderLegacyBody(snapshot)}
<tr><td${newStock ? ' class="new-stock-footer"' : ""} style="padding:${newStock ? "26px 32px 30px" : "22px 30px 30px"};background:#f8fafc;${newStock ? "border-top:1px solid #dce3ea;" : ""}font-family:Arial,sans-serif;font-size:12px;line-height:18px;color:#64748b;">${textToHtml(snapshot.footer, snapshot, newStock ? { size: 12, leading: 19, colour: "#64748b" } : {})}${snapshot.social_links ? `<p style="margin:10px 0 0;font-family:Arial,sans-serif;font-size:12px;line-height:18px;color:#64748b;">${escapeHtml(snapshot.social_links)}</p>` : ""}</td></tr>
</table>${newStock ? '<!--[if mso]></td></tr></table><![endif]-->' : ""}</td></tr></table></body></html>`;
}

export function renderCampaignPreview(campaign = {}) {
  ...
  return {
    subject: replaceTextPlaceholders(snapshot.default_subject, snapshot),
    preview_text: replaceTextPlaceholders(snapshot.preview_text, snapshot),
    html: renderEmailHtml(snapshot),
  };
}
