...
function renderVehicleImageCell(vehicle, href) {
  ...
  const image = `<img class="email-vehicle-image" src="${escapeHtml(imageUrl)}" alt="${escapeHtml(alt)}" width="280" style="display:block;width:100%;max-width:280px;height:auto;border:0;outline:none;text-decoration:none;">`;
...
function renderSelectedVehicleGrid(settings = {}, primaryColour) {
  ...
  const twoColumn = settings.layout === "two_column";
  const cards = selected.map((vehicle) => `<td class="email-vehicle-card-cell" width="${twoColumn ? "50%" : "100%"}" valign="top" style="padding:8px;">${renderSelectedVehicleCard(vehicle, productMode, primaryColour)}</td>`);
  return `<table class="email-vehicle-grid" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin:10px 0;">${renderVehicleRows(cards, twoColumn)}</table>`;
}
...
function renderTextBlock(block, values, context = {}) {
  ...
  return `<tr><td class="email-text-panel" bgcolor="${s.background_colour}" style="padding:${topPadding}px 30px ${bottomPadding}px;background:${s.background_colour};">
    ${heading ? `<h2 class="email-section-heading" style="margin:0 0 12px;font-family:Arial,sans-serif;font-size:23px;line-height:29px;color:${s.text_colour};text-align:${s.alignment === "centre" ? "center" : "left"};">${escapeHtml(heading)}</h2>` : ""}
...
function renderVehicleGridBlock(block, values, context = {}) {
  ...
  return `<tr><td class="email-vehicle-panel" style="padding:${topPadding}px 22px ${bottomPadding}px;background:#ffffff;">
...
<body style="margin:0;padding:0;background:#eef3f8;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#eef3f8" style="width:100%;background:#eef3f8;border-collapse:collapse;"><tr><td class="email-outer-padding" align="center" style="padding:24px 12px;"><table role="presentation" width="660" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:660px;background:#ffffff;border-collapse:collapse;border-radius:14px;overflow:hidden;">
${logoHtml}<tr><td class="email-hero" bgcolor="${primary}" style="padding:34px 30px;background:${primary};"><p style="margin:0 0 8px;font-family:Arial,sans-serif;font-size:13px;line-height:18px;color:#ffffff;font-weight:bold;letter-spacing:0.06em;text-transform:uppercase;">${escapeHtml(companyName)}</p><h1 class="email-hero-heading" style="margin:0;font-family:Arial,sans-serif;font-size:34px;line-height:40px;color:#ffffff;font-weight:bold;">${escapeHtml(heroHeading)}</h1></td></tr>
