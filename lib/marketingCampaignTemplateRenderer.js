...
function renderEscapedTextBlock(value, colour = "#1f2937", align = "left") {
  ...
  return normalized
    .split(/\n{2,}/)
    .map((part, index, parts) => `<p style="margin:0 0 ${index < parts.length - 1 ? 16 : 0}px;font-family:Arial,sans-serif;font-size:15px;line-height:23px;color:${colour};text-align:${align === "centre" ? "center" : "left"};">${part.replace(/\n/g, "<br>")}</p>`)
    .join("");
}
...
function renderVehicleImageCell(vehicle, href) {
  ...
  if (!imageUrl) return `<tr><td align="center" bgcolor="#e2e8f0" style="padding:30px 12px;font-family:Arial,sans-serif;font-size:13px;line-height:18px;color:#475569;">Vehicle image</td></tr>`;
...
function renderSelectedVehicleGrid(settings = {}) {
  ...
  const twoColumn = settings.layout === "two_column";
  const cards = selected.map((vehicle) => `<td width="${twoColumn ? "50%" : "100%"}" valign="top" style="padding:8px;">${renderSelectedVehicleCard(vehicle, productMode)}</td>`);
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin:10px 0;">${renderVehicleRows(cards, twoColumn)}</table>`;
}
