...
function semanticLocations(article = {}) {
  ...
  sections(article.content_markdown).forEach((section) => {
    locations.push({ section: `Article ${section.heading}`, text: section.heading });
    section.text.split(/\r?\n/).forEach((line) => { if (line.trim()) locations.push({ section: `Article ${section.heading}`, text: line }); });
  });
  ...
  (Array.isArray(article.internal_link_suggestions) ? article.internal_link_suggestions : []).forEach((item, index) => {
    locations.push({ section: `Internal-link anchor ${index + 1}`, text: item?.anchor_text || item?.label || "" });
  });
  return locations;
}
...
function protectApprovedNames(value) {
  return String(value || "")
    .replaceAll(RENT2BUY_SEPARATION_SENTENCE, "[APPROVED RENT2BUY SEPARATION]")
    .replaceAll(COMPANY_NAME, COMPANY_TOKEN);
}
...
export function validateRent2BuySemantics(article = {}, options = {}) {
  ...
  const unique = [...new Map(errors.map((item) => [`${item.category}|${item.section}|${normalize(item.phrase)}`, item])).values()];
...
export function rent2BuyPromptRule(subject = {}) {
  ...
  return `\n# Permanent Rent2Buy product rule\n${RENT2BUY_CORE_WORDING}\nFor pure Rent2Buy content, correct every validated field: article title, SEO title, meta description, excerpt, full Markdown body, every FAQ question and answer, CTA wording and every internal-link anchor. Never change link destinations. Perform a final full-text semantic scan before returning. Do not return prohibited finance, trial or delivery concepts, even in contrast or denial. The exact proper name “${COMPANY_NAME}” is allowed only as company identification; claims attached to it remain prohibited. Rewrite affected wording naturally with agreement, rental payments, eligibility, application, ownership conditions, vehicle availability, collection and Southampton. Preserve at least 80% of valid non-prohibited content and valid Markdown. Do not summarise. Place “${RENT2BUY_SEPARATION_SENTENCE}” naturally in the introduction and “${RENT2BUY_COLLECTION_SENTENCE}” in Key Features, Collection or Practical Next Steps, both before the CTA. Mixed content may retain separate Finance and Rent2Buy sections only when explicitly classified as both.`;
}
