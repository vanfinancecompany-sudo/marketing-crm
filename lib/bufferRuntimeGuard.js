...
export async function recordBufferRateLimitTelemetry(response, { source = "personal_access" } = {}) {
  const rateLimits = parseBufferRateLimitHeaders(response);
  if (!rateLimits.length) return loadBufferRuntimeState({ source });
...
export async function guardedBufferGraphql({
  ...
  await recordBufferRateLimitTelemetry(response, { source: credential.source });
