...
import { handoffBufferImageVariables } from "./socialImageHandoff.js";
...
export async function guardedBufferGraphql({
  ...
  const preparedVariables = await handoffBufferImageVariables(variables);
  const response = await fetch(url, {
    ...
    body: JSON.stringify(preparedVariables ? { query, variables: preparedVariables } : { query }),
  });
