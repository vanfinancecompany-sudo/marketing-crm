...
export function publicApplicationCta(pageType, productLock, assistantResult = {}) {
  const show = Boolean(
    assistantResult.application_cta_generated
    || assistantResult.application_mode_active
    || assistantResult.recommended_cta === "Show application button"
    || ["apply_finance", "apply_rent2buy"].includes(assistantResult.recommended_action),
  );
  if (!show) return null;
  if (["finance_vehicle", "finance_general"].includes(pageType) && productLock !== "finance") return null;
  if (pageType === "rent2buy_general" && productLock !== "rent2buy") return null;
  if (pageType === "finance_vehicle" && productLock === "finance") return {
    label: "Apply for this van",
    action: "open_current_page_finance_application",
    behavior: "same_page",
    url: null,
  };
  if (productLock === "finance") return {
    label: "Start Finance Application",
    action: "navigate",
    behavior: "same_window",
    url: FINANCE_APPLICATION_URL,
  };
  if (productLock === "rent2buy") return {
    label: "Start Rent2Buy Application",
    action: "navigate",
    behavior: "same_window",
    url: RENT2BUY_APPLICATION_URL,
  };
  return null;
}
