...
export function allowedWixOrigins(environment = process.env) {
  const configured = clean(environment.AI_ASSISTANT_ALLOWED_ORIGINS, 4000)
    ...
    .filter(Boolean);
  // The two production websites are always trusted. Optional configured origins are additive for Wix previews/test sites.
...
export function normalisePageContext(input = {}) {
  ...
  const vehicle = hasVehicleContext ? {
    ...
    apply_link: clean(suppliedVehicle.apply_link ?? suppliedVehicle.applyLink, 1000) || null,
    pricing: normalisePublicVehiclePricing(suppliedVehicle.pricing || {}),
    // Rent2Buy agreement length is intentionally not accepted from vehicle-page context.
    // Exact term details must be supported by approved Rent2Buy Business Knowledge.
    term_months: pageType === "rent2buy_general" ? null : normaliseVehicleTermMonths(suppliedVehicle.term_months ?? suppliedVehicle.termMonths),
  } : {};
...
export function initialCustomerReply(pageType, vehicle = {}) {
  ...
  if (pageType === "rent2buy_general" && hasVehicle) return "Hi — I can help with Rent2Buy costs and applying for this van. What would you like to know?";
