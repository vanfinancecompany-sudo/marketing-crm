...
import { normalisePublicVehiclePricing, normaliseVehicleTermMonths } from "./publicVehiclePricing.js";
...
function publicList(value, limit = 20) {
  const source = Array.isArray(value) ? value : clean(value, 3000).split(/[\n|,]+/);
  return source.map((item) => clean(item, 160)).filter(Boolean).slice(0, limit);
}

function publicSpecifications(value) {
  const input = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const allowed = ["engine_size", "power", "payload", "wheelbase", "doors", "seats", "euro_standard", "mpg"];
  return Object.fromEntries(allowed.map((key) => [key, clean(input[key], 120) || null]));
}
...
export function allowedWixOrigins(environment = process.env) {
  const configured = clean(environment.AI_ASSISTANT_ALLOWED_ORIGINS, 4000)
    ...
    .filter(Boolean);
  return [...new Set([...DEFAULT_WIX_ORIGINS, ...configured])];
}
...
export function normalisePageContext(input = {}) {
  ...
  const suppliedVehicle = input.vehicle && typeof input.vehicle === "object" ? input.vehicle : {};
  const hasVehicleContext = pageType === "finance_vehicle" || (
    pageType === "rent2buy_general"
    && Boolean(registration || clean(suppliedVehicle.vehicle_id, 100) || clean(suppliedVehicle.title, 200))
  );
  const vehicle = hasVehicleContext ? {
    registration: registration || null,
    vehicle_id: clean(suppliedVehicle.vehicle_id, 100) || null,
    title: clean(suppliedVehicle.title, 200) || null,
    make: clean(suppliedVehicle.make, 100) || null,
    model: clean(suppliedVehicle.model, 100) || null,
    derivative: clean(suppliedVehicle.derivative, 160) || null,
    year: clean(suppliedVehicle.year, 80) || null,
    mileage: clean(suppliedVehicle.mileage, 80) || null,
    transmission: clean(suppliedVehicle.transmission, 80) || null,
    fuel: clean(suppliedVehicle.fuel ?? suppliedVehicle.fuel_type ?? suppliedVehicle.fuelType, 80) || null,
    body_type: clean(suppliedVehicle.body_type ?? suppliedVehicle.bodyType ?? suppliedVehicle.vehicle_type, 100) || null,
    colour: clean(suppliedVehicle.colour ?? suppliedVehicle.color, 80) || null,
    stock_status: clean(suppliedVehicle.stock_status, 40) || null,
    short_description: clean(suppliedVehicle.short_description ?? suppliedVehicle.description, 1000) || null,
    important_features: publicList(suppliedVehicle.important_features ?? suppliedVehicle.features ?? suppliedVehicle.highlights),
    public_specifications: publicSpecifications(suppliedVehicle.public_specifications),
    description: clean(suppliedVehicle.description, 7000) || null,
    highlights: clean(suppliedVehicle.highlights, 7000) || null,
    specification: clean(suppliedVehicle.specification, 7000) || null,
    vehicle_link: publicLink(suppliedVehicle.vehicle_link ?? suppliedVehicle.vehicleLink),
    apply_link: publicLink(suppliedVehicle.apply_link ?? suppliedVehicle.applyLink),
    enquiry_link: publicLink(suppliedVehicle.enquiry_link ?? suppliedVehicle.enquiryLink),
    pricing: normalisePublicVehiclePricing(suppliedVehicle.pricing || {}),
    vat_status: clean(suppliedVehicle.vat_status, 80) || null,
    term_months: normaliseVehicleTermMonths(suppliedVehicle.term_months ?? suppliedVehicle.termMonths),
    final_purchase_option: pageType === "rent2buy_general" ? "£99 + VAT" : null,
  } : {};
  const product = ["finance_vehicle", "finance_general"].includes(pageType) ? "finance" : pageType === "rent2buy_general" ? "rent2buy" : null;
  const pagePath = clean(input.page_path ?? input.pagePath, 500);
  return {
    product,
    page_type: pageType,
    page_path: pagePath.startsWith("/") ? pagePath : null,
    page_title: clean(input.page_title ?? input.pageTitle, 200) || null,
    category: clean(input.category, 100) || null,
    available_customer_action: clean(input.available_customer_action ?? input.availableCustomerAction, 160) || null,
    context_source: clean(input.context_source ?? input.contextSource, 100) || null,
    vehicle,
  };
}
...
export function initialCustomerReply(pageType, vehicle = {}) {
  const hasVehicle = Boolean(clean(vehicle?.registration || vehicle?.vehicle_id || vehicle?.title, 200));
  if (pageType === "finance_vehicle") return "Hi — I can help with finance, pricing and applying for this van. What would you like to know?";
  if (pageType === "finance_general") return "Hi — how can I help with van finance?";
  if (pageType === "rent2buy_general" && hasVehicle) return "Hi — I can help with Rent2Buy costs, terms and applying for this van. What would you like to know?";
