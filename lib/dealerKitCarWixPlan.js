...
// CARS_CREATE_OR_UPDATE_RECONCILE: Cars reuses exact Wix item IDs and safely
// reconciles create/update/restore operations instead of treating existing stock
// as a special image-only path.
function carItemPublishStatus(item = {}) {
  return clean(item?.data?._publishStatus || item?._publishStatus || "", 80).toUpperCase();
}

function carReconcileTarget({ collectionId, kind, data, existingItem }) {
  let target = {
    product: "cars",
    collectionId,
    kind,
  ...
  if (!existingItem?.id) return target;
  const currentPublishStatus = carItemPublishStatus(existingItem);
  target = {
    ...target,
    operation: "update",
    itemId: clean(existingItem.id, 300),
    previousData: existingItem.data || {},
    currentPublishStatus: currentPublishStatus || null,
    publishStatusOperation: currentPublishStatus && currentPublishStatus !== "PUBLISHED" ? "publish" : null,
  };
  return target;
}
...
export function buildDealerKitCarWixPlan({ vehicle = {}, decision = {}, imageSet = {}, carListingRows = [], carDetailRows = [] } = {}) {
  ...
  const blockers = [];
  const listingRows = Array.isArray(carListingRows) ? carListingRows : [];
  const detailRows = Array.isArray(carDetailRows) ? carDetailRows : [];
  ...
  if (!imageSet.ready || !imageSet.mainUrl || !imageSet.galleryUrls?.length) blockers.push({ code: "car_media_not_ready", message: "Cars primary image is not READY in Wix Media." });
  if (listingRows.length > 1) blockers.push({ code: "car_listing_ambiguous", message: `CARFINANCE contains ${listingRows.length} rows for ${registration}. Resolve the duplicate before reconciliation.` });
  if (detailRows.length > 1) blockers.push({ code: "car_detail_ambiguous", message: `CARPAGES contains ${detailRows.length} rows for ${registration}. Resolve the duplicate before reconciliation.` });
  if (listingRows.length === 1 && !listingRows[0]?.id) blockers.push({ code: "car_listing_identity_missing", message: "The existing CARFINANCE row does not have a usable Wix item ID." });
  if (detailRows.length === 1 && !detailRows[0]?.id) blockers.push({ code: "car_detail_identity_missing", message: "The existing CARPAGES row does not have a usable Wix item ID." });

  const sections = featureSections(vehicle);
  const description = clean(vehicle.description, 10000) || headline(vehicle);
  const listingData = registration && retailPrice !== null && monthlyPrice !== null && imageSet.mainUrl ? {
    title: registration,
  ...
  const detailData = registration && retailPrice !== null && monthlyPrice !== null && imageSet.galleryUrls?.length ? {
    title: registration,
  ...
  const targets = [];
  if (listingData) targets.push(carReconcileTarget({ collectionId: "CARFINANCE", kind: "listing", data: listingData, existingItem: listingRows[0] || null }));
  if (detailData) targets.push(carReconcileTarget({ collectionId: "CARPAGES", kind: "detail", data: detailData, existingItem: detailRows[0] || null }));

  const hasExistingTargets = targets.some((target) => target.operation === "update");
  return {
    version: 2,
    mode: "cars",
    registration,
...
export function buildCarPublishConfirmation(plan = {}) {
  return {
    ...
    targetPayloads: (plan.targets || []).map((target) => ({
      ...
      currentPublishStatus: target.currentPublishStatus || null,
      desiredPublishStatus: target.desiredPublishStatus || null,
      publishStatusOperation: target.publishStatusOperation || null,
