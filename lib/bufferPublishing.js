...
export function buildBufferCreatePostInput({
  ...
  if (!["facebook", "instagram", "googlebusiness"].includes(service)) {
  ...
  const urls = mediaCandidates
    ...
    .filter(Boolean)
    .filter((value) => {
      if (seenMedia.has(value)) return false;
      seenMedia.add(value);
      return true;
    })
    .slice(0, isVideo ? 1 : 3)
    .map((value) => assertHttpUrl(value, isVideo ? "Video URL" : "Image URL"));
  if (!urls.length) {
    throw new Error(`${isVideo ? "Video URL" : "Image URL"} must be a public HTTPS URL.`);
  }
    ...
    : service === "googlebusiness"
      ? {
          google: {
            type: "whats_new",
            detailsWhatsNew: {
              button: linkUrl ? "learn_more" : "none",
              ...(linkUrl ? { link: assertHttpUrl(linkUrl, "Google Business link") } : {}),
            },
          },
        }
      : {
...
export function selectVanFinanceGoogleBusinessChannel(channels = []) {
  const available = (Array.isArray(channels) ? channels : []).filter((channel) =>
    clean(channel?.service).toLowerCase() === "googlebusiness"
      && channel?.isDisconnected !== true
      && channel?.isLocked !== true,
  );
  if (!available.length) throw new Error("No connected Buffer Google Business channel is available.");
  ...
  throw new Error("More than one Buffer Google Business channel is connected; Van Finance Company could not be selected safely.");
}
