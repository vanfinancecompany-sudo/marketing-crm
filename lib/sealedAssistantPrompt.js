...
export function buildSealedAssistantPlatformPrompt({ sections = [], live = false } = {}) {
  const context = sealedBusinessKnowledgeContext(sections);
  const task = live
    ? "Task: Reply to the current customer as the live website sales assistant using only the controlled context and evidence."
