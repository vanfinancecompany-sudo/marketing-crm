...
function formatMileage(value) {
  ...
  return Number.isFinite(number) && number >= 0 ? Math.round(number).toLocaleString("en-GB") : "";
}
...
export function dealerKitDescriptionFacts(vehicle = {}) {
  ...
  return {
    ...
    bhp: Number.isFinite(Number(vehicle.bhp)) ? Number(vehicle.bhp) : null,
    features,
...
export function buildDealerKitVehicleSpecText(vehicle = {}) {
  ...
  const rows = [
    ...
    ["MILLAGE", formatMileage(vehicle.mileage)],
    ...
    ["BHP", Number.isFinite(Number(vehicle.bhp)) ? String(Math.round(Number(vehicle.bhp))) : ""],
  ].filter(([, value]) => value);
