...
const APPLICATION_STARTED = /\b(already (?:started|applying)|application (?:is )?started|halfway through|abandoned (?:my |the )?application|continue (?:my |the )?application|didn(?:'|\s)?t finish applying|did not finish applying|stopped (?:my |the )?application|paused (?:my |the )?application)\b/i;
...
export function applicationModeReply(productContext, state = "ready") {
  ...
  if (state === "started") return `Your ${product} application is already underway. Use the APPLY NOW button on this page to continue, and ask here if you need help understanding a question.`;
  return `Great. When you’re ready, use the APPLY NOW button on this page to start your ${product} application. Complete it as accurately as you can, and ask here if you need help while you’re applying.`;
}
