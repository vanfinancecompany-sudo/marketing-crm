...
export function cleanImportRow(row, pipeline) {
  ...
  return { contact: { first_name, last_name, company: String(getField(row, FIELD_ALIASES.company) || "").trim(), email, email_normalized: email || null, phone, phone_normalized: phone || null, postcode: cleanPostcode(getField(row, FIELD_ALIASES.postcode)), pipeline: resolvedPipeline, source, sources: [source], tags: [...tags].sort(), notes: String(getField(row, FIELD_ALIASES.notes) || "").trim(), marketing_status: "active", lifecycle_status: "active", lifecycle_changed_at: new Date().toISOString(), email_ready: Boolean(email), sms_ready: Boolean(phone), facebook_ready: Boolean(email || phone) }, reason: "", invalidEmail: false, pipelineExplicit: Boolean(explicitPipeline), sourceCustomerId: String(getField(row, FIELD_ALIASES.customer_id) || "").trim().toUpperCase() };
}
...
export function mergeContactPayload(existing, incoming, options = {}) {
  ...
  return { first_name: incoming.first_name || existing.first_name || "", last_name: incoming.last_name || existing.last_name || "", company: incoming.company || existing.company || "", email: canReplaceEmail ? (incoming.email || existing.email || null) : (existing.email || null), email_normalized: emailNormalized, phone: incoming.phone || existing.phone || null, phone_normalized: phoneNormalized, postcode: incoming.postcode || existing.postcode || "", pipeline: options.pipelineExplicit ? (incoming.pipeline || existing.pipeline || "unknown") : (existing.pipeline || incoming.pipeline || "unknown"), source: incoming.source || existing.source || "csv", sources: [...sources], tags: [...tags].sort(), notes: incoming.notes || existing.notes || "", marketing_status: "active", lifecycle_status: "active", lifecycle_changed_at: new Date().toISOString(), email_ready: Boolean(emailNormalized), sms_ready: Boolean(phoneNormalized), facebook_ready: Boolean(emailNormalized || phoneNormalized), duplicate_count: Number(existing.duplicate_count || 0) + 1, last_seen_at: new Date().toISOString() };
}
