...
export function controlledVehicleNextStepReply(message = "", productContext = "finance") {
  ...
  return "The next step is to apply for a personalised Finance quote. If you’re approved and choose the van, the finance documents are sent digitally; once they’re signed, the £100 reservation deposit takes the van off sale and it enters the preparation process.";
}
...
export function controlledBusinessRuntimeReply({ message = "", productContext = "finance", runtimeContext = {} } = {}) {
  const text = normaliseCustomerMessage(message);
  ...
  if (/\b(?:deliver|delivery)\b/.test(text)) {
    const coverage = productContext === "rent2buy" ? buildRent2BuyDeliveryEvidence(message) : buildFinanceCoverageEvidence(message);
