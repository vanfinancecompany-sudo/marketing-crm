...
import { deterministicRent2BuyCoverageReply } from "./rent2buyCoverageReply.js";
import { buildFinanceCoverageEvidence, buildRent2BuyDeliveryEvidence } from "./productCoverageRules.js";
...
export function detectBuyingSignals(message, facts = {}) {
  ...
  const actions = {
    ...
    documents_ready: "Guide them to the next application step.", delivery_interest: "Apply the deterministic delivery rule only when the current question is actually about delivery logistics.", collection_interest: "Use approved collection knowledge or flag uncertainty.",
...
export function contextualClarification(message, messages = [], facts = {}) {
  const text = normaliseCustomerMessage(message);
  const recentUserMessages = messages.filter((item) => item?.role === "user").slice(-4).map((item) => normaliseCustomerMessage(item.content));
  const recent = recentUserMessages.join(" ");
  if (/^(?:and )?how long(?: does (?:it|that) normally take)?\??$/.test(text)) {
    const applicationSubject = /apply|application|approval/.test(recent);
    const deliverySubject = /deliver|delivery/.test(recent);
    if (applicationSubject && deliverySubject) return "Do you mean how long application approval normally takes, or how long delivery takes?";
    if (deliverySubject) return "Do you mean how long delivery normally takes?";
    if (applicationSubject) return "Do you mean how long the application and approval process normally takes?";
...
export function isDirectDeliveryLogisticsQuestion(question) {
  const text = clean(question).toLowerCase();
  if (!/\b(deliver|delivery)\b/.test(text)) return false;
  if (/\bhow long\b.{0,40}\bdeliver(?:y|ed|ing)?\b|\bdeliver(?:y|ed|ing)?\b.{0,40}\bhow long\b/.test(text)) return false;
  if (/\b(?:before|after|on) delivery\b/.test(text) && /\binspect|inspection|check|pdi|101|warranty|fault|problem|repair|after[- ]?sales|service|servicing|mot|belt|tyre|tire|brake|paperwork|deposit|reserve|reservation|handover|happy|satisfied\b/.test(text)) return false;
  return /\bdo you (?:offer )?deliver(?:y)?\b|\bcan you deliver\b|\bcould you deliver\b|\bwill you deliver\b|\bwould you deliver\b|\bwhere (?:do|can) you deliver\b|\bdeliver to\b|\bdelivery (?:area|coverage|cost|charge|price)\b|\bfree delivery\b|\bhow much (?:is |for )?delivery\b/.test(text);
}

export function deterministicDeliveryReply(productContext, question, coverage) {
  const rent2BuyCoverage = productContext === "rent2buy" ? deterministicRent2BuyCoverageReply(coverage) : "";
  if (rent2BuyCoverage) return rent2BuyCoverage;

  if (!isDirectDeliveryLogisticsQuestion(question)) return null;
  if (productContext === "finance") {
    ...
    if (!excluded && /\b(?:before|by)\b.{0,40}\b(?:today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday|week|date|\d{1,2}(?:st|nd|rd|th)?)\b/i.test(question)) {
      return `${reply} Typical preparation and delivery takes around 7–10 working days, subject to the van’s workshop requirements, so the requested date cannot be guaranteed until approval and preparation are complete.`;
    }
    return reply;
  }
...
export function controlledProductComparisonReply(message = "") {
  const text = normaliseCustomerMessage(message);
  const personalContext = /poor credit|bad credit|credit (?:isn.t|is not|is poor)|self[- ]?employed/.test(text);
  const asksWhichIsBetter = /\bbetter\b|which (?:option|one)|right for me|suit(?:s|ed)? me/.test(text);
  const finance = "Van Finance is lender-based: the actual quote depends on your credit profile, affordability and the lender’s assessment, so the monthly figure shown on a vehicle page is not guaranteed until you apply.";
  const rent2buy = "Rent2Buy is a separate product with no credit check: you rent the van during the agreed term and can own it after completing the term and paying the £99 + VAT final purchase option; affordability, documents and the existing Rent2Buy eligibility rules still apply.";
  const logistics = "Qualifying Finance vehicle purchases include free delivery across England, Wales and Scotland, while Rent2Buy vans are collected from Southampton.";
  if (personalContext) return `${finance} ${rent2buy} ${logistics} Given what you’ve said, Rent2Buy may be worth exploring, but I can’t guarantee eligibility or make a personal financial recommendation.`;
  if (asksWhichIsBetter) return `${finance} ${rent2buy} ${logistics} Neither is automatically better: the right route depends on your circumstances, and eligibility cannot be guaranteed.`;
  return `${finance} ${rent2buy} ${logistics}`;
}

export function controlledVehicleNextStepReply(message = "", productContext = "finance") {
  const text = normaliseCustomerMessage(message);
  if (!/\bwhat happens next\b|\bnext steps?\b|\bwhat (?:do|should) i do next\b|\bthen what\b/.test(text)) return null;
  if (productContext === "rent2buy") {
    return "The next step is to use APPLY NOW on this page. The team will assess affordability and the required documents, then confirm the current requirements for your application and chosen vehicle.";
  }
  return "First, complete the Finance application for this vehicle. A lender will assess it and, if accepted, provide a personalised Finance quote for you to review; you then choose whether to proceed. If you choose to proceed, the £100 reservation deposit is taken where applicable, followed by the vehicle preparation process and confirmed delivery or collection.";
}

export function latestCustomerQuestionRequiresAnswer(message = "") {
  const text = normaliseCustomerMessage(message);
  return /\?$/.test(clean(message)) || /^(?:and )?(?:what|what's|whats|how|where|when|why|which|who|can|could|would|will|do|does|did|is|are|has|have)\b/.test(text);
}

export function deterministicFinanceDepositReply(question = "") {
  const text = normaliseCustomerMessage(question);
  if (!/\b(?:deposit|money down|upfront)\b/.test(text)) return null;
  return "Deposits may start from £99 on qualifying Finance applications and vehicles, but the amount required varies by customer, vehicle, lender and agreement, so £99 is not guaranteed for every application.";
}

export function deterministicRent2BuyEndOfAgreementReply(question = "") {
  const text = normaliseCustomerMessage(question);
  const asksEnd = /\bwhat happens (?:at|when) (?:the )?(?:agreement )?(?:end|ends)\b|\bwhat happens at the end\b|\bdo i own\b.{0,40}\b(?:at|by) the end\b|\bfinal purchase option\b|\bfinal payment\b|\bown(?:ership)?\b.{0,40}\bend\b/.test(text);
  if (!asksEnd) return null;
  return "After you successfully complete the Rent2Buy agreement and its conditions, the current final purchase option is £99 + VAT. The van remains on hire until you exercise that option; once it is paid, ownership can transfer to you. The purchase option is optional, not automatic.";
}

export function controlledBusinessRuntimeReply({ message = "", productContext = "finance", runtimeContext = {}, rememberedFacts = {} } = {}) {
  const text = normaliseCustomerMessage(message);
  const financeDeposit = productContext === "finance" ? deterministicFinanceDepositReply(message) : null;
  const rent2BuyEnd = productContext === "rent2buy" ? deterministicRent2BuyEndOfAgreementReply(message) : null;
  if (rent2BuyEnd) return rent2BuyEnd;
  if (/\bwhat (?:day is today|is today'?s date|date is it)\b|\btoday'?s date\b/.test(text)) {
    const date = clean(runtimeContext.current_date, 20);
    const day = clean(runtimeContext.current_day, 20);
    if (date && day && /^\d{4}-\d{2}-\d{2}$/.test(date)) {
      ...
      return `Today is ${day}, ${Number(dateOfMonth)} ${monthName} ${year}.`;
    }
  }
  if (/\b(?:what time are you open|when are you open|opening hours|what are your hours)\b/.test(text)) {
    return "I don’t have verified opening hours in the business information available, so I won’t guess. The team can confirm them for you.";
  }
  if (/\bwhere are (?:you|your sites) (?:located|based)\b|\bwhere (?:is|are) your (?:site|sites|office|offices)\b/.test(text)) {
    return productContext === "rent2buy"
      ? "Our sites are based in Southampton. Rent2Buy vans are collected from Southampton rather than delivered, and applicants normally need to live within 100 miles of SO40 2NN."
      : "Our sites are based in Southampton. We’re a nationwide van company and offer free delivery for qualifying Finance purchases across England, Wales and Scotland.";
  }
  if (/\b(?:deliver|delivery)\b/.test(text)) {
    const rememberedLocation = clean(rememberedFacts.location, 120);
    const usesCustomerReference = /\b(?:to me|to my (?:area|home|location|place)|where i (?:live|am)|here)\b/i.test(text);
    const coverageQuestion = usesCustomerReference && rememberedLocation ? `Can you deliver to ${rememberedLocation}?` : message;
    const coverage = productContext === "rent2buy" ? buildRent2BuyDeliveryEvidence(coverageQuestion) : buildFinanceCoverageEvidence(coverageQuestion);
    const delivery = deterministicDeliveryReply(productContext, message, coverage);
    if (delivery && productContext === "finance" && coverage?.diagnostics?.coverage_result !== "not_covered") {
      const deliveryReply = `${delivery.replace(/ Timing is confirmed once the finance is approved and the vehicle is ready\./i, "")} Our sites are based in Southampton.`;
      return [financeDeposit, deliveryReply].filter(Boolean).join(" ");
    }
    return [financeDeposit, delivery].filter(Boolean).join(" ") || null;
  }
  if (financeDeposit) return financeDeposit;
  return null;
}

export function naturalSalesReply(intent, productContext, buyingSignals, rememberedFacts = {}) {
  if (/^(?:i )?need (?:a )?van(?: quickly)?$/.test(intent.normalised_message)) {
    return { reply: `I can help with ${productContext === "finance" ? "finding the right van finance route" : "finding the right Rent2Buy van"}. What size or type of van do you need?`, insufficient_knowledge: false, human_handoff_recommended: false, recommended_action: "continue", confidence: 100, confidence_reason: "Server-side conversational response to a plain vehicle need." };
  }
