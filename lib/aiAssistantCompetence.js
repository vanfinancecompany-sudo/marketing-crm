...
const RETRIEVAL_FOCUS_TERMS = new Set([
  "insurance", "insured", "tax", "taxed", "vat", "warranty", "servicing", "service", "mileage",
...
function focusedRetrievalWords(question, messages = []) {
  ...
  return focused.length ? focused : novel.length ? novel : queryWords;
}
...
export function rankKnowledge(question, corpus = [], { messages = [], limit = 8 } = {}) {
  const queryWords = focusedRetrievalWords(question, messages);
  ...
  }).filter((source) => source.matched_terms.length > 0).sort((a, b) => b.score - a.score || a._index - b._index).slice(0, limit).map(({ _index, ...source }) => source);
}
