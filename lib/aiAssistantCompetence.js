import { buildAiPlatformPrompt } from "./businessIntelligence.js";
...
const STOP_WORDS = new Set("a an and are as at be been but by can could did do does for from get had has have how i if in into is it me my of on or our should so that the their them there they this to van vans was we what when where which who why will with would you your".split(" "));
const clean = (value) => String(value || "").trim();
const words = (value) => clean(value).toLowerCase().replace(/[^a-z0-9£]+/g, " ").split(/\s+/).filter((word) => word.length > 1 && !STOP_WORDS.has(word));
...
export function splitArticleMarkdown(article = {}) {
  ...
  const flush = () => {
    ...
    if (passage) passages.push({ type: "article", source_id: article.id, title: article.title, heading, passage, public_url: article.live_wix_url || "", product: article.primary_product || article.category || "" });
    buffer = [];
  };
...
export function buildRetrievalCorpus({ sections = [], articles = [] } = {}) {
  ...
  const articlePassages = articles.flatMap((article) => [
    ...
    ...(Array.isArray(article.faq_json) ? article.faq_json : []).map((faq) => ({ type: "article_faq", source_id: article.id, title: article.title, heading: clean(faq.question) || "Article FAQ", passage: [clean(faq.question), clean(faq.answer)].filter(Boolean).join(": "), public_url: article.live_wix_url || "", product: article.primary_product || article.category || "" })),
  ]);
  return [...brain, ...articlePassages];
}
...
export function rankKnowledge(question, corpus = [], { messages = [], limit = 8 } = {}) {
  const queryWords = words(`${messages.slice(-4).map((message) => message.content).join(" ")} ${question}`);
  const querySet = new Set(queryWords);
  const product = detectProduct(question, messages);
  ...
  }).filter((source) => source.score > 0).sort((a, b) => b.score - a.score || a._index - b._index).slice(0, limit).map(({ _index, ...source }) => source);
}

export function buildCompetencePrompt({ question, messages = [], sources = [], sections = [], settings = {} }) {
  const base = buildAiPlatformPrompt({ sections, settings, task: "customer_assistant_competence_test", module: "ai_assistant_competence_test", requestedTask: "Answer one customer question using only the retrieved evidence." });
  const evidence = sources.map((source, index) => `[S${index + 1}] ${source.type === "business_brain" || source.type === "business_faq" ? `Business Brain — ${source.title}` : `Article — ${source.title} — ${source.heading}`}\n${source.passage}`).join("\n\n");
  const history = messages.slice(-8).map((message) => `${message.role}: ${message.content}`).join("\n");
  return `${base.prompt}\n\n# Customer-assistant rules\nAnswer as the future website assistant: concise, friendly and accurate. Maximum approximately 100 words. Do not give financial or legal advice, promise acceptance, invent terms or use knowledge outside the evidence. Clearly distinguish Finance from Rent2Buy. Business Brain evidence outranks articles. If evidence is missing or conflicts, say so plainly and set the diagnostic flags. Cite only source IDs actually used.\n\n# Conversation\n${history || "No previous messages."}\n\n# Customer question\n${question}\n\n# Retrieved evidence\n${evidence || "No relevant evidence was retrieved."}`;
}
