...
import {
  ASSISTANT_PRODUCT_CONTEXTS,
  RENT2BUY_ARTICLE_CATEGORY as PRODUCT_RENT2BUY_ARTICLE_CATEGORY,
  buildAssistantProductBrain,
  permanentRent2BuyRuntimeEvidence,
  retrievalSourceAllowedForProduct,
} from "./assistantProductBrains.js";
import { buildSealedAssistantPlatformPrompt } from "./sealedAssistantPrompt.js";
...
function normaliseRetrievalText(value) {
  const protectedTax = String(value || "").replace(/\b(vehicle|road)\s+tax\b/gi, "$1 taxed");
  return normaliseCustomerMessage(protectedTax);
}
const words = (value) => {
  const tokens = normaliseRetrievalText(value).replace(/[^a-z0-9£]+/g, " ").split(/\s+/).filter((word) => word.length > 1 && !STOP_WORDS.has(word));
...
export const RENT2BUY_ARTICLE_CATEGORY = PRODUCT_RENT2BUY_ARTICLE_CATEGORY;
export const COMPETENCE_PRODUCT_CONTEXTS = ASSISTANT_PRODUCT_CONTEXTS;
...
export function filterKnowledgeForProduct(knowledge = {}, productContext, options = {}) {
  return buildAssistantProductBrain(knowledge, productContext, options);
}
...
export function buildCompetencePrompt({ question, messages = [], sources = [], sections = [], settings = {}, productContext = "finance", comparison = false, live = false }) {
  const base = buildSealedAssistantPlatformPrompt({ sections, live });
  ...
  const boundary = comparison
    ? `The selected primary context is ${productContext}. The customer explicitly requested a comparison. Discuss each product only from its own sealed evidence; never transfer a fact from one product to the other.`
  ...
  const rent2BuyRuntimeRule = (productContext === "rent2buy" || comparison)
    ? `\n\n# Non-overridable Rent2Buy brain rules\n${permanentRent2BuyRuntimeEvidence()}\nThese rules apply only to the Rent2Buy side. If an exact Rent2Buy fact is not present in approved Business Brain evidence, say the team will confirm it. Never fill the gap from an article, Finance evidence or model inference.`
    : "";
  const deterministicRule = sources.find((source) => source.type === "coverage_rule");
  const role = live
    ? `Answer as the ${productContext === "rent2buy" ? "Rent2Buy Assistant" : "Van Finance Assistant"} on the live customer website: warm, direct, concise and natural. Never call yourself Jasmine.`
    : "Answer as the future website assistant: concise, friendly and accurate.";
  return `${base.prompt}\n\n# Customer-assistant rules\n${role} Maximum approximately 100 words. Do not give financial or legal advice, promise acceptance, invent terms or use knowledge outside the evidence. ${boundary} Business Brain evidence outranks articles. Deterministic coverage evidence outranks Business Brain, articles and model inference; if S1 is a deterministic coverage rule, its calculated conclusion is non-overridable and must be used even when another source conflicts. If evidence is missing or conflicts, say so plainly and set the diagnostic flags. Cite only source IDs actually used.${rent2BuyRuntimeRule}${deterministicRule ? `\n\n# Non-overridable coverage conclusion\n${deterministicRule.passage}` : ""}\n\n# Conversation\n${history || "No previous messages."}\n\n# Customer question\n${question}\n\n# Retrieved evidence\n${evidence || "No relevant evidence was retrieved."}`;
}
