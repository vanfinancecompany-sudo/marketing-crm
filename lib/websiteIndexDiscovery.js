...
function readableLabel(value) {
  return clean(value)
    ...
    .replace(/\bRent2buy\b/g, "Rent2Buy")
    .replace(/\bEv\b/g, "EV");
}

export function normalizeNavigationLabel(value) {
  const original = stripTags(value);
  if (!original) return "";
  const withoutPrefix = original
    .replace(/^(?:view|see|browse)(?:\s+(?:our|the))?\s*(?:[:|/–—-]\s*)?/i, "")
    .trim();
  const normalized = normalizedWords(withoutPrefix).join(" ");
  if (
    normalized === "vans van finance" ||
    normalized === "van finance vans" ||
    normalized === "vans on finance"
  ) return "Vans on Finance";
  if (/^crew vans?$/.test(normalized)) return "Crew Vans";
  if (/^tippers?(?: vans?)?$/.test(normalized)) return "Tipper Vans";
  return /^[A-Z0-9\s|/&-]+$/.test(withoutPrefix)
    ? readableLabel(withoutPrefix)
    : withoutPrefix;
}
...
export function buildDiscoveryCandidateEdit(candidate = {}, changes = {}, rootUrl = DISCOVERY_ROOT_URL) {
  const next = { ...candidate, ...changes };
  const normalizedUrl = normalizeDiscoveryUrl(next.url, rootUrl);
  return {
    title: clean(next.title, 300),
    url: normalizedUrl,
    suggested_category: next.suggested_category,
    suggested_priority: Math.max(1, Math.min(5, Number(next.suggested_priority) || 3)),
    suggested_description: clean(next.suggested_description, 5000),
    suggested_keywords: normalizedList(next.suggested_keywords),
    suggested_matching_terms: normalizedList(next.suggested_matching_terms),
    suggested_customer_intent: normalizedList(next.suggested_customer_intent),
    monitor_in_ai_visibility_when_published:
      next.monitor_in_ai_visibility_when_published !== false,
    review_notes: clean(next.review_notes, 2000),
    requires_manual_mapping: !normalizedUrl,
  };
}
...
export function extractWebsitePage(html, pageUrl, rootUrl = DISCOVERY_ROOT_URL) {
  ...
  for (const match of source.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)) {
    ...
    if (!href) continue;
    const text = normalizeNavigationLabel(
      stripTags(match[2]) || attribute(match[1], "aria-label") || attribute(match[1], "title")
    );
  ...
  for (const match of source.matchAll(/<(button|div)\b([^>]*)>([\s\S]*?)<\/\1>/gi)) {
    ...
    const text = normalizeNavigationLabel(match[3]);
