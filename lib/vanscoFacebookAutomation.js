...
function pageSpecificBranchMatches(text) {
  const value = clean(text).toLowerCase();
  if (!value) return [];
  const matches = [];
  if (
    /this vehicle is situated at our vansco 333 showroom/.test(value)
    || /vansco 333\s*[-–]\s*address:/.test(value)
  ) matches.push("vansco333");
  if (
    /vansco limited\s*[–-]\s*new forest/.test(value)
    || /this vehicle is situated at our vansco new forest/.test(value)
  ) matches.push("newForest");
  if (
    /vansco limited\s*[–-]\s*southampton airport/.test(value)
    || /this vehicle is situated at our vansco southampton airport/.test(value)
  ) matches.push("southamptonAirport");
  return [...new Set(matches)];
}
...
export function resolveVanscoBranch({ description = "", vehicleUrl = "", city = "", pageText = "" } = {}) {
  ...
  const pageMatches = pageSpecificBranchMatches(pageText);
  if (!descriptionMatches.length && !fromUrl && pageMatches.length === 1) {
    return { branchKey: pageMatches[0], source: "page", conflict: false };
  }

  if (!descriptionMatches.length && !fromUrl && !pageMatches.length && clean(city).toLowerCase() === "cadnam") {
    return { branchKey: "newForest", source: "city", conflict: false };
  }
  return {
    branchKey: "",
    source: "none",
    conflict: descriptionMatches.length > 1 || pageMatches.length > 1,
  };
}

export function vatLabelFromText(value) {
  const text = clean(value).toUpperCase();
  if (/\bNO\s+VAT\b|\bVAT\s+FREE\b|\bMARGIN\s+SCHEME\b/.test(text)) return "NO VAT";
  if (/\+\s*VAT\b|\bPLUS\s+VAT\b|\bEX(?:CLUDING)?\s+VAT\b/.test(text)) return "+ VAT";
  if (
    /\bINC(?:LUDING)?\s+VAT\b/
    .test(text)
    || /\bVAT\s+(?:INCLUDED|INCLUSIVE)\b/.test(text)
    || /\bINCLUDES\s+VAT\b/.test(text)
  ) return "INC VAT";
  return "";
}

export function vatLabelFromStatus(value) {
  const status = clean(value).toLowerCase();
  if (status === "plus_vat") return "+ VAT";
  if (status === "no_vat") return "NO VAT";
  if (status === "vat_included" || status === "inc_vat") return "INC VAT";
  return "";
}

export function hasExplicitVanscoVatLabel(value) {
  return Boolean(vatLabelFromText(value));
}

export function isVanscoCar(vehicle = {}) {
  const classification = classifyDealerKitVehicle({
    ...vehicle,
    bodyType: [vehicle?.bodyType, vehicle?.bodyStyle, vehicle?.vehicleUrl]
      .map((value) => clean(value))
      .filter(Boolean)
      .join(" "),
  });
  return classification.segment === "car";
}

export function vanscoAdvertVatLabel(vehicle = {}) {
  if (isVanscoCar(vehicle)) return "";
  return clean(vehicle?.vatLabel);
}

export function isVanscoVatResolved(vehicle = {}) {
  return isVanscoCar(vehicle) || Boolean(clean(vehicle?.vatLabel));
}

export function vanscoFacebookImageUrls(vehicle = {}) {
  const candidates = [
...
function metaImageUrls(row = {}) {
  return Object.entries(row)
    .map(([key, value]) => {
      const match = String(key || "").match(/^image\[(\d+)\]\.url$/i);
      return match ? { index: Number(match[1]), url: normalizeUrl(value) } : null;
    })
...
export function normalizeVanscoMetaRow(row) {
  ...
  const imageUrls = metaImageUrls(row);
  const imageUrl = imageUrls[0] || normalizeUrl(row?.["image[0].url"]);
  return {
...
export function buildVanscoFacebookCaption(vehicle) {
  ...
  const mileage = mileageValue
    ...
    : "";
  const vat = vanscoAdvertVatLabel(vehicle);
  if (!isVanscoCar(vehicle) && !vat) throw new Error("Vansco VAT status is unresolved.");
...
export function vanscoNextDateKey(dateKey) {
  const match = clean(dateKey).match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) throw new Error("Vansco schedule date must be YYYY-MM-DD.");
...
export function vanscoDailySlots(dateKey, requestedLimit = VANSCO_FACEBOOK_MAX_POSTS_PER_DAY) {
  ...
  const lastSlotMinutes = 21 * 60;
  const spanMinutes = lastSlotMinutes - firstSlotMinutes;

  return Array.from({ length: limit }, (_, index) => {
    const localMinutes = limit === 1
      ? firstSlotMinutes
      : Math.round(firstSlotMinutes + (spanMinutes * index) / (limit - 1));
    return {
      index,
      localMinutes,
...
export function chooseVanscoCandidate({ vehicles = [], lastPostedByKey = {}, excludedUrls = [] } = {}) {
  ...
  return [...vehicles]
    ...
    .sort((first, second) => {
      const firstKnown = first.branchKey && !first.branchConflict ? 0 : 1;
      const secondKnown = second.branchKey && !second.branchConflict ? 0 : 1;
      if (firstKnown !== secondKnown) return firstKnown - secondKnown;
