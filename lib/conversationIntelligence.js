...
export function controlledVehiclePreferenceReply({ message = "", facts = {}, nextQuestion = "" } = {}) {
  ...
  if (!preference.vehicle_type && !preference.vehicle_interest) return null;
  ...
  if (size) {
    ...
    return `A ${size} — understood. ${progression}`;
  }
...
function metadataForUpdates(updates, message, contextualUpdates, userIndex, messageId = null) {
  ...
  return Object.fromEntries(Object.keys(updates).map((field) => {
    ...
    else if (["vehicle_type", "vehicle_interest"].includes(field) && !/\b(?:i want|i need|i m looking for|im looking for|i am looking for|i m interested in|im interested in|i am interested in|my choice is|maybe|probably|ideally|prefer|preferred)\b/.test(text)) provenance = "inferred";
