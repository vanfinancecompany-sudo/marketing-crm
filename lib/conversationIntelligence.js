...
export function normaliseCustomerMessage(message) {
  return clean(message).toLowerCase()
    ...
    .replace(/\bpsrinter\b|\bsprnter\b/g, "sprinter")
    .replace(/\bnay\b/g, "any")
...
const VEHICLE_SIZES = Object.freeze([
  { key: "small", code: "SWB", value: "Short wheelbase", reply: "short-wheelbase van", pattern: /\b(?:swb|short[\s-]*wheel\s*base|small(?:\s+vans?)?)\b/ },
  { key: "medium", code: "MWB", value: "Medium wheelbase", reply: "medium-wheelbase van", pattern: /\b(?:mwb|medium[\s-]*wheel\s*base|medium(?:\s+vans?)?)\b/ },
  { key: "large", code: "LWB", value: "Long wheelbase", reply: "long-wheelbase van", pattern: /\b(?:lwb|long[\s-]*wheel\s*base|large(?:\s+vans?)?)\b/ },
]);

const VEHICLE_BODY_TYPES = Object.freeze([
  ["Crew van", /\b(?:crew(?:\s+van)?|double\s+cab)\b/],
  ["Pickup", /\b(?:pick[\s-]?up|4x4)\b/],
  ["Tipper", /\btippers?\b/],
  ["Luton", /\blutons?\b/],
  ["Dropside", /\bdrop[\s-]?sides?\b/],
  ["Low loader", /\blow[\s-]?loaders?\b/],
  ["Electric van", /\b(?:electric|ev)\s+vans?\b/],
  ["Automatic van", /\bautomatic\s+vans?\b/],
]);

const VEHICLE_MODELS = Object.freeze([
  ["Transit Custom", /\b(?:ford\s+)?(?:transit\s+)?customs?\b/],
  ["Transit", /\b(?:ford\s+)?transits?\b(?!\s+customs?)/],
  ["Sprinter", /\b(?:mercedes(?:\s+benz)?\s+)?sprinters?\b/],
  ["Crafter", /\b(?:volkswagen|vw)?\s*crafters?\b/],
  ["Vivaro", /\b(?:vauxhall\s+)?vivaros?\b/],
  ["Trafic", /\b(?:renault\s+)?trafics?\b/],
  ["Expert", /\b(?:peugeot\s+)?experts?\b/],
  ["Dispatch", /\b(?:citroen\s+)?dispatch(?:es)?\b/],
  ["Primastar", /\b(?:nissan\s+)?primastars?\b/],
  ["Proace", /\b(?:toyota\s+)?proaces?\b/],
  ["Boxer", /\b(?:peugeot\s+)?boxers?\b/],
  ["Relay", /\b(?:citroen\s+)?relays?\b/],
  ["Master", /\b(?:renault\s+)?masters?\b/],
  ["Movano", /\b(?:vauxhall\s+)?movanos?\b/],
  ["Ducato", /\b(?:fiat\s+)?ducatos?\b/],
  ["Partner", /\b(?:peugeot\s+)?partners?\b/],
  ["Berlingo", /\b(?:citroen\s+)?berlingos?\b/],
  ["Transit Connect", /\b(?:ford\s+)?(?:transit\s+)?connects?\b/],
  ["Caddy", /\b(?:volkswagen|vw)?\s*cadd(?:y|ies)\b/],
  ["Combo", /\b(?:vauxhall\s+)?combos?\b/],
]);

function joinedVehicleModels(models = []) {
  if (models.length < 2) return models[0] || "";
  return models.length === 2 ? `${models[0]} or ${models[1]}` : `${models.slice(0, -1).join(", ")} or ${models.at(-1)}`;
}

export function extractVehiclePreference(message = "") {
  const text = normaliseCustomerMessage(message);
  const size = VEHICLE_SIZES.find((item) => item.pattern.test(text)) || null;
  const bodyType = VEHICLE_BODY_TYPES.find(([, pattern]) => pattern.test(text))?.[0] || "";
  const transmission = /\b(?:automatic|auto)\b/.test(text) ? "automatic" : /\bmanual\b/.test(text) ? "manual" : null;
  const models = VEHICLE_MODELS.filter(([, pattern]) => pattern.test(text)).map(([model]) => model);
  if (models.includes("Transit Custom")) {
    const transitIndex = models.indexOf("Transit");
    if (transitIndex >= 0) models.splice(transitIndex, 1);
  }
  if (models.includes("Transit Connect")) {
    const transitIndex = models.indexOf("Transit");
    if (transitIndex >= 0) models.splice(transitIndex, 1);
  }
  const vehicleType = size?.value || bodyType;
  const vehicleInterest = joinedVehicleModels(models);
  return {
    size: size?.key || null,
    size_code: size?.code || null,
    size_reply: size?.reply || null,
...
function legacyVehicleInterest(preference, text) {
  if (preference.vehicle_interest) return preference.vehicle_interest;
  if (!preference.vehicle_type) return "";
  if (preference.size_code) return titleCase(new RegExp(`\\b${preference.size_code.toLowerCase()}\\b`).test(text) ? preference.size_code.toLowerCase() : preference.size);
  return titleCase(preference.vehicle_type);
}

function displayVehicleModel(model) {
  if (model === "Transit") return "Ford Transit";
  if (model === "Sprinter") return "Mercedes Sprinter";
  return model;
}

export function controlledVehiclePreferenceReply({ message = "", facts = {}, nextQuestion = "" } = {}) {
  const text = normaliseCustomerMessage(message);
  if (!text || /\?$/.test(clean(message)) || /^(?:what|which|where|when|why|how|can|could|would|will|do|does|did|is|are|has|have|show|any|got)\b/.test(text)) return null;
  const preference = extractVehiclePreference(message);
  if (!preference.vehicle_type && !preference.vehicle_interest && !preference.transmission) return null;
  const rememberedType = clean(facts.vehicle_type, 160);
  const rememberedTransmission = clean(facts.transmission, 40).toLowerCase();
  const transmission = preference.transmission || (["automatic", "manual"].includes(rememberedTransmission) ? rememberedTransmission : "");
  const size = preference.size_reply || (/short wheelbase/i.test(rememberedType) ? "short-wheelbase van" : /medium wheelbase/i.test(rememberedType) ? "medium-wheelbase van" : /long wheelbase/i.test(rememberedType) ? "long-wheelbase van" : "");
  const models = preference.models.length ? preference.models.map(displayVehicleModel) : clean(facts.vehicle_interest, 300).split(/\s+or\s+/i).filter((value) => !/^(?:swb|mwb|lwb|small|medium|large|short wheelbase|medium wheelbase|long wheelbase)$/i.test(value)).map(displayVehicleModel);
  const modelDescription = joinedVehicleModels(models);
  if (size && transmission && modelDescription) return `Got it — you’re looking for a ${size} with an ${transmission} gearbox, ideally ${modelDescription}.`;
  if (size && modelDescription) return `Got it — you’re looking for a ${size}, ideally ${modelDescription}.`;
  if (size) {
    const progression = facts.budget_monthly_gbp
      ? "Do you have a preferred make or model?"
      : clean(nextQuestion, 300) || "Do you have a preferred make or model?";
    return `A ${size}${transmission ? ` with an ${transmission} gearbox` : ""} — understood. ${progression}`;
  }
  if (modelDescription) return `Got it — you’re interested in ${/\bor\b/.test(modelDescription) ? modelDescription : `a ${modelDescription}`}. Do you have a preferred size or wheelbase?`;
  return null;
}
...
export function extractConversationFacts(message, previousFacts = {}) {
  ...
  const vehicle = extractVehiclePreference(text);
  if (vehicle.vehicle_type) facts.vehicle_type = vehicle.vehicle_type;
  if (vehicle.transmission) facts.transmission = vehicle.transmission;
  const vehicleInterest = legacyVehicleInterest(vehicle, text);
  if (vehicleInterest) facts.vehicle_interest = vehicleInterest;
...
function factSpecificity(field, value) {
  ...
  if (field === "vehicle_interest") {
    ...
    if (/\s+or\s+|,/.test(text)) return 3;
    if (!/^(?:swb|mwb|lwb|small|medium|large|short wheelbase|medium wheelbase|long wheelbase)$/i.test(text)) return 2;
  }
  return 1;
}
...
function metadataForUpdates(updates, message, contextualUpdates, userIndex, messageId = null) {
  ...
  return Object.fromEntries(Object.keys(updates).map((field) => {
    ...
    else if (["vehicle_type", "vehicle_interest", "transmission"].includes(field) && !/\b(?:i want|i need|i m looking for|im looking for|i am looking for|i m interested in|im interested in|i am interested in|my choice is|maybe|probably|ideally|prefer|preferred)\b/.test(text)) provenance = "inferred";
