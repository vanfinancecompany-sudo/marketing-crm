...
export function normaliseCustomerMessage(message) {
  return clean(message).toLowerCase()
    ...
    .replace(/\bself[\s-]*(?:emp|emplyd)\b/g, "self employed")
...
function businessSubIntents(text) {
  ...
  add("vat_pricing", /\bvat\b|value added tax|prices?.{0,20}(?:included|inclusive|plus|excluding|exclusive)|payments?.{0,20}(?:included|inclusive|plus|excluding|exclusive)/);
...
export function extractConversationFacts(message, previousFacts = {}) {
  ...
  if (budget) { facts.budget = `£${budget[1]}`; facts.budget_monthly_gbp = Number(budget[1].replaceAll(",", "")); }
  ...
  const deposit = text.match(/(?:deposit|money down|upfront).{0,20}£\s?([\d,]+)|£\s?([\d,]+).{0,20}(?:deposit|money down|upfront)/);
  if (deposit) { facts.deposit = `£${deposit[1] || deposit[2]}`; facts.deposit_budget_gbp = Number((deposit[1] || deposit[2]).replaceAll(",", "")); }
  if (/deliver|delivery/.test(text)) facts.delivery_interest = true;
  if (/collect|collection/.test(text)) facts.collection_interest = true;
  if (/poor credit|bad credit|credit problems?|ccj|declined|refused|turned down/.test(text)) { facts.main_concern = "credit history"; facts.credit_concern = /refused|declined|turned down/.test(text) ? "previous refusal" : "credit history"; }
...
export function buildConversationMemory(messages = [], suppliedMemory = {}) {
  ...
  const factMetadata = {};
  const corrections = [];
  for (const [index, item] of messages.filter((message) => message?.role === "user").entries()) {
    ...
    for (const [field, value] of Object.entries(updates)) {
      ...
      facts[field] = value;
      factMetadata[field] = { confidence: 0.95, source_message_id: item.id || `user-${index + 1}` };
    }
  }
  const supplied = suppliedMemory && typeof suppliedMemory === "object" && !Array.isArray(suppliedMemory) ? suppliedMemory : {};
  return { remembered_facts: { ...supplied, ...facts }, fact_metadata: factMetadata, corrections };
}
