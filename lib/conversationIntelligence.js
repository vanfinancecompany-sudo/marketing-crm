...
export function buildConversationMemory(messages = [], suppliedMemory = {}) {
  ...
  for (const item of Array.isArray(messages) ? messages : []) {
    ...
    userIndex += 1;
    const updates = {
      ...extractConversationFacts(item.content, facts),
      ...contextualFollowUpFacts(item.content, previousAssistant, facts),
    };
    for (const [field, value] of Object.entries(updates)) {
      ...
      facts[field] = value;
      factMetadata[field] = { confidence: 0.95, source_message_id: item.id || `user-${userIndex}` };
    }
    previousAssistant = "";
  }
