...
export const FACEBOOK_STORY_TARGET_PER_DAY = 3;
...
export const DEFAULT_BUFFER_AUTOMATION_CONFIG = Object.freeze({
  ...
  vanFinanceInstagramEnabled: true,
  vanFinanceInstagramChannelId: "",
  instagramDelayMinutes: 10,
  updatedAt: null,
});
...
export function normalizeBufferAutomationConfig(value = {}) {
  ...
  return {
    ...
    vanFinanceInstagramEnabled: value?.vanFinanceInstagramEnabled === undefined
      ? defaults.vanFinanceInstagramEnabled
      : Boolean(value.vanFinanceInstagramEnabled),
    vanFinanceInstagramChannelId: cleanChannelId(value?.vanFinanceInstagramChannelId),
    instagramDelayMinutes: boundedInt(value?.instagramDelayMinutes, defaults.instagramDelayMinutes, 0, 60),
...
export function facebookPostTargetForProduct(config, productKey) {
...
export function facebookStoryTargetForProduct(config, productKey) {
  return Math.min(FACEBOOK_STORY_TARGET_PER_DAY, facebookPostTargetForProduct(config, productKey));
}

function countsForProduct(config, productKey) {
  const safe = normalizeBufferAutomationConfig(config);
  const facebookTotal = facebookPostTargetForProduct(safe, productKey);
  const storyTarget = facebookStoryTargetForProduct(safe, productKey);
  const image = Math.max(0, facebookTotal - storyTarget);
  const video = productKey === "rent2buy" ? safe.rent2buyReelsPerDay : safe.vanFinanceReelsPerDay;
  return { image, video };
}
