...
const TABLE_TYPOGRAPHY_CSS = `.kh-responsive-table,.kh-responsive-table *,.kh-table-fallback,.kh-table-fallback *{font-family:inherit;font-size:inherit;line-height:inherit;color:inherit}.kh-responsive-table th,.kh-responsive-table h4,.kh-responsive-table strong,.kh-table-fallback h4,.kh-table-fallback strong{font-weight:600}.kh-responsive-table td,.kh-responsive-table td span,.kh-responsive-table .kh-table-card__field span,.kh-table-fallback .kh-table-card__field span{font-weight:400}`;
...
export function renderMalformedTableHtml(table) {
  ...
  return `<div class="kh-table-fallback"><style>${TABLE_TYPOGRAPHY_CSS}.kh-table-fallback{max-width:100%;margin:1.25rem 0}.kh-table-fallback .kh-table-card{border:1px solid #d7dce2;border-radius:8px;padding:1rem;margin:.75rem 0;overflow-wrap:anywhere}.kh-table-fallback .kh-table-card h4{margin:0 0 .75rem}.kh-table-fallback .kh-table-card__field{display:grid;grid-template-columns:minmax(7rem,40%) 1fr;gap:.75rem;padding:.5rem 0;border-top:1px solid #e8ebee}.kh-table-fallback .kh-table-card__field span{min-width:0;overflow-wrap:anywhere}@media(max-width:700px){.kh-table-fallback .kh-table-card__field{grid-template-columns:1fr}}</style>${rows.map((cells, index) => `<section class="kh-table-card"><h4>${inlineHtml(cells[0] || `Row ${index + 1}`)}</h4>${cells.slice(1).map((cell, offset) => `<div class="kh-table-card__field"><strong>Column ${offset + 2}</strong><span>${inlineHtml(cell)}</span></div>`).join("")}</section>`).join("")}</div>`;
}
