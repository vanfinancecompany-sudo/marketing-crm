...
export function parseMarkdownTable(lines, startIndex = 0) {
  ...
  if (!rows.length) {
    return {
      ...
      startIndex,
      endIndex: cursor,
      rawLines: lines.slice(startIndex, cursor),
      warning: "Markdown table has a header and separator but no readable body rows.",
    };
  }
...
function malformedTableCandidate(lines, startIndex) {
  ...
  return {
    ...
    endIndex: cursor,
    rawLines: collected,
    warning: "Malformed Markdown table was converted to a readable stacked list.",
  };
}
...
export function splitMarkdownTableSegments(markdown) {
  ...
  while (index < lines.length) {
    ...
    flushText();
    if (candidate.valid) {
      segments.push({ type: "table", table: candidate });
    } else {
