...
export const WIX_ARTICLE_BODY_TYPOGRAPHY = Object.freeze({
...
export function wixArticleBodyTypographyCss() {
  const typography = WIX_ARTICLE_BODY_TYPOGRAPHY;
  return `font-family:${typography.fontFamily};font-size:${typography.fontSize};line-height:${typography.lineHeight};color:${typography.color}`;
}
...
export function renderResponsiveTableHtml(table) {
  ...
  const bodyTypography = wixArticleBodyTypographyCss();
  return `<div class="kh-responsive-table"><style>
.kh-responsive-table,.kh-responsive-table table,.kh-responsive-table th,.kh-responsive-table td,.kh-responsive-table__mobile,.kh-table-card,.kh-table-card h4,.kh-table-card__field,.kh-table-card__field strong,.kh-table-card__field span,.kh-table-fallback{${bodyTypography}}
.kh-responsive-table{max-width:100%;margin:1.25rem 0}.kh-responsive-table__scroll{overflow-x:auto;-webkit-overflow-scrolling:touch}.kh-responsive-table table{width:100%;border-collapse:collapse;table-layout:auto}.kh-responsive-table th,.kh-responsive-table td{border:1px solid #d7dce2;padding:.75rem;text-align:left;vertical-align:top;overflow-wrap:anywhere;font-weight:400}.kh-responsive-table thead th,.kh-responsive-table tbody th{font-weight:600}.kh-responsive-table thead th{background:#f3f5f7}.kh-responsive-table__mobile{display:none}.kh-table-card{border:1px solid #d7dce2;border-radius:8px;padding:1rem;margin:.75rem 0;overflow-wrap:anywhere}.kh-table-card h4{margin:0 0 .75rem;font-weight:600}.kh-table-card__field{display:grid;grid-template-columns:minmax(7rem,40%) 1fr;gap:.75rem;padding:.5rem 0;border-top:1px solid #e8ebee}.kh-table-card__field strong{font-weight:600}.kh-table-card__field span{min-width:0;overflow-wrap:anywhere;font-weight:400}@media(max-width:700px){.kh-responsive-table__scroll{display:none}.kh-responsive-table__mobile{display:block}.kh-table-card__field{grid-template-columns:1fr}.kh-table-card__field strong{margin-bottom:.15rem}}
...
export function renderMalformedTableHtml(table) {
  ...
  const bodyTypography = wixArticleBodyTypographyCss();
  return `<div class="kh-table-fallback" style="${bodyTypography}">${rows.map((cells, index) => `<section class="kh-table-card"><h4>${inlineHtml(cells[0] || `Row ${index + 1}`)}</h4>${cells.slice(1).map((cell, offset) => `<div class="kh-table-card__field"><strong>Column ${offset + 2}</strong><span>${inlineHtml(cell)}</span></div>`).join("")}</section>`).join("")}</div>`;
}
...
function renderBasicMarkdownHtml(markdown) {
  ...
  return safe.split(/\n{2,}/).map((block) => {
    ...
    return `<p style="${wixArticleBodyTypographyCss()}">${lines.map(inlineHtml).join("<br />")}</p>`;
  }).join("\n");
}
