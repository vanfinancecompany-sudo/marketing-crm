...
export const WIX_INACTIVE_STATUS = "unpublished";
...
export function isWixKnowledgeManagedArticle(article = {}, collectionId = "") {
  ...
  return Boolean(
    clean(article.wix_item_id) &&
      ["live", "synced", WIX_INACTIVE_STATUS].includes(clean(article.wix_sync_status)) &&
...
export function wasInactiveWixArticle(article = {}) {
  return Boolean(
    article.is_active === false ||
      ...
      clean(article.wix_publication_status) === WIX_INACTIVE_STATUS ||
      clean(article.wix_sync_status) === WIX_INACTIVE_STATUS,
  );
}
