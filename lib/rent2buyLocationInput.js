...
const BLOCKED_PLACE_TOKENS = new Set([
  ...
  "automatic", "manual", "mileage", "engine", "colour", "color", "yes", "no", "please", "thanks", "thank", "cheers", "hello", "hi", "bye", "goodbye", "call",
  "what", "whats", "why", "how", "when", "where", "who", "which", "looking", "look", "just", "maybe", "sure", "okay", "ok",
  ...
  "the", "of", "then", "still", "choosing", "size", "expanding", "fleet", "thats", "everything", "exactly", "alright", "mate",
  "sound", "youre", "funny", "waste", "time", "lost", "confidence", "brilliant", "perfect", "deal",
]);
...
function extractCoverageContextPlace(question) {
  const text = clean(question, 180).replace(/[.,;:!]+$/g, "").trim();
  const match = text.match(/^(?:coverage|cover(?:ed)?|available)\s+(?:for|in|near|to)\s+(.+)$/i);
  if (!match?.[1]) return null;
  const place = extractSafeStandalonePlace(match[1]);
  return place ? { ...place, input_kind: "coverage_context_place" } : null;
}
...
export function detectRent2BuyLocationInput(question, previousAssistant = "") {
  ...
  const contextualPlace = extractCoverageContextPlace(question);
  const place = contextualPlace || extractSafeStandalonePlace(question);
  if (!place) return null;

  return {
    ...place,
    inferred_from: contextualPlace
      ? "coverage_context"
      : LOCATION_PROMPT.test(clean(previousAssistant, 2000))
