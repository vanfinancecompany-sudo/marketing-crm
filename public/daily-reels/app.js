...
function renderPayload(candidate) {
  ...
  return {
    ...
    frameSpecs: Array.from({ length: FRAME_COUNT }, () => ({})),
...
async function renderCandidate(candidate) {
  const response = await fetch("/api/youtube-mp4-render", {
    ...
    body: JSON.stringify(renderPayload(candidate)),
  });
  const result = await response.json().catch(() => ({}));
...
async function prepareToday() {
  ...
  try {
    ...
    for (let index = 0; index < queue.length; index += 1) {
      ...
      setStatus(`${label} ${index + 1}/${queue.length}: rendering ${reg} in TikTok Punch...`, "working");
      try {
        const rendered = await renderCandidate(candidate);
    ...
    if (failures.length) {
      ...
      setStatus(`${completed} reel${completed === 1 ? "" : "s"} prepared. Today's tray is ready.`);
    }
  } catch (error) {
    setProgress(0, 0);
