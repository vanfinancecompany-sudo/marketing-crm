...
async function ready(productKey, dateKey) {
  ...
  const result = await client.from("marketing_daily_activity_events")
    .select("metadata").eq("activity_date", dateKey).eq("activity_type", type).eq("source", "youtube_daily_batch").limit(100);
  ...
  for (const row of active.slice(0, 3)) {
    try {
      const response = await fetch(String(row.metadata.download_url), { method: "HEAD", redirect: "follow" });
      media.push({ status: response.status, ok: response.ok, imageCount: Number(row?.metadata?.image_count || 0), sizeBytes: Number(row?.metadata?.size_bytes || 0) });
    } catch (error) {
  ...
  try {
    ...
    for (const [key, channelId] of [["vanFinance", BUFFER_FACEBOOK_CHANNELS["Van Finance Facebook"]], ["rent2buy", BUFFER_FACEBOOK_CHANNELS["Rent2Buy Facebook"]]]) {
      ...
      out[key] = {
        ...
        sentTodayVideo: rows.filter((post) => String(post?.status || "").toLowerCase() === "sent" && (post?.assets || []).some((asset) => /^video\//i.test(String(asset?.mimeType || "")))).length,
      };
    }
    return { ok: true, ...out };
