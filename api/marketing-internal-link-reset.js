...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!suggestions.length) return response.status(200).json({ ok: true, reset_count: 0, suggestions: [] });
    ...
    return response.status(200).json({ ok: true, reset_count: updated.data?.length || suggestions.length, suggestions: updated.data || [] });
  } catch (error) {
    console.error("INTERNAL LINK RESET ERROR", { status: error.status || 500, message: clean(error.message, 500) });
    return response.status(error.status || 500).json({ ok: false, message: error.message || "Accepted links could not be reset." });
  }
}
