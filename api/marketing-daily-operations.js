...
function assertDateKey(value, fallback = londonDateKey()) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) {
    throw new ApiError(400, "Invalid UK activity date.");
  }
  return dateKey;
}
...
function addDays(dateKey, amount) {
  ...
  return new Date(Date.UTC(year, month - 1, day + amount, 12, 0, 0))
    .toISOString()
    .slice(0, 10);
}
...
function enumerateDateKeys(start, end, maxDays = 366) {
  ...
  if (!values.length || current <= end) {
    throw new ApiError(400, `Date range must be ${maxDays} days or fewer.`);
  }
  return values;
}
...
async function buildDays(supabase, startDate, endDate) {
  ...
  return dateKeys.map((dateKey) => {
    ...
    const targets = resolveTargetsForDate({
      dateKey,
      weekday: londonWeekday(noon),
      schedules,
      overrides,
    });
...
function currentScheduleRows(schedules, dateKey) {
  return Array.from({ length: 7 }, (_, weekday) => {
    const row = schedules
      .filter(
        (item) =>
          Number(item.weekday) === weekday && item.effective_from <= dateKey,
      )
      .sort((a, b) =>
    ...
    return {
      weekday,
      ...normalizeTargets(row || DEFAULT_DAILY_TARGETS),
...
async function overview(supabase, dateKey) {
  ...
  return {
    day,
    schedule: currentScheduleRows(configuration.schedules, dateKey),
    override:
      configuration.overrides.find((row) => row.activity_date === dateKey) ||
      null,
  };
}

async function postingHistory(supabase, body) {
  const days = Math.max(1, Math.min(365, Number(body.days || 180)));
  const endDate = londonDateKey();
  const startDate = addDays(endDate, -(days - 1));
  const result = await supabase
    .from("marketing_daily_activity_events")
    .select(
      "id,activity_date,activity_type,source,source_id,metadata,occurred_at",
    )
    .in("activity_type", SOCIAL_ACTIVITY_TYPES)
    .eq("source", "posting_desk")
    .gte("activity_date", startDate)
    .lte("activity_date", endDate)
    .order("occurred_at", { ascending: false })
    .limit(2500);
...
async function saveSchedule(supabase, body) {
  ...
  if (!Array.isArray(body.schedule) || body.schedule.length !== 7) {
    throw new ApiError(400, "Provide targets for all seven weekdays.");
  }
  const rows = body.schedule.map((targets, weekday) => ({
  ...
  const result = await supabase
    .from("marketing_daily_target_schedules")
    .upsert(rows, { onConflict: "effective_from,weekday" });
  if (result.error) throw result.error;
...
async function recordActivity(supabase, body) {
  ...
  if (!EVENT_ACTIVITY_TYPES.includes(activityType)) {
    throw new ApiError(400, "Unsupported daily activity type.");
  }
  const activityDate = assertDateKey(body.activity_date);
  ...
  if (sourceId) {
    ...
    result = existing.data
      ? { error: null }
      : await supabase.from("marketing_daily_activity_events").insert(payload);
  } else {
...
async function undoManualActivity(supabase, body) {
  const activityType = String(body.activity_type || "");
  if (!SOCIAL_ACTIVITY_TYPES.includes(activityType)) {
    throw new ApiError(400, "Unsupported daily activity type.");
  }
  const activityDate = assertDateKey(body.activity_date);
  ...
  try {
    ...
    if (action === "overview") {
      return response.status(200).json({
        ok: true,
        ...(await overview(supabase, assertDateKey(body.activity_date))),
      });
    }
    if (action === "totals") {
      ...
      return response.status(200).json({
        ...
        end_date: endDate,
        days,
        totals: aggregatePeriod(days),
      });
    }
    if (action === "postingHistory") {
      return response.status(200).json({
        ok: true,
        ...(await postingHistory(supabase, body)),
      });
    }
    if (action === "saveSchedule") {
      return response
        .status(200)
        .json({ ok: true, ...(await saveSchedule(supabase, body)) });
    }
    if (action === "resetDefaults") {
      return response.status(200).json({
        ...
        ...(await saveSchedule(supabase, {
          ...
          schedule: Array.from({ length: 7 }, () => DEFAULT_DAILY_TARGETS),
        })),
      });
    }
    if (action === "saveOverride") {
      return response
        .status(200)
        .json({ ok: true, ...(await saveOverride(supabase, body)) });
    }
    if (action === "recordActivity") {
      return response
        .status(200)
        .json({ ok: true, ...(await recordActivity(supabase, body)) });
    }
    if (action === "undoManualActivity") {
      return response.status(200).json({
        ok: true,
        ...(await undoManualActivity(supabase, body)),
      });
    }
    ...
    response.status(error.status || 500).json({
      ok: false,
      message: error.message || "Daily operations request failed.",
    });
  }
}
