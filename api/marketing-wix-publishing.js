...
import { WixPublishingError, createOrUpdateWixDraft, wixPublishingConfiguration } from "../lib/wixPublishing.js";
...
async function recordSyncEvent(supabase,articleId,result){
  const warnings=result.tableConversionWarnings||[];
  const event=await supabase.from("knowledge_editorial_events").insert({
    event_type:"system",article_id:articleId,
    ...
    details:{action:result.operation==="created"?"wix_draft_created":"wix_draft_updated",wix_item_id:result.itemId,wix_collection_id:result.collectionId,wix_sync_status:result.syncStatus,payload_version:result.payloadVersion,content_field_type:result.contentFieldType,recovered_missing_item:result.recoveredMissingItem,replaced_wix_item_id:result.replacedItemId,table_conversion_warnings:warnings,automatic_publication:false},
  });
...
export async function publishKnowledgeArticleToWix({supabase,articleId,environment=process.env,fetchImpl=fetch}){
  ...
  const configuration=wixPublishingConfiguration(environment);const now=new Date().toISOString();
  ...
  try{
    const result=await createOrUpdateWixDraft({article,suggestions,configuration,fetchImpl});const syncedAt=new Date().toISOString();
    ...
    return{article:savedArticle,wix:{operation:result.operation,item_id:result.itemId,collection_id:result.collectionId,sync_status:result.syncStatus,synced_at:syncedAt,dashboard_url:result.dashboardUrl,content_status:"Draft",content_field_type:result.contentFieldType,recovered_missing_item:result.recoveredMissingItem,replaced_item_id:result.replacedItemId,table_conversion_warnings:result.tableConversionWarnings||[],published:false}};
  }catch(error){
    const type=error.type||"api";const message=clean(error.message,1000)||"Wix draft creation failed.";const failedAt=new Date().toISOString();const clearStoredItemId=error.details?.clear_stored_item_id===true;
...
export default async function handler(request,response){
  ...
  let body={};
  try{body=parseBody(request);if(body.action!=="createOrUpdateDraft")throw new ApiError(400,"Unsupported Wix publishing action.","validation");const result=await publishKnowledgeArticleToWix({supabase:getSupabase(),articleId:body.article_id});return response.status(200).json({ok:true,...result});}
  catch(error){const type=error.type||(error instanceof WixPublishingError?error.type:"api");console.error("KNOWLEDGE WIX PUBLISHING ERROR",{action:clean(body.action,100),article_id:clean(body.article_id,100),type,status:error.details?.wix_status||error.status||500,message:clean(error.message,500)});return response.status(error.status||500).json({ok:false,error_type:type,message:error.status||error instanceof WixPublishingError?error.message:"Wix draft creation failed."});}
}
