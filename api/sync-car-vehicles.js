...
function mapCarRow(row) {
  ...
  return {
    ...
    payload: {
      ...
      price: clean(row?.price || row?.cashPrice || row?.cash_price || row?.vehiclePrice || row?.vehicle_price || row?.priceText || row?.price_text || ""),
      salePrice: clean(row?.salePrice || row?.saleprice || row?.sale_price || row?.monthly || row?.financeMonthly || row?.finance_monthly || row?.monthlyPrice || row?.monthly_price || ""),
