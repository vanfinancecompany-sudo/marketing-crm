...
test("site-wide request upgrades an identity-only session and answers Tell me about this van from the resolved CMS profile", async () => {
  ...
  assert.match(response.payload.reply, /CK24 NRO/i);
  assert.match(response.payload.reply, /27,000 miles/i);
  assert.match(response.payload.reply, /diesel engine/i);
  assert.match(response.payload.reply, /manual gearbox/i);
  assert.match(response.payload.reply, /priced at £22,995 \+VAT/i);
  assert.match(response.payload.reply, /Finance from £480 per month/i);
  assert.doesNotMatch(response.payload.reply, /CMS specification|Current page pricing/i);
...
test("site-wide Rent2Buy resolution remains product-separated from Finance fields", async () => {
  ...
  assert.match(response.payload.reply, /YR22 OKJ/i);
  assert.match(response.payload.reply, /initial rental is £1,800 \+ VAT/i);
  assert.match(response.payload.reply, /fixed monthly payments of £515 \+ VAT/i);
