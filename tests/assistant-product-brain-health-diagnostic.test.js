...
test("diagnose sealed-brain deterministic health failures", async () => {
...
Approval is subject to assessment. Delivery timing and vehicle availability must be confirmed.`;
  ...
  console.log("SEALED_BRAIN_HEALTH_DIAGNOSTIC", JSON.stringify({
    score: report.overall_ai_health_score,
    ...
    failed_scenarios: report.failed_scenarios.map((item) => ({
      id: item.id,
      name: item.name,
