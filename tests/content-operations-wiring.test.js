...
test("global browser anchor interception is removed", async () => {
  ...
  assert.doesNotMatch(service, /document\.querySelector\("\.youtube-generator"\)/);
...
test("all four successful YouTube video paths call direct tracking", async () => {
  ...
  assert.match(page, /handleDownloadMp4[\s\S]*format:\s*"mp4"[\s\S]*queueDownload:\s*false/);
  assert.match(page, /handleDownloadWebm[\s\S]*format:\s*"webm"[\s\S]*queueDownload:\s*false/);
  assert.match(page, /exportQueuedVehicle[\s\S]*format:\s*"mp4"[\s\S]*queueDownload:\s*true/);
  assert.match(page, /exportQueuedVehicle[\s\S]*format:\s*"webm"[\s\S]*queueDownload:\s*true/);
});
...
test("structured vehicle identity and queue status are supplied directly", async () => {
  ...
  assert.doesNotMatch(`${service}\n${page}`, /page\.textContent|querySelector\("\.youtube-generator"\)/);
});

test("operation IDs are created before downloads and reused for tracking retries", async () => {
  const page = await read("../pages/YouTubeGeneratorPage.jsx");
  const service = await read("../services/marketingDailyOperations.js");
  assert.match(page, /const operationId = createYouTubeExportOperationId\(\);[\s\S]*downloadYouTubeMp4FromServer[\s\S]*operationId/);
  assert.match(service, /sourceId:\s*`youtube-export:\$\{productKey\}:\$\{operationId\}`/);
  assert.match(service, /await recordDailyMarketingActivity\(activityType, options\)[\s\S]*await recordDailyMarketingActivity\(activityType, options\)/);
  assert.notEqual(createYouTubeExportOperationId(), createYouTubeExportOperationId());
});
...
test("Cars and non-video exports are excluded", async () => {
  ...
  assert.doesNotMatch(page, /trackCompletedDownload\([\s\S]{0,120}descriptionFilenameFromMp4/);
});
...
test("tracking failure remains non-blocking after the browser download", async () => {
  ...
  assert.match(page, /downloadYouTubeMp4FromServer[\s\S]*await trackCompletedDownload/);
});
