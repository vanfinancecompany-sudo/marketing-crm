...
import { youtubeActivityFromDownload } from "../services/marketingDailyOperations.js";
...
test("London dates and day ranges respect summer and winter time", () => {
  ...
  assert.deepEqual(londonDateRange("2026-07-19"), { start: "2026-07-18T23:00:00.000Z", end: "2026-07-19T23:00:00.000Z" });
  assert.deepEqual(londonDateRange("2026-01-19"), { start: "2026-01-19T00:00:00.000Z", end: "2026-01-20T00:00:00.000Z" });
});
...
test("Knowledge Hub target defaults to 2 and participates in completion", () => {
  ...
  const summary = summarizeDailyActivity({ targets: DEFAULT_DAILY_TARGETS, events: [{ activity_type: "knowledge_hub_article", quantity: 1 }] });
...
test("effective schedules preserve history and a one-day override wins", () => {
  const schedules = [{ effective_from: "2026-01-01", weekday: 1, ...DEFAULT_DAILY_TARGETS }, { effective_from: "2026-07-01", weekday: 1, ...DEFAULT_DAILY_TARGETS, emails_sent: 250 }];
...
test("legacy creative reels and explicit YouTube reel events are counted without creative overlap", () => {
  const summary = summarizeDailyActivity({
    ...
    events: [
      ...
      { activity_type: "rent2buy_reel", quantity: 1, source: "youtube_generator" },
      { activity_type: "van_finance_reel", quantity: 1, metadata: { creative_id: "finance-reel" } },
    ],
    generatedReels: [
...
test("YouTube MP4 and WebM exports map to the correct activity and stable source id", () => {
  const finance = youtubeActivityFromDownload({ filename: "van-finance-ab12xyz-youtube-short.mp4", href: "blob:finance-1", pageText: "MP4 ready, downloading" });
  const rent = youtubeActivityFromDownload({ filename: "rent2buy-cd34efg-youtube-short.webm", href: "blob:rent-1", pageText: "Downloading MP4 2 of 4" });
  assert.equal(finance.activityType, "van_finance_reel");
  assert.equal(finance.metadata.queue_download, false);
  assert.equal(rent.activityType, "rent2buy_reel");
  assert.equal(rent.metadata.format, "webm");
  assert.equal(rent.metadata.queue_download, true);
  assert.equal(finance.sourceId, youtubeActivityFromDownload({ filename: "van-finance-ab12xyz-youtube-short.mp4", href: "blob:finance-1", pageText: "" }).sourceId);
});

test("cars and non-video downloads create no reel activity", () => {
  assert.equal(youtubeActivityFromDownload({ filename: "car-finance-ab12xyz-youtube-short.mp4", href: "blob:car" }), null);
  assert.equal(youtubeActivityFromDownload({ filename: "van-finance-ab12xyz-description.txt", href: "blob:text" }), null);
});

test("period totals include Knowledge Hub articles", () => {
  const first = summarizeDailyActivity({ targets: DEFAULT_DAILY_TARGETS, events: [{ activity_type: "knowledge_hub_article", quantity: 1 }] });
  const second = summarizeDailyActivity({ targets: DEFAULT_DAILY_TARGETS, events: [{ activity_type: "knowledge_hub_article", quantity: 2 }] });
...
test("completed production send batches keep existing email totals", () => {
  const summary = summarizeDailyActivity({ targets: DEFAULT_DAILY_TARGETS, emailSends: [{ id: "batch-one", send_type: "production", status: "completed", sent_count: 100 }, { id: "batch-one", send_type: "production", status: "completed", sent_count: 100 }, { id: "batch-two", send_type: "production", status: "partially_failed", sent_count: 25 }] });
