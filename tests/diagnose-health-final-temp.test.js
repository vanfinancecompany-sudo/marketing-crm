...
test("TEMP diagnose remaining health class failure", async () => {
  ...
  console.log("FINAL_DIAG_METRICS", JSON.stringify({ overall: report.overall_ai_health_score, product: report.product_separation_accuracy, retrieval: report.knowledge_retrieval_accuracy }));
  const relevant = report.failed_scenarios.filter((scenario) => scenario.failures.some((failure) => ["product_separation", "knowledge_retrieval", "unsafe_promise", "awkward_clarification"].includes(failure.rule)));
  console.log("FINAL_DIAG_FAILURES", JSON.stringify(relevant, null, 2));
});
