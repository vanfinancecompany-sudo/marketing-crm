...
test("DealerKit Stock Watch retries the source but still exposes positive truth from a partial snapshot", () => {
  const source = read("api/dealerkit-stock-watch-list.js");

  assert.match(source, /loadSnapshot\(\{[\s\S]*allowPartial: true,[\s\S]*stabilityAttempts: 3,[\s\S]*\}\)/);
